...
import { SP, R, FS, FW, NURSING_SVCS, CITIES, INSURANCE, C } from '../../constants';
...
const INIT: NurseRegData = {
  ...
  scfhsNumber: '', scfhsExpiry: '', nationalId: '', crNumber: '', mohLicense: '', iban: '', accountHolderName: '',
  ...
  return (
    <NScroll>
      ...
      <View style={{ flexDirection: AR ? 'row-reverse' : 'row', gap: SP.md, marginBottom: SP.lg }}>
        <View style={{ flex: 1 }}><NBtn label={AR ? 'تحديد الكل' : 'Select All'} variant="outline" size="sm" onPress={() => update({ enabledServices: NURSING_SVCS.map(s => s.id) })} /></View>
      ...
      <NCard style={{ backgroundColor: theme.successBg, marginBottom: SP.lg }}>
        <Text style={{ fontSize: FS.sm, color: theme.success, textAlign: AR ? 'right' : 'left' }}>
          {AR ? `${data.enabledServices.length} خدمة مختارة من ${NURSING_SVCS.length}` : `${data.enabledServices.length} of ${NURSING_SVCS.length} selected`}
        </Text>
      </NCard>

      {NURSING_SVCS.map(svc => {
  ...
  return (
    <NScroll>
      ...
      <TouchableOpacity onPress={() => setShowVacationCal(true)}>
        <NInput label={AR ? 'إجازة مخططة (إيقاف الحجوزات)' : 'Planned Vacation'} placeholder={AR ? 'اختر التاريخ من التقويم...' : 'Choose vacation date...'} value={data.vacationDate} editable={false} onChange={() => {}} icon="📅" />
      </TouchableOpacity>
