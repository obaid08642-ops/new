...
import { extractRecord, profileDomainState, readProfileFields, type ProfileDomainState } from "@/lib/api/profile";
import { requirePatientAccess } from "@/lib/auth/session";
import { isLocale } from "@/lib/i18n";

type Props = { params: Promise<{ locale: string }> };
type Domain = { title: string; fields: { key: string; value: string }[]; state: ProfileDomainState };
...
export default async function ProfilePage({ params }: Props) {
  ...
  const [identity, medical, insurance] = await Promise.all([
    ...
    resolveDomain(medicalResponse, ["bloodType", "height", "weight"]),
