...
export default async function CartPage({ params }: Props) {
  ...
  return <main className={`main ${styles.page}`}>
    ...
    {hasItems ? <><section className={styles.groups}>{cart.groups.filter((group) => group.items.length).map((group) => <article className={styles.group} key={group.kind}><div className={styles.groupHead}><h2>{group.kind}</h2><span>{group.count ?? group.items.length} {t("itemCount")}</span></div>{group.items.map((item) => <div className={styles.item} key={item.lineId}><div><strong>{item.name || item.serviceId}</strong><span>{item.quantity ?? 1} × {item.price ?? 0} {cart.currency || t("currency")}</span></div><span>{item.paymentMethod || "—"}</span></div>)}</article>)}</section><section className={styles.total}><span>{t("subtotal")}</span><strong>{cart.subtotal ?? 0} {cart.currency || t("currency")}</strong><span>{t("homeVisitFee")}</span><strong>{cart.homeVisitFee ?? 0} {cart.currency || t("currency")}</strong><span>{t("total")}</span><strong>{cart.total ?? 0} {cart.currency || t("currency")}</strong></section></> : <section className={styles.state}><ShoppingCart size={25} aria-hidden="true" /><h2>{t("empty")}</h2><Link className={styles.back} href={`/${locale}/medicines`}>{t("back")}<Direction size={17} aria-hidden="true" /></Link></section>}
  </main>;
}
