...
export default async function SettingsPage({ params }: Props) {
  ...
  return <main className={`main ${styles.page}`}>
    ...
    <section className={styles.grid}>
      ...
      <article className={styles.card}><span className={styles.icon}><MonitorSmartphone size={20} aria-hidden="true" /></span><div><h2>{t("sessionsTitle")}</h2>{sessions.length ? <ul>{sessions.map((session, index) => <li key={`${session.device ?? "unknown"}-${index}`}><span>{session.device || t("deviceUnknown")}</span><strong>{session.expiresInSeconds === undefined ? t("notAvailable") : t("expiresIn", { value: Math.ceil(session.expiresInSeconds / 86400) })}</strong></li>)}</ul> : <p>{t("sessionsEmpty")}</p>}</div></article>
    </section>
