...
import { getPatientPrivacySettings, getPatientSecuritySettings, getPatientStorage } from "@/lib/api/settings-server";
import { parsePrivacySettings, parseSecuritySettings, parseStorageSummary } from "@/lib/api/settings";
...
export default async function SettingsPage({ params }: Props) {
  ...
  const [privacyResponse, securityResponse, storageResponse] = await Promise.all([
    getPatientPrivacySettings(token),
    getPatientSecuritySettings(token),
    getPatientStorage(token),
  ]);
  const responses = [privacyResponse, securityResponse, storageResponse];
