...
export default async function InsurancePage({ params }: Props) {
  ...
  return <main className="main">
    ...
    <section className={styles.claimsSection} aria-labelledby="claims-title"><div className={styles.claimsHeading}><div><p className={styles.eyebrow}><FileCheck2 size={15} aria-hidden="true" />{t("claimsEyebrow")}</p><h2 id="claims-title">{t("claimsTitle")}</h2></div><p>{t("claimsNotice")}</p></div>
      {claims.length === 0 ? <div className={styles.state}><p>{t("claimsEmpty")}</p></div> : <div className={styles.claimsList}>{claims.map((claim) => <article className={styles.card} key={claim.id}><div className={styles.claimTop}><strong>{claim.service || t("claimServiceUnknown")}</strong><span className={styles.status}>{statusLabel(claim.status)}</span></div>{claim.date ? <p className={styles.date}>{claim.date}</p> : null}</article>)}</div>}
    </section>
  </main>;
}
