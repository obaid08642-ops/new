...
vi.mock("@/lib/api/medicines-server", () => ({ getPatientMedicines: state.getPatientMedicines, getPatientMedicine: state.getPatientMedicine }));
...
describe("medicines SSR boundary", () => {
  beforeEach(() => {
    ...
    state.getPatientMedicine.mockReset();
  ...
  it("renders medicine detail through the server boundary without embedding the token or a price", async () => {
    state.getPatientMedicine.mockResolvedValue(new Response(JSON.stringify({ id: medicineId, name_en: "Catalog medicine", active_ingredient: "Ingredient", price: 99 }), { status: 200 }));

    const html = renderToStaticMarkup(await MedicineDetailPage({ params: Promise.resolve({ locale: "en", medicineId }) }));

    expect(state.getPatientMedicine).toHaveBeenCalledWith(serverToken, medicineId);
    expect(html).not.toContain(serverToken);
    expect(html).not.toContain("99");
  });
});
