...
type Reminder = { id: string; medicine_name_ar?: string; medicine_name_en?: string; dose: string; dosage_count?: number; dosage_form?: string; frequency?: 'daily' | 'weekly' | 'as_needed' | string; instructions_ar?: string | null; chronic?: boolean; active?: boolean; times?: string[]; today_doses?: Dose[] };
const reminderName = (item: Reminder, fallback: string) => item.medicine_name_ar || item.medicine_name_en || fallback;
const frequencyLabel = (reminder: Reminder, t: any) => {
  if (reminder.frequency === 'daily') return t('daily');
  if (reminder.frequency === 'weekly') return t('weekly');
  if (reminder.frequency === 'as_needed') return t('asNeeded');
  return String(reminder.frequency || '').trim().slice(0, 30) || '—';
};
...
function ReminderCard({ reminder, colors, t, actionKey, onLog, onStop, onSync }: { reminder: Reminder; colors: any; t: any; actionKey: string | null; onLog: (r: Reminder, d: Dose, s: 'taken' | 'skipped') => void; onStop: (id: string) => void; onSync: (r: Reminder) => void }) {
  ...
  const dosage = reminder.dosage_count == null ? reminder.dose : `${reminder.dose} · ${reminder.dosage_count} ${t('unitsPerDose')}`;
