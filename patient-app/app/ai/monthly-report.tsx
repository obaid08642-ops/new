...
export default function MonthlyReportScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      const collectionAt = (index: number) => results[index].status === 'fulfilled' ? results[index].value : [];
      setAppointments(collectionAt(0));
      setVitals(collectionAt(1));
      setMeds(collectionAt(2));
      setTrends(collectionAt(3));
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {!loadError && trends.filter((t: any) => t?.data?.length).length > 0 && (
