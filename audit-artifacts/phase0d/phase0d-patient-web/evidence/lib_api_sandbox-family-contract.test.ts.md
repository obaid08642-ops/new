...
- **Member path:** `lib/api/sandbox-family-contract.test.ts`
- **Member SHA-256:** `7232c8cf4458e1afa7cff4a6199c171c5eb2b520538ae6440800dd70764a1642`
- **Line count:** 41
- **Read range:** `1-41`
...
- `27: const ownerToken = await login(process.env.NABD_SANDBOX_OWNER_EMAIL, process.env.NABD_SANDBOX_OWNER_PASSWORD, baseUrl as string);`
### backend_consumers_or_contracts
- `14: const response = await fetch(`${baseUrl}/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ identifier, password }), signal: AbortSignal.timeout(12_000) });`
- `28: const list = await fetch(`${baseUrl}/family/members`, { headers: { authorization: `Bearer ${ownerToken}` }, signal: AbortSignal.timeout(12_000) });`
- `38: const response = await fetch(`${baseUrl}/family/members`, { signal: AbortSignal.timeout(12_000) });`
### auth_ownership
...
- `27: const ownerToken = await login(process.env.NABD_SANDBOX_OWNER_EMAIL, process.env.NABD_SANDBOX_OWNER_PASSWORD, baseUrl as string);`
- `28: const list = await fetch(`${baseUrl}/family/members`, { headers: { authorization: `Bearer ${ownerToken}` }, signal: AbortSignal.timeout(12_000) });`
- `34: it("rejects the self-scoped family list without a patient session", async () => {`
### state_transitions
- `29: expect(list.status).toBe(200);`
- `39: expect(response.status).toBe(401);`
### payment_insurance_relevance
- `30: const payload: unknown = await list.json();`
