...
- **Member path:** `scripts/verify-sandbox-notification-fields.mjs`
- **Member SHA-256:** `b96f34277762d9f269d2441d27876410d12848b6863b8958e94e4cc24b1763f8`
- **Line count:** 29
- **Read range:** `1-29`
...
- `21: const login = await fetch(`${baseUrl}/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ identifier, password }), signal: AbortSignal.timeout(12_000) });`
- `25: const response = await fetch(`${baseUrl}/notifications`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });`
### auth_ownership
...
- `25: const response = await fetch(`${baseUrl}/notifications`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });`
### state_transitions
- `5: if (!baseUrl || !identifier || !password) throw new Error("Notification verification requires configured Sandbox environment variables");`
- `22: if (!login.ok) throw new Error(`Sandbox login returned ${login.status}`);`
- `24: if (!accessToken) throw new Error("Sandbox login response did not contain an access token");`
- `26: if (!response.ok) throw new Error(`Notifications returned ${response.status}`);`
- `29: console.log(`notifications: ${keys.length ? `keys=${keys.join(",")}` : "empty result"}`);`
...
- `17: for (const value of [payload.data, payload.items, payload.results, payload.notifications]) if (Array.isArray(value)) return value[0];`
