...
- **Member path:** `scripts/verify-sandbox-home-care-fields.mjs`
- **Member SHA-256:** `79a1f1800291f119086737e39cdfaeef04b4542a2e730573c2361b44020ece97`
- **Line count:** 30
- **Read range:** `1-30`
...
- `25: const response = await fetch(`${baseUrl}/home-care/bookings/my`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });`
...
- `25: const response = await fetch(`${baseUrl}/home-care/bookings/my`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });`
### state_transitions
- `5: if (!baseUrl || !identifier || !password) throw new Error("Home-care verification requires configured Sandbox environment variables");`
- `22: if (!login.ok) throw new Error(`Sandbox login returned ${login.status}`);`
- `24: if (!accessToken) throw new Error("Sandbox login response did not contain an access token");`
- `26: if (!response.ok) throw new Error(`Home-care list returned ${response.status}`);`
- `30: console.log(`home-care: ${keys.length ? `keys=${keys.join(",")}` : "empty result"}; id_format=${id ? (id.length === 36 ? "uuid_length" : `length_${id.length}`) : "none"}`);`
