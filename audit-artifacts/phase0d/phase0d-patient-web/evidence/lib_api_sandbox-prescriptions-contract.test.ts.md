...
- **Member path:** `lib/api/sandbox-prescriptions-contract.test.ts`
- **Member SHA-256:** `9d69aac27c04a6d1d97a1b0d149b7f541a1cd68e5bd0e4a51beec3c206a9de12`
- **Line count:** 41
- **Read range:** `1-41`
...
- `24: it("reads the current patient's prescription list without sending, substituting, or uploading", async () => {`
...
- `14: const response = await fetch(`${baseUrl}/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ identifier, password }), signal: AbortSignal.timeout(12_000) });`
- `28: const response = await fetch(`${baseUrl}/prescriptions/mine`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });`
- `38: const response = await fetch(`${baseUrl}/prescriptions/mine`, { signal: AbortSignal.timeout(12_000) });`
### auth_ownership
...
- `27: const accessToken = await login(process.env.NABD_SANDBOX_OWNER_EMAIL, process.env.NABD_SANDBOX_OWNER_PASSWORD, baseUrl as string);`
- `28: const response = await fetch(`${baseUrl}/prescriptions/mine`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });`
- `34: it("rejects the self-scoped prescription list without a patient session", async () => {`
### state_transitions
- `24: it("reads the current patient's prescription list without sending, substituting, or uploading", async () => {`
