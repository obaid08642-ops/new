...
## 18. Phase 5 — توطين تطبيق المريض للغات الست

| المعرّف | النتيجة المثبتة | المعالجة المنفذة | القيد أو التحقق اللاحق |
|---|---|---|---|
| `I18N-01` | منصة اللغة المركزية تدعم `ar/en/ur/hi/bn/fil`، مع RTL للعربية والأردية؛ اخترت الفلبينية بصيغة `fil` وفي بيانات الكتالوج `tl` | تم تثبيت مسار اختيار لغة موحد في AppContext وLanguageManager وConfigManager وقاموس الواجهة؛ تظهر اللغات الست في welcome | يلزم اختبار تغيير اللغة على جهاز فعلي للتحقق من اتجاه RTL وإعادة تحميل التخطيط حيث تفرضه المنصة |
| `I18N-02` | جرد المصدر الحي فحص **493** ملف TS/TSX وحدد **1,478** نصاً عربياً ثابتاً مرشحاً؛ كان القاموس التلقائي اليدوي يغطي 119 فقط | تمت ترجمة **1,445** عبارة غير مغطاة إلى EN/UR/HI/BN/FIL في `autoTranslationsPhase5.json`، مع منع القيم الفارغة والتحقق من اكتمال اللغات الست | الترجمة منظمة ومتكاملة بنيوياً، لكن تعتمد مراجعة لغوية بشرية لعينة UX طبية قبل نشر المتاجر، خصوصاً تعبيرات التأمين والطوارئ والأدوية |
| `I18N-03` | كانت **799** عقدة React Native `Text` مباشرة في routes لا تمر عبر `autoTranslate` | أضيف `LocalizedText` وحُولت **41** شاشة، بحيث تمر النصوص الساكنة عبر القاموس مع حفظ JSX والأيقونات والقيم الديناميكية | عناصر النص داخل مكتبات طرف ثالث أو محتوى backend ليست مضمونة بقاموس العميل؛ يلزم عقد i18n للخادم في Phase 6 |
| `I18N-04` | كانت رسائل `Alert.alert` الحرفية خارج تغطية عناصر UI، بما فيها عناوين ورسائل الأخطاء | أضيف `showLocalizedAlert` وحُولت استدعاءات alert في routes إلى بوابة توطن العنوان والرسالة وتسميات الأزرار | رسائل أخطاء backend الخام لا ينبغي عرضها للمستخدم؛ تحتاج مواءمة error-code→i18n key في Phase 6 |
| `CFG-06` | AppContext كان يتضمن localhost runtime fallback وبيانات ميزات وتسعير وتواصل ثابتة، ويقبل لغة تخزين غير متحقق منها | صار يجلب `/config` من `ConfigManager.apiBaseUrl` ويبدأ بـruntime config فارغ؛ يُقبل saved language فقط إذا كان ضمن اللغات الست، وحُذفت بيانات الاتصال/التسعير/الميزات الثابتة | لا تثبت هذه المعالجة أن endpoint `/config` متاح في staging؛ يجب أن تعطي الواجهة حالة عدم إتاحة لا قيمة بديلة |

### بوابات Phase 5

| البوابة | النتيجة |
|---|---|
| اكتمال قاموس Phase 5 | **1,445 / 1,445** مدخلًا؛ كل مدخل يحوي AR/EN/UR/HI/BN/FIL بلا قيمة فارغة |
| مسح routes | لا عنصر `<Text>` مباشر ولا `Alert.alert` مباشر متبقٍ في نطاق `app/`؛ تمر النصوص الثابتة عبر البوابات المركزية |
| مسح AppContext | لا localhost أو هاتف/بريد دعم أو ميزات/رسوم ثابتة ضمن runtime config fallback |
| `patient: expo export --platform ios` | ناجح |
| `patient: npm test -- --runInBand` | **7 suites / 23 tests** ناجحة |

> **قيد Phase 5:** نجاح البنية والقاموس لا يكافئ مراجعة مترجم طبي أصلي أو اختبار تنسيق واجهات كل لغة على iOS/Android. لا يُدّعى اكتمال التوطين في محتوى API الديناميكي، أو النصوص داخل SDKs الخارجية، أو رسائل backend؛ يعالج ذلك بعقود error codes ومحتوى مترجم ثم UAT متعدد اللغات في staging.

## 19. بوابة الانتقال إلى Phase 6

أغلقت Phase 5 التوطين المصدرّي القابل للإثبات للواجهة الساكنة، مع حفظ حدوده بدقة. تتجه Phase 6 إلى عقود QR ومواقع الطوارئ وتهيئة runtime وأخطاء API والدفع/التأمين، ثم توثيق سيناريوهات E2E اللازمة لـstaging دون اختبار أو تعديل إنتاجي.

## 20. Phase 6 — العقود الحساسة ومواءمة الواجهات المتبقية

| المعرّف | الدليل المصدرّي | المعالجة المنفذة | النتيجة أو القيد |
|---|---|---|---|
| `CON-01` | كان endpoint `GET /config` يعيد version وAPI URL وخصائص وتسعيراً وقنوات دعم ثابتة، بينما صار AppContext يستهلكه كمصدر runtime | صار endpoint يعيد فقط حقولاً مضبوطة صراحة من البيئة (`APP_VERSION` وfeature flags والأسعار وقنوات الدعم)، ويحذف أي field غير مضبوط بدلاً من تلفيقه | لا يثبت ذلك صحة runtime config في بيئة؛ يلزم ضبط المتغيرات في staging واختبار سلوك غيابها |
| `SEC-06` | `GET /medical-profile/provider/:patientId` كان يقرأ ملفاً طبياً كاملاً بالمعرّف فقط؛ وأثبتت الفهرسة غياب مستهلك حي وعقد موافقة/علاقة علاجية مسجل | أغلق المسار fail-closed بـ403 إلى حين وجود موافقة صريحة قابلة للتدقيق وعلاقة رعاية نشطة | لا يوجد وصول ملف طبي للمزوّد حتى اعتماد: grant/revoke consent، scope، relationship lookup، audit event، وواجهة موافقة للمريض |
| `CON-02` | جواز المريض يصدر JWT QR قصير العمر، لكن لم يوجد endpoint تحقق QR ولا مستهلك موثّق؛ واجهة مزوّد QR كانت تعرض ID افتراضياً وكاميرا/حفظ/مشاركة نجاحات شكلية | أزيلت واجهة إنشاء/مسح QR الوهمية وحولت إلى حالة عدم إتاحة صادقة تشرح شرط التحقق الآمن والموافقة وتدقيق الوصول | لا يجوز تمكين QR صحي قبل endpoint verifier مقيّد بالدور والموافقة وتسجيل audit وإبطال/انتهاء token واختبار E2E |
| `CON-03` | شاشة الطوارئ لا تستقبل polyline أو مساراً مصرحاً؛ backend يعيد موقع الوحدة والحالة فقط | أبقت Phase 3 الواجهة على حالة GPS والبعد ووقت التحديث بدلاً من رسم route مصطنع | يحتاج المسار الحقيقي موافقة location، سياسة retention، schema لنقاط المسار، وservice directions محكم؛ لا يُضاف افتراضياً |
| `CON-04` | رسائل أخطاء API تختلف بين الخدمات وبعضها قد يصل كنص خام | لم يُنشأ قاموس أخطاء متخيل؛ واجهات العميل تملك حواجز صادقة محلياً فقط | Phase 7 يتطلب قائمة error codes/HTTP outcomes مع owner لكل عقد قبل UAT متعدد اللغات |

### بوابات Phase 6

| البوابة | النتيجة |
|---|---|
| فحص runtime config الثابت | ناجح؛ لا version/API URL/دعم/أسعار افتراضية في `ConfigService` |
| backend build | ناجح |
| backend tests | **26 suites / 211 tests** ناجحة |
| فحص QR الوهمي في تطبيق المزوّد | ناجح؛ لا ID افتراضي أو نجاح كاميرا/حفظ/مشاركة في `QRCodeSystem` |
| provider iOS export | ناجح |
| provider tests | **1 suite / 3 tests** ناجحة |

> **حكم Phase 6:** الحماية المصدرية أُغلقت، لكن عقود consent/QR/location/error-code لا تزال **تصميمات مطلوبة قبل الإطلاق** وليست ميزات مكتملة. لا يبرر build الناجح تمكينها أو تجاوز تجربة staging وE2E.

## 21. بوابة الانتقال إلى Phase 7

تنتقل المصالحة إلى إعداد حزمة staging وE2E والتقرير الحاكم النهائي. تشمل المتطلبات المتبقية تدوير اعتماد R2 المكشوف تاريخياً خارج Git، ضبط البيئة والمتغيرات، إنشاء حسابات اختبار منفصلة عن الإنتاج، تنفيذ acceptance matrix لكل دور، والمراجعة البشرية للتوطين.

## 22. Phase 7 — خطة staging وE2E وحكم الجاهزية

أُعدت وثيقتا تسليم مستقلتان بلا أسرار أو حسابات اختبار: `NABDAH_FINAL_REMEDIATION_AND_RELEASE_READINESS_20260815.md` كملخص تنفيذي للمبرمج وقرار الإطلاق، و`POST_REMEDIATION_E2E_EXECUTION_PLAN.md` كمصفوفة تنفيذ staging. تغطي الخطة الهوية وOTP وCORS وWebSocket وBOLA/IDOR والحجز والمختبر والصيدلية والوصفة وOCR والدفع والويبهوك والمزوّد والإدارة والتوطين وإمكانية الوصول.

| البند | الحكم |
|---|---|
| حفظ الحزم الأربع المعالجة | مكتمل على `manus/on-live-reconciliation` |
| البوابات المحلية | ناجحة ضمن النتائج المثبتة لكل مرحلة |
| تقرير Markdown موحد للمبرمج | مكتمل |
| خطة E2E/staging مفصلة | مكتملة |
| تدوير اعتماد R2 المكشوف تاريخياً | **واجب تشغيلي مفتوح** |
| lockfiles وdependency audit | **واجب هندسي مفتوح** |
| consent/QR/location/error-code contracts | **عقود مفتوحة قبل تمكين المزايا** |
| E2E وUAT متعدد اللغات على staging | **إلزامي قبل الإصدار** |

> **القرار النهائي:** لا يُدمج هذا الفرع في `main` ولا يُنشر إلى الإنتاج أو المتاجر اعتماداً على نتائج المصدر والبناء وحدها. الجاهزية الحالية هي **مناسبة لمراجعة الكود والتحضير لـstaging فقط**؛ قبول الإصدار يحتاج أدلة E2E موثقة، مراجعة تشغيل وأمن، وموافقة المنتج/الخصوصية على المزايا الطبية والمالية الحساسة.

## 23. استئناف ما بعد Phase 7 — عقد موظفي المنشأة

كشف جرد ما بعد Phase 7 عدم تطابق فعلي بين شاشة `FacilityDashboard` في تطبيق المزوّد وعقد الخلفية المسجل `HospitalStaffModule`: كانت الشاشة تستدعي `/provider/features/staff` وترسل `role`، بينما العقد الحي هو `/hospital/staff` ويتطلب `staff_role` ويعيد `id` للموظف المنشأ. صُحح consumer ليستخدم المسار والحقول والاستجابة الحية، وأزيل توليد المعرّف المحلي `NBD-*` لصالح المعرّف العائد من الخادم.

| بوابة الإصلاح | النتيجة |
|---|---|
| تطبيق المزوّد — Jest | 3/3 ناجحة |
| تطبيق المزوّد — Expo iOS export | ناجح |
| فرع `main` | لم يُعدّل |
| اختبارات API/staging الفعلية | لم تُنفذ؛ تحتاج بيئة منفصلة |

تبقى مراجعة بقية مسارات المنشأة والموظفين، وبقية العقود الحساسة المحددة في خطة E2E، أعمالاً مفتوحة ولا يُستنتج من نجاح البناء أنها مقبولة تشغيلية.

## 24. تدقيق تنفيذ الخطة — مصفوفة 16 أغسطس 2026

أعيدت مطابقة مراحل Phase 0 إلى Phase 7، والالتزامات، وبوابات البناء والاختبار، وخطة staging/E2E، ثم أُنشئت الوثيقة `EXECUTION_COMPLETION_MATRIX_20260816.md`. الحكم المصحح هو أن المعالجة المصدرية المحلية أغلقت ما أمكن إثباته، لكنها لا تعني تنفيذ كل الخطة: جميع سيناريوهات E2E في الخطة ما زالت غير منفذة على staging، وعقود consent/QR/location/error-code غير معتمدة، وتدوير R2 وlockfiles وUAT اللغات والأجهزة الفعلية مفتوحة.

كما وسّع التدقيق إصلاح facility بعد Phase 7: عولجت قراءات `/provider/facility/subaccounts` غير المعرفة، ومعرّفات `NBD-*` و`lastLogin` المصطنعة، والحذف المحلي، وبطاقة QR والحفظ والمشاركة الشكلية. صارت الشاشة تستخدم `/hospital/staff` و`DELETE /hospital/staff/:id` أو حالة عدم إتاحة صادقة. نجحت بوابة المزوّد 3/3 وتصدير iOS. يبقى اختبار ملكية مالك المنشأة في staging إلزامياً.
...
## ملحق استئناف الخطة الأساسية — 2026-08-17

أعيد فتح الخطة الأساسية بعد ملاحظة أن معالجة Gatekeeper السابقة أغلقت عيوباً محددة فقط ولم تغلق كل بنود Phase 0–7 وخطة E2E. تمت إعادة قراءة مصفوفة الإكمال وخطة E2E الرسمية، وثُبت أن consent/QR/location/error-code، lockfiles، مراجعة اللغات والأجهزة، وE2E للهوية والمال والصيدلية وWebSocket ما زالت تتطلب أدلة مستقلة.

في دفعة مصدرية لاحقة عولجت مفاتيح LiveKit الوهمية، وجعل غياب Redis في Device Trust فشلاً آمناً مع مطابقة مالك challenge، وأضيفت ownership checks إلى عمليات provider features الحساسة، ومُنع معرف تقرير أشعة صناعي، وأصلحت حدود refund عند 24 ساعة. تمت إضافة اختبارات سلبية لهذه العقود، واجتازت الدفعة TypeScript و`27 suites / 218 tests`، وأعيد بناء أرشيف backend النظيف دون node_modules أو dist أو pycache.

يبقى حكم المصالحة: **مناسب لمراجعة المصدر والتحضير المرحلي، وليس جاهزاً للإنتاج أو المتاجر**. staging الحالية أثبتت login وradiology، لكنها ما زالت تعيد 403 للمختبر و500 لموظفي المنشأة، ولذلك يجب نشر النسخة الأخيرة ثم إعادة مصفوفة E2E الكاملة. لا تُعتبر البنود التشغيلية أو العقودية مغلقة من هذا الملحق.
