...
export async function scheduleMedicationNotifications(
  ...
  // Never reinterpret an unknown recurrence as a daily alert. The Backend
  // persists only daily/weekly/as-needed; an unexpected value must remain
  // unscheduled until the saved contract is corrected.
  if (!['daily', 'weekly', 'as_needed'].includes(String(reminder.frequency || '')) || !reminder.active || reminder.frequency === 'as_needed' || Platform.OS === 'web') {
