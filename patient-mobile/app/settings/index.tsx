...
import { BASE_URL, clearStoredSession } from "../../src/utils/api";
...
export default function SettingsScreen() {
  ...
  const handlePress = (item: SettingsItem) => {
    ...
    if (item.label === "تسجيل الخروج") {
      // F-C8: logout must fully destroy the session — revoke server-side, wipe
      // SecureStore (access+refresh), and clear persisted redux state. Previously
      // tokens survived logout, exposing PHI on shared devices.
      (async () => {
        try {
