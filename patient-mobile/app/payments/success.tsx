...
export default function PaymentSuccessScreen() {
  ...
  // F-M4: deep-link params alone are spoofable — re-verify the transaction
  // server-side before presenting the receipt as truth.
  const [verified, setVerified] = useState(!params.moyasarId);
  useEffect(() => {
    const payId = params.moyasarId ? String(params.moyasarId) : null;
    if (!payId) return;
    let alive = true;
    (async () => {
      try {
        await apiFetch(`/payments/verify/${payId}`, { method: 'POST' });
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background } ]}>
      {!verified && (
        <View style={{ backgroundColor: '#B45309', paddingVertical: 8, paddingHorizontal: 16 }}>
          <AppText style={{ color: '#FFF', textAlign: 'center', fontSize: 12 }}>
            جارٍ التحقق من عملية الدفع من الخادم… إن استمرت هذه الرسالة فلم يتم تأكيد الدفع
          </AppText>
        </View>
      )}
