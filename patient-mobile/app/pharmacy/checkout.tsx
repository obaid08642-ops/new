...
export default function PharmacyCheckoutScreen() {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      let orderId: string = '';
      try {
        // ═══ PH-PHARMACY (governing rules) ═══
        // إرسال الطلب يبثّه للصيدليات بلا أي دفع. العروض تُجمع ثم يختار المريض عرضًا،
        // وبعدها فقط يدفع (بطاقة/COD/تحمّل التأمين).
        const created = await apiFetch('/patient/pharmacy/orders', {
          method: 'POST',
          body: JSON.stringify({
            items: payload.items.map((i: any) => ({ medicine_id: i.medicine_id, qty: i.qty, name_ar: i.name_ar })),
        ...
        throw new Error('فشل إرسال الطلب للصيدليات. يرجى التأكد من اتصالك بالإنترنت والمحاولة مجدداً.');
      }

      clearCart();
      // العروض قادمة من صيدليات متعددة — شاشة المقارنة هي الخطوة التالية (لا الدفع)
      router.replace({ pathname: '/pharmacy/offers', params: { orderId } });
    } catch (e) {
      showLocalizedAlert('خطأ', 'حدث خطأ أثناء إرسال الطلب، يرجى المحاولة مجدداً');
    } finally {
      setSubmitting(false);
    }
  };
