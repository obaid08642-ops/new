...
export default function PharmacyPaymentScreen() {
  ...
  const { orderId, kind: kindParam } = useLocalSearchParams<{ orderId: string; kind?: string }>();
  // 'pharmacy-order' = PH-PHARMACY selected-offer flow (price = offer snapshot server-side)
  const payKind = String(kindParam || '') === 'pharmacy-order' ? 'pharmacy-order' : 'pharmacy';
  ...
  const loadOrder = useCallback(async () => {
    try {
      setLoadError(false);
      // PH-PHARMACY flow stores orders under /patient/pharmacy/orders
      const data = payKind === 'pharmacy-order'
        ? await apiFetch(`/patient/pharmacy/orders/${orderId}`)
  ...
  // PH-PHARMACY: price = the SELECTED OFFER snapshot (subtotal + its delivery fee).
  const isOfferFlow = payKind === 'pharmacy-order';
  const serverTotal = isOfferFlow
    ? Math.round((Number(order?.selected_offer?.subtotal_estimate || 0) + Number(order?.selected_offer?.delivery_fee || 0)) * 100) / 100
    : isInsurance ? Number(order?.insurance_copay || 0) : Number(order?.total || 0);
  // Split wallet+card: wallet portion was already debited at order creation.
  const walletApplied = Number(order?.wallet_applied || 0);
  const amountToPay = Math.max(0, Math.round((serverTotal - walletApplied) * 100) / 100);
  const alreadyPaid = order?.payment_status === 'paid';

  const isCash = order?.payment_method === 'cash';

  const registerCod = async () => {
    if (!orderId) return;
    setProcessing(true);
    try {
      await apiFetch(`/patient/pharmacy/orders/${orderId}/cod`, { method: 'POST' });
      clearCart();
      router.replace({ pathname: '/pharmacy/order-tracking', params: { orderId } });
    } catch (e: any) {
      showLocalizedAlert('تعذر تسجيل الدفع عند التسليم', e?.message || 'حاول مجدداً');
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handlePay = async () => {
    ...
    if (amountToPay <= 0) {
      // Fully covered by insurance / wallet / points — nothing to charge.
      clearCart();
      router.replace({ pathname: '/pharmacy/order-tracking', params: { orderId } });
      return;
    }
    setProcessing(true);
    try {
      const txn = await apiFetch<any>(`/payments/intent/${payKind}/${orderId}`, { method: 'POST', headers: paymentIntentHeaders(payKind, orderId) });
    ...
    <View style={[styles.container, { backgroundColor: colors.bg, paddingTop: insets.top + 16 } ]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 180 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.amountCard, { backgroundColor: isInsurance ? '#E2F7F2' : '#DEF5F9' } ]}>
          ...
          {walletApplied > 0 && (
            <LocalizedText style={{ fontFamily: 'Cairo-Regular', fontSize: 12, color: '#2BB89C', marginTop: 4 }}>
              مدفوع من المحفظة: -{walletApplied.toFixed(2)} ر.س
            </LocalizedText>
          )}
        </View>
        ...
        <View style={[styles.methodCard, { backgroundColor: colors.s, borderColor: colors.bd, flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
          ...
          <View style={{ flex: 1, marginHorizontal: 14, alignItems: isRTL ? 'flex-end' : 'flex-start' }}>
            <LocalizedText style={{ fontFamily: 'Cairo-Bold', fontSize: 15, color: colors.n }}>بطاقة بنكية / مدى / Apple Pay</LocalizedText>
      ...
      <View style={[styles.footer, { backgroundColor: colors.s, borderTopColor: colors.bd, paddingBottom: insets.bottom + 16 } ]}>
        {isOfferFlow && !alreadyPaid && order?.status === 'offer_selected' && (
          <TouchableOpacity onPress={registerCod} disabled={processing} style={{ alignItems: 'center', paddingVertical: 10, marginBottom: 6 }}>
            <LocalizedText style={{ color: '#16A34A', fontWeight: '700' }}>
               الدفع نقداً عند التوصيل / الاستلام
            </LocalizedText>
          </TouchableOpacity>
        )}
        <TouchableOpacity
