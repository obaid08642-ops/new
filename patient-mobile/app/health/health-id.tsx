...
export default function HealthIDScreen() {
  ...
  const [healthQr, setHealthQr] = useState<{ health_id?: string; qr_payload?: string; issued_at?: string } | null>(null);

  useEffect(() => {
    (async () => {
      try {
        const [p, ec, cm, qr] = await Promise.all([
          apiFetch('/users/me/profile').catch(() => null),
          apiFetch('/health/emergency-contacts').catch(() => []),
          apiFetch('/health/chronic-meds').catch(() => []),
          apiFetch('/users/me/health-id').catch(() => null),
        ]);
        setProfile(p || null);
        setContacts(Array.isArray(ec) ? ec : []);
        setMeds(Array.isArray(cm) ? cm : []);
        setHealthQr(qr && typeof qr.qr_payload === 'string' ? qr : null);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const qrValue = healthQr?.qr_payload || null;
  const healthId = healthQr?.health_id || null;
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 80 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.idCard, { backgroundColor: '#1E3A5F' }]}>
          ...
          {/* QR contains a server-signed, short-lived health-card token only. */}
          {qrValue && (
            <View style={styles.qrSection}>
              ...
              <View style={styles.qrInfo}>
                {healthId && (
                  <AppText variant="caption" color="rgba(255,255,255,0.6)" style={{ fontWeight: '400' }}>{healthId}</AppText>
                )}
                <AppText variant="caption" color="rgba(255,255,255,0.75)" style={{ fontWeight: '400' }}>رمز مؤقت موقّع للتحقق من البطاقة الصحية</AppText>
