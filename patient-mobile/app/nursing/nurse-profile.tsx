...
export default function NursingMegaProfile() {
  ...
  const { nurseId, flow, serviceId, patientMemberId } = useLocalSearchParams();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (patientMemberId && String(patientMemberId).length > 0) payload.member_id = String(patientMemberId);

      const res = await apiFetch('/home-care/bookings', { method: 'POST', body: JSON.stringify(payload) });
      const bookingId = res?.id || res?.booking_id;

      if (flow === 'insurance') {
        // PH-SERVICE: لا دفع الآن — طلب التأمين فقط، القرار يصل كإشعار ثم يدفع
        // المريض التحمّل (co-pay) لتأكيد الحجز (نفس مسار lab/radiology).
        if (!bookingId) throw new Error('booking_failed');
        const insReq = await apiFetch('/insurance/requests', {
          method: 'POST',
          body: JSON.stringify({ booking_id: bookingId, booking_kind: 'nursing' }),
        }).catch(() => null);
        setProcessing(false);
        router.replace({
          pathname: '/insurance/payment-split',
          params: insReq?.id ? { request_id: insReq.id } : { orderId: bookingId },
        });
        return;
      }

      // Cash → ادفع الآن لتأكيد الحجز (intent سيرفري ثم شاشة المعالجة)
      const intent = await apiFetch(`/payments/intent/nursing/${bookingId}`, {
        method: 'POST',
        headers: { 'Idempotency-Key': `nur-${bookingId}-${Date.now()}` },
      }).catch(() => null);
      setProcessing(false);
      if (intent?.checkout_url || intent?.id) {
        router.replace({
          pathname: '/payments/processing',
          params: { moyasarId: intent.id, paymentUrl: intent.checkout_url || '', bookingId, bookingKind: 'nursing', amount: String(intent.amount ?? totalServiceFee) },
        });
      } else {
  ...
  // التواريخ منطقية وتسير من اليسار لليمين (اليوم أول شيئ يظهر عند offset=0)
