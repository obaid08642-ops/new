        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 28, paddingTop: 20, paddingBottom: 120, flexGrow: 1 }} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.resendContainer}>
            ...
            {timer > 0 ? (
              ...
              <TouchableOpacity
                onPress={async () => {
                  // F-C2: actually request a new code — previously this only reset
                  // the countdown, locking out users who never received an SMS.
                  setTimer(60);
                  setOtp(['', '', '', '', '', '']);
                  try {
                    await apiFetch('/auth/send-otp', {
                      method: 'POST',
                      body: JSON.stringify({ phone: phone || (params.phone as string) }),
                    });
                    Alert.alert('تم', 'تم إرسال رمز جديد');
                  } catch {
                    Alert.alert('تعذر الإرسال', 'تحقق من الاتصال وحاول مجدداً');
                    setTimer(10);
                  }
                }}
              >
