...
**الحالة:** قائمة فرضيات وفجوات مرشحة من الفحص السابق؛ **ليست حكمًا نهائيًا على `main`**.
**المصدر:** قواعد الأعمال الملزمة، تدقيق رحلات سابق، فحص مصدر جزئي، ونتيجة E2E معزولة على فرع مراجعة.
**قاعدة قراءة ملزمة:** يبدأ وكيل Audit بالتحقق من `main` عند `22526bedb77a3d8148219036367e4714f401aecc`، ويربط كل حكم بملف/route/screen/اختبار في `main`. إلى أن يفعل ذلك، تعامل حالات `MISSING` و`INCONSISTENT` و`PARTIAL` أدناه كـ**فرضيات تدقيق** لا أوامر بناء نهائية.

لا يعني `PASS` في E2E ضيق أن المجال صار كاملًا، ولا يعني وجود feature في فرع آخر أنها موجودة أو مكتملة في `main`. لا يجوز البدء في أي قصة بناء من هذا السجل قبل قبول تقرير المرحلة 0 الذي يثبتها أو يصححها.
