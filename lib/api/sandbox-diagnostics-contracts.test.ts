...
describeSandbox("Sandbox diagnostic booking contracts", () => {
  it("allows the owner to read list endpoints and rejects another patient for any available detail", async () => {
    ...
    for (const domain of ["labs", "radiology"]) {
      const list = await fetch(`${baseUrl}/${domain}/bookings/mine`, { headers: { authorization: `Bearer ${ownerToken}` }, signal: AbortSignal.timeout(12_000) });
      expect(list.status).toBe(200);
      const resourceId = firstResourceId(await list.json().catch(() => null));
      if (!resourceId) continue;

      const ownerDetail = await fetch(`${baseUrl}/${domain}/bookings/${encodeURIComponent(resourceId)}`, { headers: { authorization: `Bearer ${ownerToken}` }, signal: AbortSignal.timeout(12_000) });
      expect(ownerDetail.status).toBe(200);
      const otherDetail = await fetch(`${baseUrl}/${domain}/bookings/${encodeURIComponent(resourceId)}`, { headers: { authorization: `Bearer ${otherToken}` }, signal: AbortSignal.timeout(12_000) });
