...
export class FamilyService {
  ...
  async addCalendarEvent(userId: string, body: {
    ...
    if (!eventDate || Number.isNaN(eventDate.getTime())) throw new BadRequestException('valid event_date is required');
    const memberId = String(body.member_user_id || '').trim();
    if (!memberId) throw new BadRequestException('member_user_id is required');
    const members = Array.isArray((group as any).members) ? (group as any).members : [];
    const member = members.find((candidate: any) => candidate?.user_id === memberId);
    if (memberId !== (group as any).owner_id && !member) throw new NotFoundException('Member not found in your family group');
    const type = String(body.type || '').trim();
    if (!['appointment', 'order', 'lab', 'reminder', 'medication'].includes(type)) {
      throw new BadRequestException('valid calendar event type is required');
    }
  ...
  async getCalendarEvents(userId: string) {
    ...
    return events.map((event: any) => ({
      ...event,
      can_delete: (group as any).owner_id === userId || event.created_by === userId,
    }));
  }
  ...
  async deleteCalendarEvent(userId: string, eventId: string) {
    const group = await this.findGroupByOwnerOrMember(userId);
    if (!group) throw new NotFoundException('Not part of a family group');
    const event: any = await this.calendarM.findOne({ id: eventId, group_id: (group as any).id, is_deleted: { $ne: true } }).lean();
    if (!event) throw new NotFoundException('Calendar event not found');
    if ((group as any).owner_id !== userId && event.created_by !== userId) {
      throw new ForbiddenException('Only the event creator or group owner can delete this event');
    }
    const result: any = await this.calendarM.updateOne(
      { id: eventId, group_id: (group as any).id, is_deleted: { $ne: true } },
      { is_deleted: true },
    );
    if (result?.matchedCount === 0 || result?.nMatched === 0) throw new NotFoundException('Calendar event not found');
    return { ok: true };
  }
