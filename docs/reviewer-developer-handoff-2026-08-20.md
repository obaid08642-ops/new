...
**تحقق مرئي لاحق:** فُحصت شاشة الدخول العربية فعلياً على عرض هاتف `390px` بعد إدخال أساس التصميم، وأزيل القص الأفقي. لا يغني ذلك عن التحقق النهائي من لوحة المريض الخاصة وبقية المسارات بعد تسجيل الدخول.

**تحقق معاينة خاص:** أضيف نطاق المعاينة التطويري صراحةً إلى إعداد Next حتى تحمل موارد الواجهة عند الاختبار الداخلي. يقتصر الإعداد على بيئة التطوير ولا يغير رؤوس أو سياسات الإنتاج.

**تحقق لوحة Sandbox:** نجح تسجيل الدخول بالحساب المعزول وانتقلت المعاينة إلى لوحة المريض المعاد تصميمها. لا يظهر access token في HTML. بقيت ملاحظة واجهة مفتوحة: شريط الرأس العام يعرض «دخول المريض» حتى داخل الجلسة، ويحتاج استبداله بإجراء حساب/خروج واعٍ بالجلسة.

**تحديث تحقق الرأس:** عُرضت الآن إجراءات «الحساب» و«تسجيل الخروج» بدلاً من «دخول المريض» في لوحة Sandbox، بلا اسم أو بريد أو أي قيمة جلسة في HTML. يجري التحقق منفصلاً من إتمام الخروج ومسح الوصول الخاص.

**تحقق مسار الطلبات:** ظل نموذج دخول Sandbox متجاوباً على عرض الهاتف بعد توسيع صفحة الطلبات. يجري الآن فحص البطاقة بعد الانتقال إلى قائمة الطلبات الحية، من دون إدراج أي بيانات اختبار في الكود.

**حالة الجلسة أثناء الفحص:** أعيد تسجيل الدخول بالحساب المعزول بنجاح وعادت لوحة المريض بالرأس الواعي بالجلسة. لم تُسجل القيم السرية أو بيانات المورد في وثائق المشروع.

**تحقق المواعيد الحي:** عُرضت قائمة المواعيد الحية ببطاقات الخدمة والحالة والموعد فقط. لم تضف إعادة التصميم طبيباً أو تخصصاً أو موضعاً أو بيانات انتظار إلى HTML، وحافظت الروابط على معرف المورد ضمن مسار تفصيله المحمي فقط.

**تحقق تذكيرات الدواء الحي:** عُرضت الحالة الفارغة بوضوح على حساب Sandbox، مع إبقاء التحذير والحدود القرائية ظاهرين. لا تعرض الصفحة إجراء تسجيل تناول أو تأجيل أو جدولة أو إعادة صرف.

**تحقق الإشعارات الحي:** أظهرت الخلفية بعض حقول العنوان/النص بصيغة مفاتيح تقنية. تحجب الواجهة الآن هذه المفاتيح بدلاً من عرضها للمريض، وتعرض تسمية عامة آمنة فقط عند غياب محتوى قابل للعرض. لا تعرض أولوية خام ولا تنفذ تعليم قراءة أو روابط حمولة. يلزم Backend بإرجاع نص محلي قابل للعرض أو `title_key` و`body_key` بعقد i18n رسمي، بدلاً من وضع المفتاح في `title` أو `body`.

**تحقق Premium للهاتف:** استبدلت علامة الحرف بعلامة نبض/حماية Vector أصلية، وتحققت صفحة البداية العربية على عرض `390px`. بقيت الدعوات والأيقونات قابلة للقراءة، وتحول ترتيب البطاقات إلى عمود واحد دون قص أفقي. يظل محدد اللغات قابلاً للتمرير الأفقي عمداً لاستيعاب اللغات الست بلا تصغير غير مقروء.

**تحقق لوحة Premium الحي:** عرض حساب Sandbox لوحة المريض بعلامة نبض/حماية Vector داخل طبقة الترحيب وبطاقات وصول راقية. بقيت قائمة الروابط محكومة بالمسارات المعتمدة فقط، ولم تظهر بيانات مريض أو توكن في HTML أو في تركيبة البطاقات.

**تحقق كتالوج Premium الحي:** عُرض الكتالوج العام بواجهة بحث وبطاقات Vector مع البيانات القادمة من المصدر فقط. لا تظهر سلة أو سعر أو شراء أو وصفة أو تأمين. حدثت مهلة اتصال عابرة من المصدر العام أثناء الفحص الأول؛ تحولها طبقة الجلب الآن إلى حالة خطأ قرائية محلية قابلة لإعادة المحاولة بدلاً من استثناء SSR أو أي بيانات بديلة. يبقى المسار `noindex` بسبب G-SEO-002.

**تحقق الخط والحركة:** فُحصت الصفحة العامة بالعربية RTL والإنجليزية LTR بعد التحويل إلى سلسلة الخط النظامي. على أجهزة Apple تستخدم السلسلة خط النظام المتاح محلياً من دون توزيع SF، وتبقى بدائل النصوص صريحة لبقية الأجهزة. التحولات مقتصرة على opacity وtransform للأزرار والبطاقات والطبقة الافتتاحية، وتتوقف عملياً عند تفضيل تقليل الحركة.
