...
| سلامة كائنات Git | `git fsck --full` لم يبلغ عن فساد في التاريخ القابل للوصول، لكنه أظهر commit معزولاً واحداً `89a6e9c…`. فحص metadata يعرّفه كبقايا `git stash` بعنوان `wip-check`، غير سلف للرأس ومشتركه الأخير `4278728…`. | لا يُنسب هذا الكائن المعزول إلى العمل المعروض أو إلى خطة بناء قبل مراجعة محتواه في مساحة منفصلة. |
...
### 3.5 فحص ساكن للمواد الحساسة يحتاج تدقيق ملكية

فُحص المصدر المتتبع بحثاً عن أنماط مفاتيح خاصة شائعة من دون نسخ القيم إلى هذا التقرير. لم يظهر ملف `.env` إنتاجي متتبع، لكن ظهرت ملفات تكوين جوال (`GoogleService-Info.plist` و`google-services.json`) وملفات E2E/testing مرتبطة بإعدادات sandbox. هذه الملفات لا تثبت تسرب سر بمفردها، لكنها تستلزم تدقيق مالك المشروع للتأكد من أن كل مفتاح مقصود ومقيد بالـpackage/bundle والـAPI وأنه ليس اعتماد إنتاج قابل لإساءة الاستخدام.

**قرار:** قبل أي فرع كود أو نشر، نفّذ secret-manager inventory وrotate/restrict لكل اعتماد حقيقي، واحذف أو استبدل أي قيمة اختبارية غير مسموحة. لا تُدرج قيم الاعتمادات في Git أو تقارير المراجعة.
...
**تحديث بعد طلب المالك الصريح للرفع:** أُنشئ الفرع المعزول `quarantine/workstation-source-51a84c7` عند `6d4d42f`. يحتوي لقطة من `workstation@51a84c7` المتتبع فقط، مع `WORKSTATION_UPLOAD_MANIFEST.json` وبصمات الملفات و`UPLOAD_SCOPE.md`. استُبعدت dependencies والمخرجات وتغييرات worktree غير الملتزمة وملفات `.DS_Store` وملفا تكوين الجوال اللذان يحتاجان تدقيق اعتماد، واستُبدل رابط Provider المكسور بعلامة `provider/SOURCE_MISSING.md`. لذلك يظل الفرع **لقطة عزل قابلة للمراجعة، لا فرع دمج ولا دليلاً على الجاهزية**.
