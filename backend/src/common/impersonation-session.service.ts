...
function rolesOf(user: any): string[] { return Array.from(new Set([role(user?.role), role(user?.provider_type), role(user?.providerType)].filter(Boolean))); }
...
export class ImpersonationSessionService {
  ...
  async validate(payload: any) {
    ...
    const impersonator: any = await this.connection.collection('users').findOne({ id: session.impersonator_id }, { projection: { id: 1, role: 1, active: 1, suspended: 1, custom_role_keys: 1 } });
    ...
    const roles = rolesOf(impersonator);
    const hasPolicy = roles.some((role) => (ROLE_PERMISSIONS[role as keyof typeof ROLE_PERMISSIONS] || []).includes(Permission.USER_IMPERSONATE));
