...
export class LabBooking extends Document {
  ...
  @Prop({ default: 0 }) insurance_copay_amount: number; // written by the insurance copay mirror (insurance-engine)
  @Prop() payment_status?: string;
  @Prop() transaction_id?: string;
  @Prop() paid_at?: Date;
  // Family on-behalf booking: the account this booking is for (patient_id stays
  // the booker's family member id; booked_by_user_id pattern mirrors appointments)
