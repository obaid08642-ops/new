...
export class Appointment {
  ...
  @Prop() payment_id?: string; // gateway transaction id carried through reschedules
  // PH-SERVICE insurance branch (governing rules): decision mirror + co-pay
  @Prop({ enum: ['NONE', 'PENDING', 'APPROVED', 'PARTIAL_APPROVAL', 'REJECTED'], default: 'NONE' }) insurance_status?: string;
  @Prop({ default: 0 }) insurance_copay?: number;
  @Prop({ index: true, sparse: true }) insurance_request_id?: string;
  // PH governing: bookings on behalf of a family dependent
  @Prop({ index: true, sparse: true }) booked_for_member_id?: string;
  @Prop() booked_for_name?: string;
