...
import { Controller, Post, Get, Patch, Body, Param, Query, BadRequestException, ConflictException, NotFoundException } from '@nestjs/common';
...
import { CreateCampaignDto, CreateReferralDto, UpdateCrmTagDto, CreateStaffAccountDto, HomeCareCheckinDto, HomeCareSubmitReportDto, RadiologyUploadReportDto } from './simulated-features.dto';
...
export class SimulatedFeaturesController {
  ...
  // ===================== SUB-ACCOUNTS (STAFF) =====================
  @Post('staff')
  async createStaffAccount(@CurrentUser() user: any, @Body() body: CreateStaffAccountDto) {
    const exists = await this.userModel.findOne({ phone: body.phone });
    if (exists) throw new ConflictException('Staff phone number already exists');
    ...
    return this.userModel.create({
      ...
      parent_provider_account_id: user.id,
  ...
  @Get('staff')
  async listStaffAccounts(@CurrentUser() user: any) {
    return this.userModel.find({ parent_provider_account_id: user.id }).sort({ createdAt: -1 });
  }
