...
export class ProviderRequestsController {
  ...
  async requestInsuranceCopay(@CurrentUser() u: any, @Param('id') id: string, @Body() body: any) {
    const { approvalStatus, patientCopay, companyCopay, approvalCode, reason } = body || {};
    // 1) Load the provider request and verify ownership
    const preq: any = await this.conn.collection('provider_requests').findOne({ $or: [{ id }, { _id: id as any }] } as any);
    if (!preq) throw new NotFoundException('الطلب غير موجود');
    const owner = preq.provider_account_id || preq.provider_id;
    if (owner && String(owner) !== String(u?.id)) throw new ForbiddenException('لا تملك هذا الطلب');
    ...
    const patientShare = Math.max(0, Number(patientCopay) || 0);
    const companyShare = Math.max(0, Number(companyCopay) || 0);
    const price = patientShare + companyShare;
    ...
    if (!rejected && price <= 0) throw new BadRequestException('مبلغ الخدمة مطلوب');
    ...
    const policy = profile?.insurance?.company_id
      ? profile.insurance
      : { company_id: 'unknown', company_name: 'تأمين', policy_number: approvalCode || null };
    ...
    const patch: any = {
      ...
      service_type: preq.type || 'consultation', price: price || patientShare, policy,
...
export class ProviderWalletController {
  ...
  async requestWithdrawal(@CurrentUser() u: any, @Body() body: { amount?: number; iban?: string }) {
    const bal = await this.ledger.providerBalance(u.id);
    ...
    if (bal.negative) {
      throw new BadRequestException(`negative_balance: رصيدك ${balance} ر.س بسبب استردادات/تسويات — الأرباح الجديدة تسوّي الدين أولاً`);
    }
    if (amount < minimum) {
      throw new BadRequestException(`الحد الأدنى للسحب ${minimum} ر.س (المتاح: ${balance}، قيد التسوية: ${bal.pending})`);
    }
    if (amount > balance) {
      throw new BadRequestException(`الرصيد المتاح غير كافٍ (${balance} ر.س؛ ${bal.pending} ر.س ما زال قيد التسوية)`);
    }

    const pending = await this.withdrawals.findOne({ provider_id: u.id, status: 'pending' });
    if (pending) throw new BadRequestException('يوجد لديك طلب سحب قيد المراجعة بالفعل');

    const profile: any = await this.conn.collection('provider_profiles').findOne(
      { $or: [{ user_id: u.id }, { account_id: u.id }] } as any,
    );
    let ibanRaw = body?.iban || profile?.iban;
    let bankHolder = profile?.bank_account_name || null;
    if (!ibanRaw) {
      const bank: any = await this.conn.collection('provider_bank_accounts').findOne({ account_id: u.id });
      ibanRaw = bank?.iban;
      bankHolder = bankHolder || bank?.holder_name || null;
    }
    if (!ibanRaw) throw new BadRequestException('iban_required — أضف الآيبان في ملفك أو أرسله مع الطلب');
    const iban = String(ibanRaw).replace(/\s+/g, '').toUpperCase();
    if (!/^SA\d{22}$/.test(iban)) throw new BadRequestException('invalid_iban — الآيبان السعودي يجب أن يكون SA متبوعًا بـ 22 رقمًا');

    const doc = {
      id: `wd_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
      provider_id: u.id,
      provider_type: u.role,
      amount,
      iban,
