...
export class ProviderOpsService {
  ...
  // ═══ SECURITY (F-C12): ops actions must be performed by the owning provider account.
  // Admins bypass. Prevents any JWT holder from QC-ing labs, signing nursing visits,
  // ending consultations or completing ambulance missions they were never assigned —
  // several of these paths also credit earnings to the caller.
  private assertOpsOwnership(user: any, booking: any | null | undefined, ownerFields: string[], kind: string) {
    if (!booking) throw new NotFoundException(`${kind} not found`);
    const role = String(user?.role || '').toLowerCase();
    if (role === 'admin' || role === 'super_admin') return;
    const owners = ownerFields.map((f) => booking[f]).filter(Boolean).map(String);
    if (!user?.id || !owners.includes(String(user.id))) {
      throw new ForbiddenException('not_assigned_provider');
    }
  }
  ...
  async labQc(user: any, bookingId: string, action: string, body: any = {}) {
    ...
    this.assertOpsOwnership(user, b, ['lab_id', 'provider_account_id', 'pharmacy_id'], 'lab booking');
  ...
  async nursingSign(user: any, bookingId: string, signatureBase64: string, signerName: string) {
    ...
    const owned: any = await this.conn.collection('homecarebookings').findOne({ id: bookingId } as any);
    this.assertOpsOwnership(user, owned, ['nurse_id', 'provider_id', 'assigned_nurse_id'], 'nursing booking');
    if (owned && !['CONFIRMED', 'CARE_IN_PROGRESS', 'IN_PROGRESS', 'ARRIVED', 'IN_TRANSIT'].includes(String(owned.state))) {
      throw new BadRequestException('booking_not_in_signable_state');
    }
    await this.conn.collection('homecarebookings').updateOne(
      { id: bookingId } as any,
      {
  ...
  async ambulanceEta(bookingId: string, fromLat: number, fromLng: number) {
    const b: any = await this.conn.collection('emergency_requests').findOne({ id: bookingId } as any);
    if (!b) throw new NotFoundException('emergency not found');
  ...
  async ambulanceHandover(user: any, bookingId: string, body: { hospital_name: string; notes?: string }) {
    if (!body?.hospital_name) throw new BadRequestException('hospital_name required');
    const em0: any = await this.conn.collection('emergency_requests').findOne({ id: bookingId } as any);
    this.assertOpsOwnership(user, em0, ['assigned_ambulance_id', 'ambulance_id', 'driver_id'], 'emergency request');
    await this.conn.collection('emergency_requests').updateOne(
      { id: bookingId } as any,
      { $set: { handover: { hospital_name: body.hospital_name, notes: body.notes || null, by: user.id, at: new Date() }, state: 'HANDED_OVER', updatedAt: new Date() } },
    );
  ...
  async ambulanceComplete(user: any, bookingId: string, body: { summary: string; outcome: string; vitals?: any }) {
    if (!body?.summary || !body?.outcome) throw new BadRequestException('summary and outcome required');
    const em0b: any = await this.conn.collection('emergency_requests').findOne({ id: bookingId } as any);
    this.assertOpsOwnership(user, em0b, ['assigned_ambulance_id', 'ambulance_id', 'driver_id'], 'emergency request');
    await this.conn.collection('emergency_requests').updateOne(
      { id: bookingId } as any,
      { $set: { completion_report: { ...body, by: user.id, at: new Date() }, state: 'COMPLETED', updatedAt: new Date() } },
    );
    // 💰 Credit the ambulance provider (idempotent per emergency)
    const em: any = await this.conn.collection('emergency_requests').findOne({ id: bookingId } as any);
    const fare = Number(em?.fare ?? em?.amount ?? (body as any)?.amount ?? 0);
    await this.creditEarning(em?.assigned_ambulance_id || user.id, 'ambulance', fare, 'emergency', bookingId);
  ...
  async endConsultation(user: any, body: any) {
    ...
    // SECURITY (F-C12): only the assigned doctor (or admin) may end a consultation —
    // this path also credits doctor earnings.
    const apptPre: any = await this.conn.collection('doctor_appointments').findOne({ id: appointmentId });
    this.assertOpsOwnership(user, apptPre, ['doctor_id', 'provider_account_id'], 'appointment');
    const now = new Date();
...
export class ProviderOpsController {
  ...
  // Ambulance
  @Get('ambulance/:id/eta') eta(@Param('id') id: string, @Query('lat') lat: string, @Query('lng') lng: string) { return this.svc.ambulanceEta(id, parseFloat(lat), parseFloat(lng)); }
