...
const STATUS_MAP: Record<string, UnifiedStatus> = {
  ...
  REPORTED: 'completed',
  // Radiology Booking states
  PENDING: 'pending',
  SCHEDULED: 'pending',
  IN_PROGRESS: 'in_progress',
  COMPLETED: 'completed',
  REPORT_PUBLISHED: 'completed',
  // Home Care booking states
...
function unify(raw?: string): UnifiedStatus {
...
export class TimelineService {
  constructor(
    @Inject('OrderRepository') private orderM: OrderRepository,
    @Inject('PrescriptionRepository') private rxM: PrescriptionRepository,
    @Inject('LabBookingRepository') private labBM: LabBookingRepository,
    @Inject('LabResultRepository') private labRM: LabResultRepository,
    @Inject('HomeCareBookingRepository') private hcM: HomeCareBookingRepository,
    @Inject('AppointmentRepository') private apptM: AppointmentRepository,
    @Inject('VitalReadingRepository') private vitalM: VitalReadingRepository,
    @Inject('MedicationReminderRepository') private remM: MedicationReminderRepository,
    @Inject('CustomServiceRequestRepository') private customM: CustomServiceRequestRepository,
    @Inject('RadiologyBookingRepository') private radBM: RadiologyBookingRepository,
    @Inject('MedicalReportRepository') private mrM: MedicalReportRepository,
  ) {}

  async build(
    user: any,
    opts: { kinds?: string[]; limit: number; since?: Date; until?: Date },
  ): Promise<{ events: TimelineEvent[]; total: number }> {
    const patient_id = user?.id;
    if (!patient_id) return { events: [], total: 0 };
    const wants = (k: string) => !opts.kinds || opts.kinds.includes(k);
    const dateMatch: any = {};
    if (opts.since) dateMatch.$gte = opts.since;
    if (opts.until) dateMatch.$lte = opts.until;
    ...
    if (wants('order')) {
      ...
      for (const o of docs as any[]) {
        ...
        evs.push({
          ...
          title: (o.items || []).slice(0, 2).map((x: any) => x.name_ar).join(' • ') || 'طلب صيدلية',
    ...
    if (wants('lab_result')) {
      // Only labs (source != radiology). Radiology reports live under 'radiology_report'.
      const docs = await this.labRM.find({ patient_id, source: { $ne: 'radiology' } }).sort({ reported_at: -1, createdAt: -1 }).limit(opts.limit).lean();
      for (const r of docs as any[]) {
        const us: UnifiedStatus = r.critical ? 'critical' : 'completed';
        evs.push({
          kind: 'lab_result',
          id: String(r.id || r._id),
    ...
    if (wants('medical_report')) {
      ...
      for (const m of docs as any[]) {
        const us: UnifiedStatus = m.critical ? 'critical' : 'completed';
        evs.push({
          kind: 'medical_report',
    ...
    if (wants('vital')) {
      // Only abnormal/critical vitals appear in timeline (others are in their own section)
      ...
      for (const v of docs as any[]) {
        const us: UnifiedStatus = v.flag === 'critical' ? 'critical' : 'active';
        evs.push({
          kind: 'vital',
    ...
    if (wants('reminder')) {
      // Last 30 reminder log entries
      const docs = await this.remM.find({ patient_id }).lean();
      for (const r of docs as any[]) {
        for (const entry of (r.log || []).slice(-10)) {
          const us: UnifiedStatus = entry.status === 'taken' ? 'completed' : entry.status === 'skipped' ? 'cancelled' : 'pending';
          evs.push({
            kind: 'reminder',
  ...
  async summary(user: any): Promise<Record<string, number>> {
    const patient_id = user?.id;
