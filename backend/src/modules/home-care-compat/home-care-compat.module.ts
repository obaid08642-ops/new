...
export class HomeCareCompatController {
  ...
  @Post('bookings') async createBooking(@CurrentUser() u: any, @Body() body: any) {
    ...
    if (!svc) throw new NotFoundException('service not found');
    // Optional preferred nurse: must be an APPROVED nursing provider profile
    let providerId: string | undefined;
    if (body?.provider_id) {
      const p: any = await this.profiles.findOne({ id: String(body.provider_id), provider_type: { $in: ['nursing', 'nurse'] }, approval_status: 'approved' }).lean();
      if (!p) throw new BadRequestException('provider_not_available');
      providerId = p.id;
    }
    // Family on-behalf booking: shared group + owner or 'booking' permission
    let bookedForMemberId: string | undefined;
    if (body?.member_id && String(body.member_id) !== String(u.id)) {
      const group: any = await (this.bookings as any).db.collection('family_groups').findOne({
        is_deleted: { $ne: true },
        'members.user_id': { $all: [String(u.id), String(body.member_id)] },
      });
      if (!group) throw new ForbiddenException('patient is not a member of your family group');
      if (group.owner_id !== String(u.id)) {
        const me = (group.members || []).find((m: any) => m.user_id === String(u.id));
    ...
    const isInsurance = String(body.payment_method || '') === 'insurance';
    const doc = await this.bookings.create({
      ...
      payment_method: isInsurance ? 'insurance' : 'card',
      insurance_status: isInsurance ? 'pending' : 'none',
      booked_for_member_id: bookedForMemberId,
      provider_id: providerId,
      // PH-SERVICE: insurance bookings park at PENDING_INSURANCE until the
      // provider decides (mirror flips to WAITING_COPAY); cash/card stays NEW_REQUEST.
      state: isInsurance ? 'PENDING_INSURANCE' : 'NEW_REQUEST',
      state_history: [{ state: isInsurance ? 'PENDING_INSURANCE' : 'NEW_REQUEST', at: new Date(), by: u.id }],
    });
