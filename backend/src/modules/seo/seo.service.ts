...
export class SeoService {
  ...
  private composeDescription(type: string, e: any): string {
    switch (type) {
      ...
      case 'article': {
        return e.seo_description_ar || e.excerpt_ar || e.seo_description_en || e.excerpt_en || 'مقال صحي موثوق على منصة نبض';
      }
      case 'lab-service': {
        const parts = [
  ...
  async sitemap(): Promise<string> {
    ...
    await pushEntities('doctor', this.providerM, { type: 'doctor', active: { $ne: false }, is_deleted: { $ne: true } }, 0.85, 'weekly');
    ...
    await pushEntities('article', this.articleM, { status: 'PUBLISHED', is_deleted: { $ne: true } }, 0.6, 'weekly');
  ...
  async llmsTxt(): Promise<string> {
    ...
    try {
      const doctors = await this.providerM
        .find({ type: 'doctor', active: { $ne: false }, is_deleted: { $ne: true } }, { id: 1, name_ar: 1, name_en: 1, full_name: 1, specialty: 1, city: 1, rating_avg: 1 })
        .sort({ rating_avg: -1 })
        .limit(30)
        .lean();
      if (doctors.length) {
    ...
    try {
      const meds = await this.medM
        .find({ active: { $ne: false }, is_deleted: { $ne: true } }, { id: 1, name_ar: 1, name_en: 1, category: 1 })
        .limit(30)
        .lean();
      if (meds.length) {
    ...
    try {
      const labSvcs = await this.labSvcM
        .find({ active: { $ne: false }, is_deleted: { $ne: true } }, { id: 1, name_ar: 1, name_en: 1 })
        .limit(15)
        .lean();
      if (labSvcs.length) {
    ...
    try {
      const articles = await this.articleM
        .find({ status: 'PUBLISHED', is_deleted: { $ne: true } }, { id: 1, title_ar: 1, title_en: 1, category: 1 })
        .limit(20)
        .lean();
      if (articles.length) {
