...
export class StorageService {
  ...
  async upload(input: { owner_account_id: string; owner_kind?: string; mime: string; data_base64: string; original_name?: string; visibility?: 'private' | 'public_read'; customKey?: string; target?: 'r2' | 'cloudinary' }) {
    ...
    try {
      ...
      // Graceful degradation: if S3/R2 credentials are invalid/expired, keep the
      // platform functional via inline storage instead of hard-failing uploads.
      if (this.adapter instanceof S3R2Adapter) {
        this.logger.error(`S3/R2 put failed (${e.message}) — falling back to inline base64 storage`);
        adapterRes = await new Base64Adapter().put({ mime: input.mime, data_base64: input.data_base64, original_name: input.original_name || 'file', customKey: input.customKey });
      } else {
        throw e;
      }
    }
  ...
  async signedUrl(id: string, requester: { id: string; role?: string }) {
    ...
    if (o.backend === 'cloudinary' && o.external_key) {
      ...
      return { url: o.external_url, expires_in: null, kind: 'cloudinary_public_fallback' };
    }
    ...
    if (o.visibility !== 'public_read' && o.external_key && S3R2Adapter.configured()) {
      try {
        ...
        this.logger.warn(`R2 presign failed: ${e.message} — API fallback`);
      }
    }

    // Public object → CDN URL directly
  ...
  async uploadCloudinary(input: { owner_account_id: string; owner_kind?: string; mime: string; data_base64: string; original_name?: string; visibility?: 'private' | 'public_read'; customKey?: string }) {
    ...
    const result = await cloudinary.uploader.upload(`data:${input.mime};base64,${input.data_base64}`, {
      public_id: publicId,
      resource_type: 'image',
      type: input.visibility === 'public_read' ? 'upload' : 'authenticated',
    ...
    return { id: obj.id, mime: obj.mime, size_bytes: obj.size_bytes, url: meta.secureUrl, thumbnail_url: meta.thumbnailUrl, meta };
  }
}
...
export class StorageController {
  ...
  async upload(@Body() body: any, @CurrentUser() user: any) {
    ...
    const customKey = target === 'cloudinary' && typeof body.customKey === 'string'
      ...
      : undefined;
    return this.svc.upload({ owner_account_id: user.id, owner_kind: user.role, mime: body.mime, data_base64: body.data_base64, original_name: body.original_name, visibility: body.visibility, customKey, target });
  }
  @Get(':id')
  ...
  async uploadCloudinary(@Body() body: any, @CurrentUser() user: any) {
    return this.svc.uploadCloudinary({ owner_account_id: user.id, owner_kind: body.owner_kind || user.role, mime: body.mime, data_base64: body.data_base64, original_name: body.original_name, visibility: body.visibility });
  }
}
