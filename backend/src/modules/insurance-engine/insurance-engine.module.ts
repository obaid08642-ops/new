...
export class InsuranceFlowService {
  ...
  async createRequest(user: any, body: any) {
    ...
    if (!body?.price || Number(body.price) <= 0) throw new BadRequestException('price must be positive');
    ...
    const doc = await this.requests.create({
      ...
      booking_id: body.booking_id, booking_kind: body.booking_kind,
      service_type: body.service_type, channel: body.channel,
      price: Number(body.price), policy,
  ...
  async payCopay(user: any, id: string, body: any) {
    ...
    if (req.state === 'APPROVED_FULL') {
      ...
      if (!body?.payment_id) throw new BadRequestException('payment_id is required (Moyasar payment for the copay)');
      req.payment_id = body.payment_id;
      this.push(req, 'COPAY_PAID', user.id, `payment ${body.payment_id}`);
