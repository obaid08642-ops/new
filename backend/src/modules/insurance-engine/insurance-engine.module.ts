...
export class InsuranceFlowService {
  ...
  async decide(user: any, id: string, body: any) {
    ...
    await req.save();
    // PH-SERVICE: mirror the coverage decision onto the linked booking so the
    // patient-facing status reflects APPROVED/PARTIAL/REJECTED + their co-pay.
    if (req.booking_kind === 'consultation' && req.booking_id) {
      const stMap: any = { APPROVED_FULL: 'APPROVED', APPROVED_PARTIAL: 'PARTIAL_APPROVAL', REJECTED: 'REJECTED' };
      const insStatus = stMap[req.state];
      if (insStatus) {
        await this.appointments.updateOne(
          { id: req.booking_id },
          { $set: { insurance_status: insStatus, insurance_copay: Number(req.copay_amount || 0), updatedAt: new Date() }, $push: { state_history: { state: `insurance_${req.state}`, at: new Date(), by_user_id: user.id, by_role: user.role, note: `insreq:${req.id}` } } as any },
        ).catch(() => null);
      }
    }
    // PH-SERVICE (E): labs/radiology/nursing — approval parks the booking at
    // WAITING_COPAY; rejection leaves it for the patient's cash-or-cancel choice.
    if (['lab', 'radiology', 'nursing'].includes(String(req.booking_kind)) && req.booking_id) {
      const model = req.booking_kind === 'lab' ? this.labs : req.booking_kind === 'radiology' ? this.radiology : this.homeCare;
      if (req.state === 'APPROVED_FULL' || req.state === 'APPROVED_PARTIAL') {
        await model.updateOne(
          { id: req.booking_id },
          { $set: { state: 'WAITING_COPAY', insurance_status: 'approved', insurance_copay_amount: Number(req.copay_amount || 0), updatedAt: new Date() }, $push: { state_history: { from: 'PENDING_INSURANCE', to: 'WAITING_COPAY', at: new Date(), by: user.id, note: `insreq:${req.id}` } } as any },
        ).catch(() => null);
      } else if (req.state === 'REJECTED') {
        await model.updateOne(
          { id: req.booking_id },
          { $set: { insurance_status: 'rejected', updatedAt: new Date() }, $push: { state_history: { from: 'PENDING_INSURANCE', to: 'PENDING_INSURANCE', at: new Date(), by: user.id, note: `rejected:${String(req.rejection_reason || '').slice(0, 120)}` } } as any },
        ).catch(() => null);
      }
    }
    this.events.emit('insurance.decided', { request_id: req.id, patient_id: req.patient_id, state: req.state, copay_amount: req.copay_amount, booking_kind: req.booking_kind, booking_id: req.booking_id });
    return req.toObject();
  }
  ...
  async payCopay(user: any, id: string, body: any) {
    ...
    await req.save();
    // PH-SERVICE: co-pay settled ⇒ the consultation booking is CONFIRMED.
    if (req.booking_kind === 'consultation' && req.booking_id) {
      await this.appointments.updateOne(
        { id: req.booking_id },
        { $set: { status: 'CONFIRMED', payment_status: 'paid', insurance_status: 'APPROVED', updatedAt: new Date() }, $push: { state_history: { state: 'CONFIRMED', at: new Date(), by_user_id: user.id, by_role: user.role, note: `copay_settled:${req.id}` } } as any },
      ).catch(() => null);
    }
    // E: same settlement confirms labs/radiology/nursing bookings.
    if (['lab', 'radiology', 'nursing'].includes(String(req.booking_kind)) && req.booking_id) {
      const model2 = req.booking_kind === 'lab' ? this.labs : req.booking_kind === 'radiology' ? this.radiology : this.homeCare;
      await model2.updateOne(
        { id: req.booking_id },
        { $set: { state: 'CONFIRMED', payment_status: 'paid', insurance_status: 'approved', updatedAt: new Date() }, $push: { state_history: { from: 'WAITING_COPAY', to: 'CONFIRMED', at: new Date(), by: user.id, note: `copay_settled:${req.id}` } } as any },
      ).catch(() => null);
    }
    this.events.emit('insurance.copay.paid', { request_id: req.id, provider_id: req.provider_id, patient_id: req.patient_id, booking_kind: req.booking_kind, booking_id: req.booking_id });
    return req.toObject();
  }
...
export class RefundService {
  ...
  async request(user: any, body: any) {
    ...
    if (!body?.reason || typeof body.reason !== 'string' || body.reason.trim().length < 3) throw new BadRequestException('reason is required');
    const dup = await this.refunds.findOne({ booking_id: body.booking_id, state: { $ne: 'REJECTED' } });
    if (dup) return dup.toObject();
    // ═══ SECURITY (F-C6): refund window AND amount come from stored records, never
    // from the client body. Previously a client could send a future scheduled_at for
    // a 100% window, or inflate amount_paid. Ownership is enforced against every
    // booking family; unknown kinds are rejected.
    let scheduledAt: Date | undefined;
    let paid = NaN;
    let matchedKind = String(body?.booking_kind || 'booking');
    const kinds: Array<[string, string]> = [
      ['appointments', 'appointment'], ['labbookings', 'lab'], ['radiologybookings', 'radiology'],
      ['homecarebookings', 'nursing'], ['orders', 'pharmacy'],
    ];
    for (const [coll, kind] of kinds) {
      const rec: any = await this.conn.collection(coll).findOne({ id: body.booking_id });
      if (!rec) continue;
      const owner = String(rec.patient_id || rec.user_id || '');
      if (!owner || owner !== String(user.id)) throw new ForbiddenException('not_booking_owner');
      paid = Number(rec.total ?? rec.amount ?? rec.price ?? rec.fee ?? 0);
      const rawSlot = rec.slot_start ?? rec.scheduled_at ?? rec.reschedule?.new_slot_start ?? rec.visit_date;
      scheduledAt = rawSlot ? new Date(rawSlot) : undefined;
      matchedKind = kind;
      break;
    }
    if (!Number.isFinite(paid) || paid <= 0) throw new BadRequestException('amount_paid must be positive');
    ...
    const policy = this.policyFor(scheduledAt);
    const doc = await this.refunds.create({
      patient_id: user.id, booking_id: body.booking_id, booking_kind: matchedKind,
...
export class FinanceCoreController {
  ...
  // SECURITY (F-C4): commission accrual is an internal settlement hook.
  // Exposing it to any JWT holder lets anyone forge provider earnings that feed payouts.
  // Admin-only + explicit actor attribution for the audit trail.
  @Post('ledger/accrue')
  @Roles(UserRole.ADMIN, UserRole.SUPER_ADMIN)
  accrue(@CurrentUser() u: any, @Body() b: any) {
    return this.finance.accrue({ ...b, accrued_by: u?.id, source: 'admin_api' });
  }
