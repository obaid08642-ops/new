...
describe('RefundService', () => {
  ...
  beforeEach(() => {
    ...
    service = new RefundService(refunds, events, fraud as any, {
        // F-C6: ownership/amount/schedule resolve from the stored booking record.
        collection: jest.fn().mockImplementation((coll: string) => ({
          findOne: jest.fn().mockImplementation(async (query: any) => {
            if (coll !== 'appointments' || query?.id !== 'bk-1') return null;
            return { id: 'bk-1', patient_id: 'pat-1', total: 300, slot_start: hoursFromNow(10).toISOString() };
          }),
        })),
      } as any);
    jest.clearAllMocks();
