...
export class RealtimeGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  joinChannel(@ConnectedSocket() client: Socket, @MessageBody() data: { channel: string }) {
    // Arbitrary channel names made every Socket.IO room subscribable by any
    // authenticated user. Supported rooms must have their own purpose-specific
    // handler and server-side authorization (thread, appointment, or user room).
  ...
  async handleWaitingRoomJoin(
    ...
    try {
      ...
      if (!this.isWaitingRoomParticipant(appt, user)) return { ok: false, error: 'not_participant' };
      if (!this.isWaitingRoomOpen(appt.status)) return { ok: false, error: 'appointment_not_open' };
  ...
  private isWaitingRoomParticipant(appt: any, user: any): boolean {
    return [appt.patient_id, appt.booked_by_user_id, appt.doctor_user_id]
      .filter(Boolean)
      .includes(user.id);
  }

  private isWaitingRoomOpen(status: unknown): boolean {
