...
export class HospitalStaffService {
  ...
  /** Ownership guard: the staff record may be linked via either legacy field name. */
  private assertOwnsStaff(acc: any, staff: UserDocument, user: any) {
    const ownerId = acc?.id;
    if (!ownerId || user.role === UserRole.ADMIN) return;
    const linked = staff.get('parent_account_id') ?? staff.get('parent_provider_account_id');
    if (linked !== ownerId) throw new ForbiddenException('not_owner');
  }

  /** Cryptographically-random temporary password generated SERVER-SIDE and shown once. */
  ...
  async list(user: any) {
    ...
    const filter: any = acc ? { $or: [{ parent_provider_account_id: acc.id }, { parent_account_id: acc.id }] } : {};
  ...
  async create(user: any, body: { full_name: string; phone: string; email?: string; password?: string; staff_role: string; department?: string; permissions?: string[]; schedule?: any; specialty?: string; degree?: string; years_experience?: number; license_number?: string; consultation_fee?: number }) {
    ...
    // Password is generated SERVER-SIDE when not supplied (never client-side random),
    // and returned exactly once so the owner can hand it to the staff member.
    const tempPassword = body.password || this.generateTempPassword();
    if (!body.password && tempPassword.length < 12) throw new BadRequestException('weak_password');
    ...
    const u = await this.users.create({
      ...
      role,
      verified: true,
      // F-C14 canonical field per user.schema.ts — 'parent_account_id' is
      // stripped by Mongoose strict mode, which re-broke owner access.
    ...
    if (!body.password) (o as any).temp_password = tempPassword;
    return o;
  }
  ...
  async update(user: any, staffId: string, body: { full_name?: string; phone?: string; email?: string; department?: string; permissions?: string[]; schedule?: any; specialty?: string; degree?: string; years_experience?: number; license_number?: string; consultation_fee?: number }) {
    ...
    this.assertOwnsStaff(acc, staff, user);
    const patch: any = {};
  ...
  async resetPassword(user: any, staffId: string, newPassword: string) {
    ...
    if (!staff) throw new NotFoundException('staff_not_found');
    this.assertOwnsStaff(acc, staff, user);
    // Server-generated reset credential when the caller does not supply one.
    const pw = newPassword || this.generateTempPassword();
    if (pw.length < 6) throw new BadRequestException('weak_password');
    const hash = await bcrypt.hash(pw, 10);
    await this.users.updateOne({ id: staffId }, { $set: { password_hash: hash } });
    return { ok: true, ...(newPassword ? {} : { temp_password: pw }) };
  }
}
