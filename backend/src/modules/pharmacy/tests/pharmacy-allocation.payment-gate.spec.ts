...
describe('PharmacyAllocationService fulfillment gate', () => {
  function setup(orderPatch: any = {}, paymentEvidence: any = null) {
    ...
    const paymentCollection = { findOne: jest.fn().mockResolvedValue(paymentEvidence) };
    ...
    const orders: any = { findOne: jest.fn().mockReturnValueOnce({ lean: jest.fn().mockResolvedValue(order) }).mockResolvedValue(orderDocument), db: { collection: jest.fn((name: string) => { expect(name).toBe('pharmacy_payment_evidence'); return paymentCollection; }) } };
    const notifications: any = { notifyPatientAllocationConfirmed: jest.fn().mockResolvedValue(undefined) };
    const service = new PharmacyAllocationService(allocs, orders, {} as any, {} as any, notifications, {} as any, {} as any);
  ...
  it('fails before delivery transition until a COD collection-proof and settlement/reconciliation command is implemented', async () => {
    const { service } = setup({ payment_method: 'cod' });
    await expect(service.delivered({ id: 'pharmacy-1', role: 'pharmacy' }, 'alloc-1')).rejects.toBeInstanceOf(ServiceUnavailableException);
  });
});
