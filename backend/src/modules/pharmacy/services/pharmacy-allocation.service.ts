...
export class PharmacyAllocationService {
  ...
  async delivered(user: any, id: string) {
    ...
    // 💵 COD settlement (P4): collecting the cash on delivery flips the parent
    // order's payment_status from cod_pending_collection → cod_collected.
    if (doc) {
      try {
        const order = await this.orders.findOne({ id: doc.order_id });
        if (order && (order as any).payment_method === 'cod' && (order as any).payment_status === 'cod_pending_collection') {
          await this.orders.updateOne(
            { id: doc.order_id },
            { $set: { payment_status: 'cod_collected', cod_collected_at: new Date(), cod_collected_by: user.id } } as any,
          );
        }
      } catch { /* COD mirror must not block delivery confirmation */ }
    }
    ...
    if (doc) {
      try {
        ...
        if (!dup) {
          // Gross = allocation basket total; fall back to the patient-selected
          // offer estimate when the pharmacy never edited the basket (offer flow).
          let gross = Number(doc.totals?.total ?? 0);
          if (!gross) {
            const parent: any = await this.orders.findOne({ id: doc.order_id });
            gross = Number(parent?.selected_offer?.subtotal_estimate || 0)
              + Number(parent?.selected_offer?.delivery_fee || 0);
          }
          if (gross > 0) {
