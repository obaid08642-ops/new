import { Body, Controller, Get, Param, Post, Patch, UseGuards, Query, ForbiddenException, ServiceUnavailableException, BadRequestException } from '@nestjs/common';
...
export class PatientPharmacyController {
  constructor(private orders: PharmacyOrderService, private broadcast: PharmacyBroadcastService) {}
  ...
  // ═══ PH-PHARMACY governing rules: compare offers → select one pharmacy offer ═══
  @Get('orders/:id/offers')
  offers(@CurrentUser() u: any, @Param('id') id: string) { return this.broadcast.listOffersForPatient(u, id); }
  @Post('orders/:id/select-offer')
  selectOffer(@CurrentUser() u: any, @Param('id') id: string, @Body() b: any) {
    const pid = String(b?.pharmacy_account_id || '');
    if (!pid) throw new BadRequestException('pharmacy_account_id_required');
    return this.broadcast.patientSelectOffer(u, id, pid);
  }
  // PH-PHARMACY cash branch: deferred collection commitment (delivery/pickup)
  @Post('orders/:id/cod')
  registerCod(@CurrentUser() u: any, @Param('id') id: string) { return this.orders.registerCod(u, id); }
  // PH-PHARMACY (D-003): pay the selected offer from wallet balance (atomic debit)
  @Post('orders/:id/pay-wallet')
  payWallet(@CurrentUser() u: any, @Param('id') id: string) { return this.orders.payWithWallet(u, id); }
