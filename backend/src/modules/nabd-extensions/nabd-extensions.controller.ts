...
export class NabdExtensionsController {
  ...
  // SECURITY: manual wallet mint/debit is admin-only. Patients/providers must never
  // be able to credit their own balance or bypass insufficient-funds checks.
  @Post('wallet/credit')
  @Roles(UserRole.ADMIN, UserRole.SUPER_ADMIN)
  async creditWallet(@CurrentUser() user: any, @Body() body: any) {
    const amount = Number(body.amount);
    if (!Number.isFinite(amount) || amount <= 0) throw new BadRequestException('amount_must_be_positive');
    return this.svc.processWalletTransaction({
      ownerId: String(body.ownerId || ''),
      ownerType: body.ownerType === 'provider' ? 'provider' : 'patient',
      amount,
      type: 'credit',
      referenceType: body.referenceType || 'admin_adjustment',
      referenceId: body.referenceId || `admin:${user.id}`,
      description: body.description || 'Admin Wallet Credit',
    });
  }
