...
export class HospitalService {
  ...
  private assertFacilityActor(actor?: any, write = false): void {
    if (!actor) return;
    const role = String(actor.role || actor.provider_type || actor.providerType || '').toLowerCase();
    const allowed = ['hospital', 'hospital_admin', 'branch_admin', 'receptionist', 'finance', 'admin', 'super_admin'];
    if (!allowed.includes(role)) throw new ForbiddenException('hospital facility role required');
    if (write && ['receptionist', 'finance'].includes(role)) throw new ForbiddenException('facility write permission required');
  }
