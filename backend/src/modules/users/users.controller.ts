...
export class UsersController {
  ...
  // ═══ GDPR/PDPL (F-M1): real data-subject export. Bounded bundle, owner-only. ═══
  @Get('me/export')
  async exportMyData(@CurrentUser('id') id: string) {
    const db = this.users['connection']?.db;
    const collections = ['users', 'patientprofiles', 'orders', 'appointments', 'labbookings', 'radiologybookings', 'homecarebookings', 'transactions'];
    const bundle: Record<string, any[]> = {};
    for (const c of collections) {
      try {
        bundle[c] = await db.collection(c).find({
          $or: [{ patient_id: id }, { user_id: id }, { id }, { patient_account_id: id }],
        }).limit(200).project({ _id: 0 }).toArray();
      } catch { bundle[c] = []; }
    }
    return { generated_at: new Date().toISOString(), format: 'nabd-export-v1', data: bundle };
  }
