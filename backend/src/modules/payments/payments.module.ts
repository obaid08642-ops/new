...
import { PharmacyOrder, PharmacyOrderSchema } from '../pharmacy/schemas/pharmacy.schema';
...
function normalizeKind(k: string) {
  const m: any = { orders: 'pharmacy', pharmacy: 'pharmacy', 'pharmacy-order': 'pharmacy-order', 'pharmacy_order': 'pharmacy-order', lab: 'lab', labs: 'lab', radiology: 'radiology', rads: 'radiology', nursing: 'nursing', 'home-care': 'nursing', homecare: 'nursing', consultation: 'consultation', appt: 'consultation', insurance: 'insurance', 'insurance-copay': 'insurance', copay: 'insurance' };
  return m[k];
}
...
export class PaymentsService {
  ...
  private modelFor(k: string): Model<any> {
    ...
    if (kind === 'pharmacy-order') return this.pharmacyOrders;
  ...
  async createPaymentIntent(user: any, type: string, id: string, idempotencyKey: string) {
    ...
    let amount: number;
    if (kind === 'pharmacy-order') {
      // PH-PHARMACY: the price is the SELECTED OFFER snapshot — never recomputed client-side.
      const so = booking.selected_offer;
      if (!so) throw new BadRequestException('no_selected_offer');
      amount = Math.round((Number(so.subtotal_estimate || 0) + Number(so.delivery_fee || 0)) * 100) / 100;
    } else amount = kind === 'insurance' ? (booking.copay_amount || 0) : (booking.total || booking.totals?.total || booking.price || 0);
  ...
  async verifyPayment(user: any, transactionId: string) {
    ...
    if (result.status === 'paid') {
      ...
      // PH-PHARMACY: paid selected offer ⇒ CONFIRMED (PAYMENT_PENDING → CONFIRMED)
      if (t.booking_kind === 'pharmacy-order') {
        await this.pharmacyOrders.updateOne(
          { id: t.booking_id, status: 'offer_selected' },
          { $set: { status: 'confirmed', confirmed_at: new Date() }, $push: { timeline: { ts: new Date(), event: 'payment_confirmed', meta: { transaction_id: t.id } } } as any },
        );
      }
