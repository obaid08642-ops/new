import { Module, Injectable, Controller, Post, Get, Body, Param, Logger, BadRequestException, BadGatewayException, NotFoundException, UseGuards, Req, HttpCode } from '@nestjs/common';
...
export class PaymentsService {
  ...
  async verifyPayment(user: any, transactionId: string) {
    ...
    // E5-F2: verifying a transaction triggers gateway sync + booking confirmation —
    // restrict to the owner, staff, or the internal webhook path (role 'system').
    const canVerify = t.patient_id === user.id || ['admin', 'system', 'provider', 'pharmacy', 'doctor'].includes(user.role);
    if (!canVerify) throw new BadRequestException('not_authorized');
  ...
  async capturePayment(user: any, transactionId: string) {
    if (!['admin', 'provider', 'pharmacy', 'doctor'].includes(user.role)) throw new BadRequestException('not_authorized');
    const t = await this.txns.findOne({ id: transactionId });
    if (!t) throw new NotFoundException('txn_not_found');
  ...
  async listForBooking(user: any, type: string, id: string) {
    ...
    if (!booking) throw new NotFoundException('booking_not_found');
    const staffRoles = ['admin', 'provider', 'pharmacy', 'doctor', 'finance'];
