import { Controller, Get, Post, Patch, Delete, Body, Param, Query, UseGuards, BadRequestException, ForbiddenException, Optional } from '@nestjs/common';
...
export class AdminController {
  ...
  async listDisputes(@Query('status') status = 'open') {
    const db = this.userModel.db;
    const states = status === 'all' ? ['open', 'pending', 'in_review', 'resolved', 'closed'] : [status, 'pending', 'in_review'];
    const tickets = await db.collection('supporttickets')
      .find({ status: { $in: states } } as any)
      .sort({ updatedAt: -1 }).limit(100).toArray();
    const accountIds = [...new Set(tickets.map((t: any) => t.account_id).filter(Boolean))];
    const users = accountIds.length
      ? await db.collection('users').find({ id: { $in: accountIds } } as any, { projection: { id: 1, full_name: 1, role: 1 } }).toArray()
      : [];
    const nameOf = new Map(users.map((u: any) => [u.id, u.full_name]));
    return {
      data: tickets.map((t: any) => ({
        id: t.id || String(t._id),
        orderId: t.order_id || t.booking_id || t.id || String(t._id),
        patientName: nameOf.get(t.account_id) || null,
        providerName: t.provider_name || null,
        amount: t.amount || 0,
        reason: t.subject || t.last_message || t.category || 'شكوى',
        status: t.status === 'open' || t.status === 'pending' || t.status === 'in_review' ? 'PENDING' : 'RESOLVED',
