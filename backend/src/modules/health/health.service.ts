...
export class HealthService {
  ...
  // VITALS
  private normalizeVitalType(rawType: unknown): string {
    ...
    if (!VALID_TYPES.includes(type)) throw new BadRequestException('invalid vital type');
    return type;
  }
  ...
  /**
   * Complete refill flow: validate ownership → resolve default address → create
   * a real pharmacy order. Creating an order is not proof of dispensing, so
   * this path never invents a restocked pill count or next-refill date.
   */
  async refillNow(user: any, id: string) {
    const prep = await this.prepareRefill(user, id);
    const reminder: any = prep.reminder;
    if (!reminder.chronic) throw new BadRequestException('chronic reminder required for refill');
    if (!reminder.medicine_id) throw new BadRequestException('eligible medicine identity required for refill');
    if (reminder.refill_pending_order_id) {
      return { ok: true, existing: true, order_id: reminder.refill_pending_order_id, state: 'REFILL_PENDING_FULFILLMENT' };
    }
    const reservation = uuid();
    const reserved = await this.reminders.findOneAndUpdate(
      { id, patient_id: user.id, chronic: true, refill_pending_order_id: { $exists: false }, refill_creation_lock: { $exists: false } },
      { $set: { refill_creation_lock: reservation } },
      { new: true },
    );
    if (!reserved) {
      const current: any = await this.reminders.findOne({ id, patient_id: user.id });
      if (current?.refill_pending_order_id) {
        return { ok: true, existing: true, order_id: current.refill_pending_order_id, state: 'REFILL_PENDING_FULFILLMENT' };
      }
      throw new ConflictException('refill creation already in progress');
    }
    ...
    try {
      ...
      if (!order?.id) throw new BadRequestException('refill order could not be created');
      ...
      // Persist only the pending order identity. Dispensed quantity and follow-up
      // dates need a verified fulfilment integration, never a fixed 30-day guess.
      await this.reminders.findOneAndUpdate(
        { id, patient_id: user.id, refill_creation_lock: reservation },
        { $set: { refill_pending_order_id: order.id }, $unset: { refill_creation_lock: 1 } },
      );
      return { ok: true, order_id: order.id, state: order.state, fulfillment_pending: true };
    } catch (error) {
      await this.reminders.findOneAndUpdate(
        { id, patient_id: user.id, refill_creation_lock: reservation },
        { $unset: { refill_creation_lock: 1 } },
      ).catch(() => undefined);
      throw error;
    }
  }
  ...
  /** Monthly health report (parity #20 completion): real records for the
   * requested month (default current), reduced server-side. BP stores
   * "sys/dia" strings, so JS reduction avoids brittle $toDouble casts. */
  async monthlyReport(user: any, month?: string) {
    const match = /^\d{4}-\d{2}$/.test(String(month || '')) ? String(month) : new Date().toISOString().slice(0, 7);
    const start = new Date(`${match}-01T00:00:00.000Z`);
    const end = new Date(start.getTime());
    end.setUTCMonth(end.getUTCMonth() + 1);
    const window = { $gte: start, $lt: end };
    const pid = user.id;

    const summarize = (nums: number[]) => nums.length
      ? { count: nums.length, avg: Math.round((nums.reduce((a, b) => a + b, 0) / nums.length) * 100) / 100, min: Math.min(...nums), max: Math.max(...nums) }
      : null;

    // Vitals — expand bp into systolic readings.
    const vitalsByType: Record<string, any> = {};
    try {
      const rows = await this.conn.db.collection('vitalreadings')
        .find({ patient_id: pid, deleted_at: null, measured_at: window }, { projection: { type: 1, value: 1, value_secondary: 1 } })
        .limit(2000).toArray();
      ...
      for (const r of rows) {
        ...
        buckets[t] = buckets[t] || [];
        if (t === 'bp') {
          const sys = Number(String(r.value || '').split('/')[0]);
          if (Number.isFinite(sys) && sys > 0) buckets[t].push(sys);
        } else {
          const v = Number(r.value);
          if (Number.isFinite(v)) buckets[t].push(v);
        }
      }
      for (const [type, nums] of Object.entries(buckets)) vitalsByType[type] = summarize(nums);
    } catch { /* brand-new accounts may lack the collection */ }

    let sleep: any = null;
    try {
      const rows = await this.conn.db.collection('sleepreadings')
        .find({ patient_id: pid, measured_at: window }, { projection: { sleep_score: 1, duration_hours: 1 } })
        .limit(500).toArray();
      const scores = rows.map((r: any) => Number(r.sleep_score)).filter(Number.isFinite);
      const hours = rows.map((r: any) => Number(r.duration_hours)).filter(Number.isFinite);
      sleep = scores.length ? { count: scores.length, avg_score: summarize(scores)?.avg ?? 0, avg_hours: summarize(hours)?.avg ?? 0 } : null;
    } catch { /* ignore */ }

    const moodCounts: Record<string, number> = {};
    try {
      const rows = await this.conn.db.collection('mood_entries')
        .find({ patient_id: pid, logged_at: window }, { projection: { mood: 1 } })
        .limit(500).toArray();
      for (const r of rows) moodCounts[String(r.mood)] = (moodCounts[String(r.mood)] || 0) + 1;
    } catch { /* ignore */ }

    let water_ml = 0;
    try {
      const rows = await this.conn.db.collection('nutrition_water_logs')
        .find({ patient_id: pid, logged_at: window }, { projection: { amount_ml: 1 } })
        .limit(1000).toArray();
      water_ml = rows.reduce((sum: number, r: any) => sum + (Number(r.amount_ml) || 0), 0);
    } catch { /* ignore */ }

    return { month: match, generated_at: new Date().toISOString(), vitals: vitalsByType, sleep, mood: moodCounts, water_ml };
  }
