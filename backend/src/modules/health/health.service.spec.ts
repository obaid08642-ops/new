...
describe('HealthService medication reminder contract', () => {
  ...
  it('fails closed before creating a legacy refill order or reserving reminder state', async () => {
    const reservationUpdate = jest.fn();
    const { service, reminders, orders } = makeService({ findOneAndUpdate: reservationUpdate });
    await expect(service.refillNow(user, 'reminder-1')).rejects.toMatchObject({ response: { message: 'canonical_pharmacy_flow_required' } });
    expect(orders.create).not.toHaveBeenCalled();
    expect(reservationUpdate).not.toHaveBeenCalled();
    expect(reminders.db.collection).not.toHaveBeenCalled();
  });
});
