...
export class AppointmentsService {
  ...
  async create(user: any, body: {
    ...
    try {
      const appt = await this.apptModel.create({
        ...
        booked_for_member_id: bookedBy ? body.for_member_id : undefined,
        doctor_id: doctor.id,
      ...
      // PH-SERVICE (governing rules):
      //  - cash  → instant CONFIRMED
      //  - card  → PENDING until payment.completed webhook
      //  - insurance → PENDING (REQUEST_SUBMITTED); CONFIRMED only after the
      //    provider decision is uploaded and the patient's co-pay settles
      //    (insurance-engine mirrors decisions onto insurance_status).
      if (pm === 'cash') {
        await this.transition(appt.id, APPT_STATES.CONFIRMED, { id: 'system', role: 'system' }, 'auto-confirmed (cash)');
        this.events.emit('appointment.confirmed', { id: appt.id });
      } else if (pm === 'insurance') {
        await this.apptModel.updateOne({ id: appt.id }, { $set: { insurance_status: 'PENDING' } as any });
      }
  ...
  /**
   * F-C13: reschedule inherits the original payment posture.
   * - insurance bookings keep their insurance flow (PENDING_INSURANCE semantics map to PENDING here)
   * - card bookings that were never paid stay PENDING (a fresh intent is required)
   * - cash / already-paid CONFIRMED bookings stay CONFIRMED
   */
  private inheritRescheduleStatus(appt: any): ApptState {
    const pm = String((appt as any).payment_method || 'cash');
    if (pm === 'cash') return APPT_STATES.CONFIRMED;
    if (appt.status === APPT_STATES.CONFIRMED || appt.status === APPT_STATES.CHECKED_IN || appt.status === APPT_STATES.IN_PROGRESS) {
      return APPT_STATES.CONFIRMED; // already settled upstream
    }
    return APPT_STATES.PENDING;
  }
  ...
  async reschedule(id: string, user: any, body: { slot_start: string }) {
    ...
    const fresh = await this.apptModel.create({
      ...
      // F-C13: a reschedule must inherit the ORIGINAL payment state. Hard-coding
      // CONFIRMED let an unpaid PENDING card booking become a free CONFIRMED clone.
      status: this.inheritRescheduleStatus(appt),
      price: appt.price,
      ...
      payment_method: (appt as any).payment_method,
      payment_id: (appt as any).payment_id,
      rescheduled_from_id: appt.id,
      state_history: [{ state: this.inheritRescheduleStatus(appt), at: new Date(), by_user_id: user.id, by_role: user.role, note: 'rescheduled-from-' + appt.id }],
    });
    try {
