...
export class AdminNotificationCenterService {
  ...
  private validateCampaignInput(body: any) {
    ...
    if (!segment.startsWith('user:') && body?.audience_confirmed !== true) {
      throw new BadRequestException('audience confirmation required');
    }
    ...
    if (route && (typeof route !== 'string' || !route.startsWith('/') || route.includes('//') || route.includes('..') || route.length > 160)) {
      throw new BadRequestException('unsafe deep link route');
    }
  ...
  async resolveSegment(segment: string): Promise<string[]> {
    ...
    if (segment.startsWith('user:')) {
      ...
      if (!user?.id) throw new NotFoundException('target user not found');
      return [user.id];
    }
  ...
  async sendCampaign(id: string) {
    ...
    try {
      ...
      if (userIds.length === 0) throw new BadRequestException('campaign audience is empty');
