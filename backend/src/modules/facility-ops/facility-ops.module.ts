...
export class ShiftsService {
  ...
  /** P3: complete the shifts CRUD — real edit with ownership. */
  async updateShift(facilityId: string, shiftId: string, body: any) {
    const shift = await this.shiftModel.findOne({ id: shiftId, facility_id: facilityId });
    if (!shift) throw new NotFoundException('shift_not_found');
    const patch: any = {};
    for (const k of ['user_id', 'department_id', 'start_time', 'end_time', 'day_of_week']) {
      if (body?.[k] !== undefined) patch[k] = String(body[k]);
    }
    if (body?.status && ['scheduled', 'substitute', 'cancelled'].includes(String(body.status))) patch.status = body.status;
    await this.shiftModel.updateOne({ id: shiftId }, { $set: patch });
    return (await this.shiftModel.findOne({ id: shiftId }, { projection: { _id: 0 } }).lean()) as any;
  }

  /** P3: delete a shift — hard remove by the owning facility only. */
  async deleteShift(facilityId: string, shiftId: string) {
    const res = await this.shiftModel.deleteOne({ id: shiftId, facility_id: facilityId });
    if (!res.deletedCount) throw new NotFoundException('shift_not_found');
    return { ok: true };
  }
...
export class FacilityShiftsController {
  ...
  @Patch(':id')
  updateShift(@CurrentUser() u: any, @Param('id') id: string, @Body() b: any) {
