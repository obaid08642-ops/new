...
export class LabsService {
  ...
  async compatibleProviders(testIds: string[]) {
    ...
    const services = await this.svcModel.find({ id: { $in: ids }, active: true }, { _id: 0, category: 1 });
  ...
  async uploadReport(id: string, user: any, body: { name?: string; mime?: string; base64?: string; url?: string; notes?: string; structuredData?: any[] }) {
    ...
    const reportable = [LabBookingState.SAMPLE_COLLECTED, LabBookingState.PROCESSING, LabBookingState.RESULT_UPLOADED, LabBookingState.REPORTED];
  ...
  async registerSample(user: any, body: { lab_order_id: string; barcode: string; tests: string[]; notes?: string }) {
    ...
    if (b.state === LabBookingState.CONFIRMED || b.state === LabBookingState.NEW_REQUEST) {
  ...
  async updateSampleStage(user: any, sampleId: string, stage: 'received' | 'analyzing' | 'result_ready' | 'sent', notes?: string) {
    ...
    if (b) {
      ...
      else if (stage === 'result_ready') targetBookingState = LabBookingState.RESULT_UPLOADED;

      // State machine: never resurrect terminal bookings, and only advance within the sample pipeline
      const pipeline = [LabBookingState.SAMPLE_COLLECTED, LabBookingState.PROCESSING];
      if (targetBookingState && b.state !== targetBookingState && pipeline.includes(b.state)) {
