...
export class LabsService {
  ...
  /** Family on-behalf check (same rule as appointments): shared group + owner or 'booking' permission. */
  ...
  async book(user: any, data: any) {
    ...
    if (data.member_id && String(data.member_id) !== String(user.id)) {
      await this.assertFamilyBookingRight(user.id, String(data.member_id));
      bookedForMemberId = String(data.member_id);
    }
