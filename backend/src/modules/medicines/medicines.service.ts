...
import { localizeMedicineStructured, DbLang } from './med-i18n';
...
export class MedicinesService {
  ...
  async list(search?: string, category?: string, includeUnverified = true, maxItems = 500, userId?: string) {
    ...
    const cacheKey = `med:list:${search || ''}:${category || ''}:${includeUnverified}:${cap}`;
    const cached = await this.redis.getJson<any[]>(cacheKey);
    if (cached) return cached;
    let rows: any[] = [];
    ...
    if (search && search.trim().length >= 2) {
      try {
        rows = await this.model.find(
          { $text: { $search: search.trim() }, is_deleted: { $ne: true }, ...(category ? { category } : {}) } as any,
  ...
  async cursorPage(search: string | undefined, category: string | undefined, cursor: string | undefined, limit = 30) {
    const safeLimit = Math.min(Math.max(limit, 1), 100);
    const q: any = { is_deleted: { $ne: true } };
    if (search) Object.assign(q, this.buildQuery(search, category));
    ...
    const cacheKey = `med:cursor:${search || ''}:${category || ''}:${cursor || ''}:${safeLimit}`;
    const cached = await this.redis.getJson<any>(cacheKey);
    if (cached) return cached;
  ...
  async generateHotMedicines() {
    ...
    for (const t of terms) {
      ...
      const hits = await this.model.find(
        { $or: [{ name_ar: { $regex: term, $options: 'i' } }, { name_en: { $regex: term, $options: 'i' } }], is_deleted: { $ne: true } },
        { _id: 0, id: 1 } as any,
      ).limit(50);
    ...
    for (const s of scored) {
      const med: any = await this.model.findOne({ id: s.id }, MedicinesService.CARD_PROJECTION);
      if (med) {
  ...
  async hot() {
    const cached = await this.redis.getJson<any[]>('med:hot:full');
    if (cached) return cached;
    const rows = await this.hotCol.find({}, { projection: { _id: 0 } }).sort({ score: -1 }).limit(50).toArray();
    const result = rows.map((r: any) => ({ ...r.medicine, hot_score: r.score }));
    await this.redis.setJson('med:hot:full', result, 3600);
    return result;
  }
  ...
  async autocomplete(query: string) {
    ...
    const cacheKey = `med:autocomplete:${q.toLowerCase()}`;
    const cached = await this.redis.getJson<any[]>(cacheKey);
    if (cached) return cached;
  ...
  async categories() {
    const cacheKey = 'med:categories-v2';
    const cached = await this.redis.getJson<any[]>(cacheKey);
    if (cached) return cached;
  ...
  async adminUpdateCatalog(medicineId: string, patch: any, adminId: string) {
    ...
    await this.model.updateOne({ id: medicineId }, { $set: { ...clean, ...extra, updatedAt: new Date() } });
    ...
    this.audit('medicine.admin_direct_edit', medicineId, adminId, 'admin', { before, after, images_deleted: removed });
    await this.invalidateCache();
    return { ok: true, updated: Object.keys({ ...clean, ...extra }) };
  }
}
