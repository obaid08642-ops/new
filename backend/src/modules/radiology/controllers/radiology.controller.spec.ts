...
describe('RadiologyController booking contract', () => {
  ...
  beforeEach(() => {
    ...
    services.findOne.mockReturnValue({ lean: jest.fn().mockResolvedValue({ id: 'rad-1', short_code: 'RAD-1', name_ar: 'أشعة موثقة', name_en: 'Verified scan', price: 180 }) });
  ...
  it('rejects raw scan fields and invalid schedules before persisting a booking', async () => {
    ...
    await expect(controller.book(patient, { service_id: 'rad-1', scheduled_at: 'invalid-date' })).rejects.toThrow('scheduled_at_must_be_in_the_future');
  ...
  it('resolves service names on the server and creates an owned pending booking', async () => {
    const result = await controller.book(patient, {
      service_id: 'rad-1', scheduled_at: future, delivery_mode: 'IN_CENTER',
      total: 1, scan_name_ar: 'اسم مزيف', scan_name_en: 'Client fake', scan_type_code: 'CLIENT-FAKE',
    });
    ...
    expect(bookings.create).toHaveBeenCalledWith(expect.objectContaining({
      patient_id: 'mongo-patient', service_id: 'rad-1', scan_type_code: 'RAD-1', scan_name_ar: 'أشعة موثقة', scan_name_en: 'Verified scan',
      price: 180, total: 180, delivery_mode: 'IN_CENTER', status: 'PENDING_ACCEPTANCE',
    }));
  });
});
