...
class AdminNursingPortalController extends AdminController {
  ...
  async requests() {
    const rows = await this.conn.collection('homecarebookings')
      .find({
        $and: [
          { $or: [{ state: { $in: ['REQUESTED', 'MATCHING', 'PENDING', 'requested', 'pending'] } }, { status: { $in: ['REQUESTED', 'MATCHING', 'PENDING', 'requested', 'pending'] } }] },
          { $or: [{ provider_id: null }, { provider_id: '' }, { provider_id: { $exists: false } }] },
        ],
      } as any)
  ...
  async assign(@Param('id') id: string, @CurrentUser() user: any, @Body() body: { provider_id?: string; nurse_id?: string }) {
    const pid = String(body?.provider_id || body?.nurse_id || '').trim();
    if (!pid) throw new BadRequestException('provider_id مطلوب');
    const profile = await this.conn.collection('provider_profiles')
      .findOne({ $or: [{ id: pid }, { user_id: pid }, { account_id: pid }] } as any);
    if (!profile) throw new NotFoundException('مزوّد الخدمة غير موجود');
    const res = await this.conn.collection('homecarebookings').updateOne(
      byId(id) as any,
      { $set: { provider_id: pid, state: 'ASSIGNED', assigned_by: uid(user), assigned_at: now(), updatedAt: now() } },
    );
    if (!res.matchedCount) throw new NotFoundException('الطلب غير موجود');
    return { ok: true };
  }
}
