...
class FamilyChatController {
  constructor(@InjectConnection() private conn: Connection) {}

  private async familyOf(userId: string) {
    // family owner = the patient; members share owner_id
    const member = await this.conn.collection('familymembers').findOne({ $or: [{ user_id: userId }, { member_user_id: userId }, { owner_id: userId }] } as any);
    return member ? ((member as any).owner_id || userId) : userId;
  }

  @Get('messages')
  async list(@CurrentUser() u: any, @Query('limit') limit = '50') {
