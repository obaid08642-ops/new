...
class OffersDetailController {
  ...
  /** Patient offers index — approved, currently running campaigns only. */
  @Get()
  async listOffers(@CurrentUser() user: any) {
    void user;
    const now = new Date();
    const rows = await this.conn.collection('promotioncampaigns')
      .find({ status: 'approved', end_date: { $gte: now } } as any)
    ...
    return rows.map((r: any) => ({
      ...
      discounted_price: r.discounted_price ?? null,
