...
let TourService = TourService_1 = class TourService {
    constructor(userModel) {
        ...
        this.logger = new common_1.Logger(TourService_1.name);
    }
    async getUserTourStatus(userId) {
        const user = await this.userModel.findOne({ id: userId }).lean();
        return user?.tour_progress || [];
    }
    async markStepComplete(userId, stepId) {
        await this.userModel.updateOne({ id: userId }, { $addToSet: { tour_progress: stepId } });
        return { ok: true };
    }
};
