...
**PROJECT-CONTEXT.md · v2.0 · 2026-08-01 · Production: https://api.nabd.plus · آخر commit: `9c6093c`**

> **ما هذا الملف؟** كل ما يحتاجه مبرمج جديد لفهم المشروع وإكماله من الصفر:
> المعمارية، كل وحدة، كل شاشة (239+114+33)، نماذج البيانات، مجموعات الـ APIs (1122 مسار)،
> **محرك التسوية المالية**، **النظام القانوني**، المتغيرات (109)، التشغيل المحلي، النشر الإنتاجي، الاختبار، وسير الأعمال.
> كل محتواه مُتحقق من الكود الحي والإنتاج المنشور — لا توثيق نظري.
>
> **جديد v2.0:** محرك تسوية أرباح المزودين (عمولة+ض.ق.م، منع ازدواجية، مُثبت بالأرقام الحية) · لوحة الإسعاف (5 شاشات + API قبول ذري) · شاشات عمليات الطبيب (إجازات/قوالب/تشخيصات/حظر) · أزرار QC المختبر · النظام القانوني الثنائي اللغة v1.1 · تطهير 4 موك مختبئة · إحصائيات طبيب حقيقية · إزالة رصيد 1000 الوهمي من المحافظ.
...
│  Expo RN 239 شاشة   Expo RN 114 شاشة      Next.js 33 صفحة      │
└──────────┬──────────────────┬───────────────────┬──────────────┘
           │ HTTPS/WSS        │                   │
┌──────────▼──────────────────▼───────────────────▼──────────────┐
│  nginx (443) — api.nabd.plus · admin. · provider. · live.      │
├────────────────────────────────────────────────────────────────┤
│  nabdah-backend — NestJS · 97 وحدة · 1122 مسار API             │
...
```
src/screens/{auth,doctor,pharmacy,lab,radiology,nursing,facility,ambulance,shared}/
  ← 8 أدوار، كل دور dashboard مستقل + Drug Index مشترك
src/screens/ambulance/AmbulanceDashboard.tsx   ← v2.0: لوحة الإسعاف (5 شاشات)
src/screens/doctor/DoctorOpsScreens.tsx        ← v2.0: إجازات/قوالب وصفات/تشخيصات/حظر
src/screens/lab/LabQcActions.tsx               ← v2.0: أزرار QC داخل تفاصيل الطلب
src/constants/index.ts  ← API_BASE = EXPO_PUBLIC_API_URL || localIp
src/navigation          ← Stack per role + tabs (114 Stack.Screen مسجلة)
```
...
### medicines_master (21,134) — أهم مجموعة
```
...
## 7ب) محرك التسوية المالية (v2.0 — حرج)

**المبدأ:** كل خدمة تُكتمل = قيد ربح للمزود في `platformledgerentries` — بدون هذا كان المال لا يصل لأي مزود إطلاقاً.

```
صافي المزود = الإجمالي − العمولة − (ض.ق.م 15% على العمولة فقط)
مثال مُثبت حياً: صيدلية 100 ر.س ← 87.35 (عمولة 11%) · طبيب 200 ر.س ← 177 (عمولة 10%)
```

| نقطة القيد | الخدمة | الموقع في الكود |
|---|---|---|
| `delivered` للتخصيص | صيدلية | `pharmacy/services/pharmacy-allocation.service.ts` |
| `endConsultation` | طبيب | `provider-ops/provider-ops.module.ts` |
| `nursingSign` (توقيع المريض) | تمريض | نفسه |
| `labQc` action=`double_verify` | مختبر | نفسه |
| `ambulanceComplete` | إسعاف | نفسه |

- `creditEarning(providerId, serviceType, gross, refType, refId)` — **منع ازدواجية** على (ref_type, ref_id): نفس الخدمة لا تُقيد مرتين أبداً
- نسب العمولات من `finance_config.key=commissions` (صيدلية 11% فعلياً · افتراضي 10% · VAT 15% على العمولة فقط)
- المحفظة تقرأ من نفس الدفتر: `GET /provider/wallet` ← {available, escrow, dues} · `GET /provider/wallet/transactions`
- السحب: حد أدنى 100 ر.س · أسبوعي (الأحد) · معالجة 3 أيام — عبر `payouts/provider-payouts.controller.ts`
- **v2.0:** أُزيل رصيد 1000 الافتتاحي الوهمي — المحافظ الجديدة تبدأ بصفر (`wallet.service.ts`)

## 7ج) النظام القانوني (v2.0)

- 5 سياسات في `legal_policies` (v1.1): اتفاقية مزود · شروط استخدام · خصوصية · إخلاء طبي · استرداد — عربي (نص المالك حرفياً) + إنجليزي
- تقديم ثنائي اللغة: `GET /legal/policy/:key?lang=ar|en` (ar افتراضي، en fallback)
- القبول الإلكتروني الملزم: `POST /legal/accept/:key` — أرشيف SHA256 في `legal_acceptances`
- تحديث الأدمن لأي سياسة = رفع نسخة تلقائي + إجبار إعادة القبول على كل المستخدمين
- الواجهة: `ContractModal` (مزود — زر "أوافق") · `app/(auth)/terms.tsx` + `privacy.tsx` (مريض — نصوص مدمجة احتياطية)

## 7د) واجهات سائق الإسعاف (v2.0)

- `GET /emergency/driver/missions` ← {pool: SOS غير مُسندة, mine: مهامي النشطة}
- `POST /emergency/:id/claim` — **قبول ذري**: `findOneAndUpdate` بفلتر `assigned_ambulance_id: null` — السائق الثاني يُرفض `already_claimed_or_closed` (مُختبر بسائقين متزامنين)
- المهمة: `GET /provider/ops/ambulance/:id/eta` (haversine) → `handover` → `complete` (+ قيد ربح تلقائي)
- حقول جديدة في مخطط الطوارئ: `claimed_at` · يستخدم `assigned_ambulance_id` الموجود أصلاً
...
## 12) جرد تطبيق المزود (44 ملف — 114 شاشة مسجلة في النافيجيتور)

> **v2.0:** العد بالتسجيل الفعلي في Stack.Screen: منشأة 43 · طبيب 43 · تمريض 37 · صيدلية 34 · مختبر 33 · أشعة 5+ · **إسعاف 5 (جديدة)** · جذر 8 = **114 شاشة فريدة**. الملفات الجديدة: `ambulance/AmbulanceDashboard.tsx` · `doctor/DoctorOpsScreens.tsx` · `lab/LabQcActions.tsx` · `facility/` (DischargeSummary, Announcements, AuditLog, InternalChat, Invitation, LeaveRequests, PatientTracker, ProfileConfig, Resources, UnifiedCalendar).
...
## 16) المتبقي الخارجي (قائمة تشغيل المالك — لا يوجد متبقٍ برمجي معروف)
...
4. Firebase FCM+Analytics · Sentry DSN · reCAPTCHA · Resend · بوابات الدفع (Moyasar اختبار ثم إنتاج)
5. SHA256 fingerprint + TeamID لملفي deep links (`deploy/nginx/well-known/`)
6. المراجعة القانونية السعودية للسياسات الخمس ← ثم إضافة: السجل التجاري · الاسم القانوني · العنوان · التواصل
7. EAS Project ID الحقيقي (حالياً fallback تطويري) + بناء التطبيقين للتحقق البصري النهائي على جهاز

## 17) سجل تحقق v2.0 (2026-08-01 — كلها أدلة حية على الإنتاج)

| الفحص | النتيجة |
|---|---|
| فحص تقاطع API (مزود+أدمن) | 143 استدعاء × 1122 مسار = **صفر مكسور** |
| التسوية: صيدلية 100←87.35 · طبيب 200←177 | مُثبت بالأرقام + منع ازدواجية |
| رحلة تخصيص صيدلية كاملة | pending_review→confirmed→preparing→ready→out→delivered ✅ |
| سباق قبول SOS (سائقان متزامنان) | الثاني يُرفض ذرياً ✅ |
| موك مختبئة أُزيلت | رادار صيدلية dummy endpoint · توصيل #123 · إحصائيات طبيب ثابتة · محافظ 1000 |
| إحصائيات الطبيب | إعادة كتابة كاملة: محفظة/معاملات/تقييمات/مواعيد حقيقية |
| صحة الإنتاج | 200 · 9/9 حاويات healthy |
