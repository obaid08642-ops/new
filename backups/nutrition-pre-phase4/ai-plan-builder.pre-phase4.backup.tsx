...
export default function AIPlanBuilderScreen() {
  ...
  const generate = async () => {
    // E2: require real body metrics — a diet plan for fabricated 70kg/170cm/30y numbers is a health risk
    const w = Number(form.weight), h = Number(form.height), a = Number(form.age);
    if (!w || !h || !a) {
      showLocalizedAlert('بيانات ناقصة', 'أدخل وزنك وطولك وعمرك الحقيقيين قبل إنشاء الخطة.');
      return;
    }
    const tw = Number(form.targetWeight);
    setGenerating(true);
    try {
      const planBody: any = { goal, gender, weight: w, height: h, age: a, activity, diet, allergies };
      if (tw) planBody.targetWeight = tw;
      const res = await apiFetch<any>('/ai/generate-diet-plan', {
        method: 'POST',
        body: JSON.stringify(planBody),
      });

      // Update backend profile target parameters as well (real values only)
      ...
      if (tw) profileBody.target_weight_kg = tw;
      if (typeof res?.calories === 'number') profileBody.daily_calorie_target = res.calories;
      ...
      showLocalizedAlert('خطأ', 'فشل إنشاء الخطة الغذائية. يرجى المحاولة لاحقاً.');
    } finally {
      setGenerating(false);
    }
  };
