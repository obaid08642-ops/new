...
export default function CalorieAnalyzerScreen() {
  ...
  const analyzeByText = async () => {
    ...
    try {
      ...
      showLocalizedAlert("خطأ", "فشل تحليل الوجبة. يرجى المحاولة لاحقاً.");
    } finally {
      setAnalyzing(false);
    }
  };

  const analyzeByPhoto = async () => {
    try {
      const perm = await ImagePicker.requestCameraPermissionsAsync();
      if (!perm.granted) {
        showLocalizedAlert("إذن الكاميرا", "نحتاج إذن الكاميرا لتصوير الوجبة.");
        return;
      }
      ...
      const res = await apiFetch<any>("/ai/analyze-meal", {
        method: "POST",
        body: JSON.stringify({ query: "حلّل الوجبة في هذه الصورة", image_base64: shot.assets[0].base64 }),
      });
      setResult(res);
    } catch (e: any) {
      showLocalizedAlert("خطأ", "فشل تحليل الصورة. يرجى المحاولة لاحقاً.");
    } finally {
      setAnalyzing(false);
    }
  };
