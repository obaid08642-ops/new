...
const SIZES = [
  { label: 'كوب صغير', ml: 150, icon: 'cup' },
  { label: 'كوب عادي', ml: 250, icon: 'cup-water' },
  { label: 'قنينة 500', ml: 500, icon: 'bottle-soda-outline' },
  { label: 'قنينة كبيرة', ml: 750, icon: 'bottle-soda-classic-outline' },
];
...
export default function WaterTrackerScreen() {
  ...
  const addWater = async (size: typeof SIZES[0]) => {
    ...
    try {
      ...
      showLocalizedAlert('خطأ', 'تعذر تسجيل الماء. تأكد من اتصالك بالإنترنت.');
    } finally {
      setSaving(false);
    }
  };
          ...
          <View style={styles.sizesGrid}>
            {SIZES.map((size, i) => (
              <TouchableOpacity key={i} onPress={() => addWater(size)} disabled={saving}
                ...
                <Icon name={size.icon} size={22} color={colors.primary} />
        ...
        <View style={[styles.card, { backgroundColor: isDark ? colors.surface : colors.white } ]}>
          ...
          {loading ? (
            ...
            data.logs.slice().reverse().map((entry, i) => {
              const sizeInfo = SIZES.find(s => s.ml === entry.amount_ml) || { icon: 'cup-water' };
              return (
                <View key={i} style={[styles.logRow, { borderBottomColor: colors.border } ]}>
                  ...
                  <Icon name={sizeInfo.icon} size={16} color={colors.primary} />
                </View>
              );
            })
          )}
