...
export class AiController {
  ...
  @Public()
  @Throttle({ default: { limit: 10, ttl: 60000 } }) // E5-F4: public endpoint on paid LLM quota
  @Post('triage')
  ...
  @Get('triage/history')
  triageHistory(@Req() req: any) {
    return this.svc.triageHistory(req.user?.id);
  }
  ...
  /**
   * EPIC4/S21: AI analysis of one of the patient's OWN medical reports.
   * Loads the real report server-side (ownership-checked) so the client can
   * never analyze someone else's document, then runs the AI gateway.
   */
  @Post('analyze-report')
  analyzeReport(@Req() req: any, @Body() body: { report_id?: string; text?: string }) {
    return this.svc.analyzeReportForPatient(req.user?.id, body?.report_id, body?.text);
  }
