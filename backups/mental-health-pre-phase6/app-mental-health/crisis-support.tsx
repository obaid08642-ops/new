...
const RELATIONSHIP_AR: Record<string, string> = {
  ...
  mental_health_helpline: 'خط مساندة نفسية',
  social_protection: 'حماية اجتماعية',
};
const RELATIONSHIP_COLOR: Record<string, string> = {
  emergency_service: '#DC2626',
  mental_health_helpline: '#7A6BEA',
  social_protection: '#0D9488',
};
...
export default function CrisisSupportScreen() {
  ...
  useEffect(() => {
    // E2: single fetch — backend returns { user_contacts, default_contacts }.
    // Previously this endpoint was called twice and hotlines read a non-existent shape → always empty.
    apiFetch('/mental-health/crisis-contacts')
      .then((res: any) => {
        setUserContacts(Array.isArray(res?.user_contacts) ? res.user_contacts : []);
        setHotlines(Array.isArray(res?.default_contacts) ? res.default_contacts : []);
      })
      .catch(() => {});
  }, []);
    ...
    <View style={[styles.container, { backgroundColor: colors.background } ]}>
      <View style={[styles.header, { paddingTop: insets.top + 8 } ]}>
        ...
        <View style={[styles.urgentBanner, { backgroundColor: isDark ? colors.surface : colors.white } ]}>
          <AppText variant="bodySM">إذا كنت في خطر فوري — اتصل بـ 911 الآن</AppText>
          <TouchableOpacity onPress={() => call('911')} style={styles.callEmergencyBtn}>
            <Icon name="call" size={18} color="#F0695C" />
            <AppText variant="bodySM">911</AppText>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.safetyCard, { backgroundColor: '#FEF3C7' } ]}>
          <AppText variant="bodySM">خطوات الأمان الآن </AppText>
        ...
        {/* Hotlines — real default contacts from GET /mental-health/crisis-contacts */}
        {hotlines.length > 0 && (
          <>
            <AppText variant="bodySM">خطوط المساعدة المتخصصة </AppText>
            {hotlines.map((line, i) => (
              <TouchableOpacity key={line.id || i} onPress={() => call(line.phone)}
                style={[styles.hotlineCard, { backgroundColor: isDark ? colors.surface : colors.white }]}
                activeOpacity={0.85}>
                <View style={[styles.callBtn, { backgroundColor: RELATIONSHIP_COLOR[line.relationship] || '#7A6BEA' } ]}>
                  <Icon name="call" size={18} color="#fff" />
                </View>
                <View style={styles.hotlineInfo}>
                  <AppText variant="bodySM">{line.contact_name}</AppText>
                  <AppText variant="bodySM">{RELATIONSHIP_AR[line.relationship] || 'خط مساعدة'}</AppText>
                  <AppText variant="bodySM">{line.phone}</AppText>
                </View>
              </TouchableOpacity>
            ))}
          </>
        ...
        {/* Self-help tools */}
        <AppText variant="bodySM">أدوات المساعدة الذاتية الآن </AppText>
