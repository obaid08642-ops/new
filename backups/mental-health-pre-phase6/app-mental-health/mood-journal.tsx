...
const MOOD_EMOJIS = [
  { icon: 'emoticon-excited-outline', label: 'رائع', value: 5, color: '#5BA84F' },
  { icon: 'emoticon-happy-outline', label: 'جيد', value: 4, color: '#84CC16' },
  { icon: 'emoticon-neutral-outline', label: 'محايد', value: 3, color: '#F0A526' },
  { icon: 'emoticon-sad-outline', label: 'حزين', value: 2, color: '#F97316' },
  { icon: 'emoticon-frown-outline', label: 'قلق', value: 1, color: '#F0695C' },
];
...
export default function MoodJournalScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showLocalizedAlert('خطأ', 'تعذر حفظ المزاج');
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background } ]}>
      <View style={[styles.header, { paddingTop: insets.top + 8 } ]}>
        ...
        <AppText variant="bodySM">{new Date().toLocaleDateString(dateLocale(), { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</AppText>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, { backgroundColor: isDark ? colors.surface : colors.white } ]}>
          <AppText variant="bodySM">كيف مزاجك اليوم؟ </AppText>
          <View style={styles.moodRow}>
            {MOOD_EMOJIS.map(m => (
              <TouchableOpacity key={m.value} onPress={() => setSelectedMood(m.value)}
                ...
                <Icon name={m.icon} size={26} color={selectedMood === m.value ? m.color : colors.textSecondary} />
        ...
        {loadingHistory ? (
          ...
          const dateStr = new Date(entry.logged_at ?? entry.createdAt).toLocaleDateString(dateLocale(), { weekday: 'short', month: 'short', day: 'numeric' });
          return (
