  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setInsCompanies(Array.isArray(list) && list.length ? list : []);
      } catch { setInsCompanies([]); }
    })();
  }, [showInsurance]);
            ...
            {!isGuest && userProfile.insuranceId ? (
              ...
              <View style={{ gap: 10 }}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingVertical: 4 }}>
                  {(insCompanies.length ? insCompanies : INSURANCE_COMPANIES).map(company => (
                    <TouchableOpacity key={company.id} onPress={() => { setInsCompany(company.id); setInsCategory(''); }} style={[st.insChip, { borderColor: insCompany === company.id ? colors.primary : colors.border, backgroundColor: insCompany === company.id ? colors.primarySurface : 'transparent' } ]}>
                      <AppText variant="labelSM" color={insCompany === company.id ? colors.primary : colors.textSecondary} numberOfLines={1}>{company.name_ar || company.name}</AppText>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
                {insCompany !== '' && (
                  <View style={{ gap: 6 }}>
                    ...
                    <View style={{ flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 6 }}>
                      {(insCategories.length ? insCategories : INSURANCE_CATEGORIES).map(cat => (
