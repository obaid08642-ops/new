...
export default function OrderDetails() {
  ...
  const handleDownload = async () => {
    // E2 fix: was a fake "تم التحميل" alert after a malformed request. Now opens the
    // real report URL if the provider published one, otherwise says so honestly.
    const url = order?.report_url || order?.report?.url || order?.result_pdf_url
      || (Array.isArray(order?.reports) && order.reports[0]?.url);
    if (url) {
      try { await Linking.openURL(url); } catch {
        showLocalizedAlert('تعذر الفتح', 'تعذر فتح ملف التقرير على هذا الجهاز.');
      }
    } else {
      showLocalizedAlert('التقرير غير متاح بعد', 'سيظهر رابط التقرير هنا فور نشره من المزود.');
    }
  };
