...
export default function InsuranceUpload() {
  ...
  const [companies, setCompanies] = useState<any[]>(INSURANCE_COMPANIES_FULL);
  ...
  React.useEffect(() => {
    ...
    const fetchCompanies = async () => {
      try {
        // Live catalog from the backend; constants remain as offline fallback
  ...
  const activeCompany = companies.find(c => c.id === selCompany || c.code === selCompany) || companies[0];
            ...
            <View style={styles.section}>
              ...
              >
                <View style={{ flexDirection: I18nManager.isRTL ? 'row' : 'row-reverse', alignItems: 'center' }}>
                  ...
                  <AppText style={{ fontSize: 15, fontWeight: 'bold', color: colors.textPrimary }}>{pickLocalized(activeCompany?.name_ar, activeCompany?.name)} - {activeClass?.nameAr}</AppText>
                </View>
                ...
                <View style={{ backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 16, marginTop: 8, padding: 12 }}>
                  <ScrollView style={{ maxHeight: 200 }} nestedScrollEnabled={true}>
                    {companies.slice(0, 15).map(c => (
                      <TouchableOpacity
                        key={c.id || c.code}
                        style={{ paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border }} onPress={() => { setSelCompany(c.id || c.code); setSelClass('b'); setShowInsPicker(false); }}
                      >
