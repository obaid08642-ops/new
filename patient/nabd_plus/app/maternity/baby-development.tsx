...
import Fetus3DViewer from '../../src/components/Fetus3DViewer';
...
const safeCareForWeek = (week: number) => {
  if (week <= 12) return [
    'اختاري وجبات صغيرة ومتوازنة واشربي الماء بانتظام بحسب ما يناسبك.',
    'راجعي مقدم الرعاية قبل بدء أو إيقاف أي مكمل أو دواء.'
  ];
  if (week <= 27) return [
    'استمري في التنوع الغذائي والراحة والنشاط الذي أقرّه فريق الرعاية المتابع لحملك.',
    'ناقشي أي تغير مزعج أو سؤال غذائي خاص مع مقدم الرعاية.'
  ];
  return [
    'حافظي على الوجبات المنتظمة والترطيب والراحة وفق خطة الرعاية الخاصة بك.',
    'تواصلي مع فريق الرعاية عند أي تغير يقلقك أو عند وجود أسئلة عن حركة الجنين.'
  ];
};
...
export default function BabyDevelopmentScreen() {
    ...
    async function loadCurrentWeek() {
      try {
        ...
        if (profile && profile.is_pregnant) {
          ...
          const verifiedWeek = Math.max(1, Math.min(40, Number(profile.current_week)));
          setCurrentWeek(verifiedWeek);
          setSelectedWeek(verifiedWeek);
        } else {
          setCurrentWeek(null);
  ...
  const data = selectedWeek ? WEEKS_DATA[selectedWeek] : null;

  const handlePrevWeek = () => {
    if (selectedWeek && selectedWeek > 1) setSelectedWeek(selectedWeek - 1);
  };
  
  const handleNextWeek = () => {
    if (selectedWeek && selectedWeek < 40) setSelectedWeek(selectedWeek + 1);
  };

  // Premium-vector policy: size comparison is conveyed with the ruler vector icon
  // (no emoji glyphs anywhere in the app).
  ...
  if (!selectedWeek || !data) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background, justifyContent: 'center', padding: 24 }]}> 
        <View style={[styles.emptyState, { backgroundColor: isDark ? colors.backgroundSecondary : '#FFF1F5' }]}> 
          <Icon name="heart" size={34} color="#EC4899" />
          <AppText variant="h5" style={{ textAlign: 'center' }}>أكملي ملف متابعة الحمل</AppText>
          <AppText variant="bodySM" color={colors.textSecondary} style={{ textAlign: 'center', lineHeight: 22 }}>
            سنعرض أسبوع الحمل الصحيح والمرئيات التعليمية بعد حفظ تاريخ آخر دورة أو موعد الولادة المتوقع.
          </AppText>
          <TouchableOpacity onPress={() => router.push('/maternity/maternity-setup')} style={styles.emptyAction}>
            <AppText variant="bodySM" color="#FFF" style={{ fontWeight: '800' }}>إعداد متابعة الحمل</AppText>
          </TouchableOpacity>
        </View>
      </View>
    );
    ...
    <View style={[styles.container, { backgroundColor: colors.background } ]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.vizCard, { backgroundColor: cardBg } ]}>
          <View style={styles.visualContainer}>
            {is3DMode ? (
              <Fetus3DViewer
                ...
                height={280}
                accent="#EC4899"
                background={isDark ? '#3B2432' : '#FCE7F3'}
              />
            ) : (
            ...
            >
              <AppText variant="caption" color={is3DMode ? '#FFF' : colors.textPrimary} style={{ fontWeight: 'bold' }}>
                {is3DMode ? 'عرض الصورة' : 'مجسم تفاعلي'}
              </AppText>
            </TouchableOpacity>
          </View>
          ...
          <View style={{ alignItems: 'center' }}>
            <View style={[styles.rotateLabel, { backgroundColor: isDark ? colors.backgroundSecondary : '#FCE8F1', borderWidth: 1, borderColor: '#FBC4D6' } ]}>
              <Icon name={is3DMode ? 'refresh' : 'search'} size={14} color="#EC4899" />
              <AppText variant="caption" color="#EC4899" style={{ fontFamily: 'Cairo-Bold' }}>
                {is3DMode ? 'اسحبي لتدوير المجسم واضغطي على الجزء التعليمي' : 'يمكنك تكبير وتصغير الصورة'}
              </AppText>
            </View>
            <AppText variant="caption" color={colors.textSecondary} style={{ textAlign: 'center', marginTop: 8, fontSize: 10, lineHeight: 16 }}>
        ...
        {/* Week Navigator Controller */}
          <View style={[styles.controllerCard, { backgroundColor: cardBg } ]}>
            {currentWeek && currentWeek !== selectedWeek && (
              <TouchableOpacity onPress={() => setSelectedWeek(currentWeek)} style={styles.currentWeekPill}>
                <AppText variant="caption" color="#9D174D" style={{ fontWeight: '800' }}>العودة إلى أسبوعك الحالي</AppText>
              </TouchableOpacity>
            )}
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statBox, { backgroundColor: isDark ? colors.backgroundSecondary : '#FDF4E0' } ]}>
              <View style={{ flexDirection: 'row-reverse', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                ...
                <Icon name="straighten" size={18} color="#F0A526" />
              </View>
        ...
        <View style={[styles.adviceCard, { backgroundColor: adaptivePinkBg } ]}>
          ...
          {safeCareForWeek(selectedWeek).map((tip, idx) => (
            <View key={idx} style={styles.bulletRow}>
...
const styles = StyleSheet.create({
  ...
  currentWeekPill: { alignSelf: 'center', backgroundColor: '#FCE7F3', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 7, marginBottom: 12 },
  emptyState: { borderRadius: 24, padding: 24, alignItems: 'center', gap: 14 },
  emptyAction: { backgroundColor: '#EC4899', borderRadius: 14, paddingHorizontal: 18, paddingVertical: 12, marginTop: 4 },
