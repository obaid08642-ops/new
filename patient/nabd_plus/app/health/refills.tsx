          ...
          onPress: async () => {
            ...
            try {
              ...
              if (res?.ok && res?.order_id) {
                // Reload from server — never fabricate refill quantities/days locally
                loadRefills();
                showLocalizedAlert(
                  'تم إنشاء الطلب',
                  'أُرسل طلب إعادة الصرف للصيدليات. ستصلك العروض قريباً.',
                  [{ text: 'تتبع الطلب', onPress: () => router.push('/pharmacy/order-tracking') }, { text: 'حسناً', style: 'cancel' }],
                );
              } else {
