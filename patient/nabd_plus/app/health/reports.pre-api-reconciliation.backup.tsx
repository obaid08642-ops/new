...
export default function ReportsScreen() {
  ...
  // Guests CAN view reports — device-bound guest account.
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background } ]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={[styles.reportCard, { backgroundColor: isDark ? colors.surface : colors.white }]}
            activeOpacity={0.85}
            onPress={() => router.push({ pathname: '/reports/view-report', params: { id: item.id } })}
          >
            <View style={styles.reportHeader}>
              <View style={styles.reportActions}>
                {item.critical && (
                  <View style={[styles.pdfBtn, { backgroundColor: '#FEE2E2' }]}>
                    <Icon name="warning" size={14} color="#DC2626" />
                    <AppText variant="bodySM" color="#DC2626">حرج</AppText>
                  </View>
                )}
              </View>
