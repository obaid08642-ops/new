...
### 4ج-2. نتيجة تقييم نفاذ/Nafath (بحث 24/07 — dga.gov.sa/oss · توثيق API)
- **نفاذ** هوية وطنية (DGA/NIC): 17.2M تحميل · 470+ منصة مدمجة · 380M طلب تحقق · متوسط إتمام 20ث · الرعاية الصحية حالة استخدام مذكورة رسميًا.
- **ليس OIDC/SAML قياسيًا** — بروتوكول خاص: `POST /ExtNafath/request {nationalId}` ← `{transId, random}` ← المستخدم يوافق في تطبيق نفاذ (بيومتري) ← استطلاع `/ExtNafath/status` أو Webhook ← `GET /ExtNafath/details/{transId}` للبيانات الموثقة. يدعم مواطن (NIN) ومقيم (Iqama).
- **المتطلب:** تسجيل كيان قانوني لدى DGA + SP API Key/Secret (شروط أهلية حسب النشاط) — لا يمكن التنفيذ قبل الحصول على الاعتماد.
- **القرار:** قيمة عالية لـ ① KYC المزودين (يغني عن رفع مستندات يدوية) ② توثيق هوية المريض (يقلل الاحتيال ويملأ البيانات تلقائيًا). **التصميم جاهز للتنفيذ عند الحصول على الاعتماد:** موديول `nafath` (request/status/callback webhook + ربط national_id بالمستخدم + حالة verified). مسجل كبند M8 (متعلق باعتماد خارجي).
...
| M6 | أمن وامتثال + التوجيهات المؤسسية ER (§4ج) | ✅ **مكتملة** (4 دفعات، فرع m6-seo-enterprise) — بنود اعتماد خارجي في M8 |
...
**متبقي M6 (نُقل لـ M7/M8):** N+1 (M7) · i18n محتوى الباك إند (M7) · نفاذ (اعتماد DGA خارجي — §4ج-2) · ZATCA-2 (اعتماد خارجي) · إحصاء التسليم UI.

### M6 — الدفعة 4 (مكتملة 24/07): SEO-2 + نفاذ + استرجاع — **M6 مغلقة**
- **موديول articles كامل:** مخطط ثنائي اللغة + عام (منشورة فقط: list/categories/slug+views) + CMS أدمن (CRUD/publish) + دمج SEO كامل (نوع article: resolve/meta/JSON-LD Article/sitemap 0.6) + صفحة `/s/article/:slug` + دليل `/articles` SSR + بطاقة رئيسية + robots + زرع 3 مقالات (`scripts/seed-articles.ts`).
- **استرجاع:** `scripts/seed-insurance-companies.ts` (M3) كان مفقودًا من التسليم — أُعيد إنشاؤه (10 شركات، upsert).
- **نفاذ:** تقييم موثق §4ج-2 — بروتوكول DGA خاص يتطلب اعتماد كيان؛ التصميم جاهز، التنفيذ M8.
...
| M7 | جودة: اختبارات حرجة · إصلاح أخطاء إنتاجية · N+1 · إزالة @ts-nocheck (تجريبي) | ✅ (دفعة أولى 24/07) |
...
## 5ز. M7 — الجودة الشاملة (الدفعة الأولى — 24/07)

### أ) الاختبارات — من خط أساس مكسور إلى أخضر كامل
- **الحالة النهائية: 23 جناحًا / 184 اختبارًا — كلها خضراء + tsc نظيف.**
- خط الأساس كان 5 أجنحة فاشلة (مواصفات قديمة من M0: إضافة RedisService لكونستركتور AuthService، token صار كائنًا).
- أُضيفت اختبارات حرجة جديدة:
  - `insurance-engine/tests/insurance-flow.spec.ts` (26 اختبارًا): قرار المزود (approve_full/partial/reject)، حساب copay، صلاحيات (provider/admin/stranger)، payCopay بكل الحالات، cancel، Quote (BR-1: القنوات online-only العشرة لا تسمح clinic_pay)، نوافذ الاسترداد (>24س=100%، 4–24س=50%، <4س=0%، منتهي=0%)، منع التكرار.
  - `billing/tests/billing-zatca.spec.ts`: TLV QR وسوم 1–5 (ترتيب/قيم/طول بايت UTF-8 عربي/متجه مرجعي ASCII) + استخراج VAT 15% الشامل. صار `tlvQr` مُصدَّرًا (export).
  - `care/tests/appointments-states.spec.ts` (13 اختبارًا): آلة حالات الموعد المغلقة، رفض التحولات غير الصالحة والنهائية، ختم confirmed_at + history + أحداث، قواعد الإلغاء (مريض >24س=100% مصدر / <24س=50% محفظة / no-show=0% / طبيب=100%+غرامة 50ر)، reschedule.

### ب) أخطاء إنتاجية حقيقية اكتُشفت وأُصلحت (كلها كان يخفيها @ts-nocheck)
1. **pdfmake:** `require('pdfmake/js/printer')` — الملف الفعلي `Printer.js` (حرف كبير)؛ ينهار على أنظمة ملفات حساسة لحالة الأحرف (لينكس إنتاجي!). صُحح في `lab-pdf.service.ts`.
2. **مخططات مفقودة كليًا:** `CarePlanSchema`/`HomeCarePackageSchema`/`MedicalSupplyRequestSchema` كانت تُستورد في home-care.module وغير موجودة في home-care.schema.ts — أُضيفت (مع HomeCarePackage جديد: اسم/سعر/جلسات/مدة).
3. **HomeCareBookingState غير مستورد** في home-care.service (سطر checkIn) — ReferenceError عند أول check-in.
4. **خلل check-in حرج:** التقرير كان يُنشأ بـ `home_care_order_id` (يُسقطه المخطط) وبلا `booking_id` المطلوب ← كان سيفشل validation إنتاجيًا. صُحح: `booking_id` + `patient_id`، وأُضيفت حقول الزيارة للمخطط (check_in/out_time, gps, completed_tasks, vitals_logged)، وحقول الحجز (patient_name/phone, service_id/en, notes, sessions_count).

### ج) N+1 (ER-10)
- فحص آلي: 30 نقطة await-داخل-حلقة؛ المزروعة (seeds) مقبولة.
- **أُصلح:** `orders.service.createOrder` — جلب الأدوية دفعة واحدة `$in` بدل findOne لكل عنصر (أكثر مسار كتابة سخونة).
- **مقبول بمبرر:** care.service available_today (hasSlotsToday لكل طبيب — كسر مبكر بحد limit)، assignment-strategy expireStale (cron دفعات صغيرة)، pharmacy-broadcast/shortage (مطابقة $or لكل عنصر — إعادة تصميم مؤجلة).


### هـ) الدفعة الثانية (24/07 — لاحقًا نفس اليوم): صفر @ts-nocheck كلي + إزالة الموك الإنتاجي
- **@ts-nocheck: من 204 ملفات إلى صفر في الباك إند كاملًا** — بعد الإزالة ظهرت 109 أخطاء TypeScript حقيقية أُصلحت كلها؛ كشفت أخطاء إنتاجية جسيمة:
  1. `chat.controller` مستورد في app.module وغير موجود (كان سيمنع الإقلاع لو فُحص) + تكرار تسجيل ProviderOpsModule.
  2. **FCM مكسور كليًا:** كود M6 كُتب بواجهة firebase-admin v11 بينما المثبّت v14 (لا `admin.apps/messaging`) — أُعيد كتابته بواجهة v14 (`getApps/cert/getMessaging`).
  3. **RBAC ناقص 5 أدوار** (guest/hospital_admin/branch_admin/receptionist/nursing) في ROLE_PERMISSIONS ← صلاحيات فارغة لها.
  4. حالات مستخدمة وغير معرّفة في المخططات (تفشل validation عند الحفظ): `LabBookingState.CREATED` (حجوزات المختبر!)، `NursingBookingState.PROVIDER_ASSIGNED` و`CANCELLED`، `RadiologyBookingState.SCHEDULED`، `RESULT_READY`.
  5. `family.service.getMemberHealth` — كان ينهار دائمًا (this.connection غير موجود + متغير memberId غير معرّف).
  6. حقول مفقودة تُسقط البيانات صامتًا: HomeCareBooking (home_visit_fee/transportation_fee)، ProviderAccount.phone_e164، ProviderProfile.business_name، NursingVisitReport (gps/أوقات/مهام).
  7. ازدواج ProviderAvailability: المستودعان كانا يشيران لمخطط working-hours اليتيم بدل مخطط الحالة الحقيقي في provider/schemas/requests.schema + `account_id`→`provider_account_id` في provider-dashboard (توفر المزود كان لا يُحفظ/يُقرأ أبدًا).
  8. ServiceOwnership: فحص الملكية يقرأ `provider_account_id` غير الموجود (الحقل account_id) ← كل مزود غير admin كان يُرفض دائمًا في updateService/deleteService.
  9. مستودعات repositories بأنواع Document خاطئة + `distinct` مفقود من MongoRepository + استيرادات `any` مولّدة (31 ملفًا) + appointment.repository يستورد من extra.schemas بدل appointment.schema.
  10. ترتيب وسيطات ioredis `SET ... EX ttl NX` في قفل الحجز.
- **إزالة الموك/البليس هولدر الإنتاجي (اشتراط المستخدم):**
  - محفظة الممرضة: رصيد 1250/عمولة 400/معاملة وهمية ← قيم حقيقية (صفر/فارغ).
  - loyalty: حذف البذر التلقائي للمكافآت الوهمية — القائمة من DB فقط.
  - detectFraud: كان يختلق تنبيهات بمعرفات mock ← كشف حقيقي (سرعة حجوزات ≥10/24س، محاولات دفع فاشلة ≥5/6س) مع منع تكرار التنبيه.
  - **حذف AIB2BProcurementController وPharmacyReturnsController الوهميين** — كانا يطمسان مسارات حقيقية: Gemini الحقيقي (AiController) وإرجاعات حقيقية مع استرداد محفظة (ReturnsModule).
  - LeaveRequestsController: قائمة ثابتة + no-op ← مخطط LeaveRequest حقيقي + إنشاء/قرار مع قيود facility.
  - radiology: `parent_appointment_id: 'APT-1234'` ← referring_doctor_id الحقيقي.
- **سوكت المزود:** DoctorDashboard كان يرسل `(user)?.token` (فارغ غالبًا) وPharmacyChatResponder بلا auth إطلاقًا — كلاهما كان سيُرفض من البوابة ← الآن `auth: (cb)` بجلب JWT الحقيقي من Vault (Tokens.getAccess). المريض متوافق مسبقًا (مساران JWT).
- **أداء:** Sentry tracesSampleRate ← 0.1 إنتاجي / 1.0 تطوير. GeoSpatial: موثق كدين M8 — المواقع {lat,lng} مع haversine داخل التطبيق؛ فهرس 2dsphere بلا GeoJSON سيكون شكليًا (لا يُضاف زيفًا).
- **البوابات النهائية:** `tsc --noEmit`=0 (204 راية محذوفة) · `jest`=184/184 · `nest build`=ناجح.

### د) استراتيجية @ts-nocheck (204 ملف backend) — **اكتملت كليًا في الدفعة الثانية (انظر هـ أعلاه)**
- القرار: إزالة تدريجية موجهة بالحرجية، وليس big-bang. **الدفعة الأولى أزالت 6 ملفات home-care كاملة (service + 5 repositories) بصفر أخطاء متبقية** — وكشفت الخللين 3 و4 أعلاه.
- الترتيب القادم: auth.service ← notifications.service ← care/appointments ← labs/radiology ← billing ← ثم الباقي (موثق في الديون).

**ملاحظة M7-تسليم:** رفع فرع `m7-quality` إلى GitHub تعذّر لغياب رصيد الدفع في الجلسة — الزيب `nabdah-backend-M7.zip` + `سجل_تغييرات_المرحلة_M7.md` هما مرجع التسليم حتى يُرفع.


### و) الاختبار الحي E2E (24/07 — بعد الدفعة الثانية): بيئة حقيقية داخل الصندوق
- **بنية:** mongodb-memory-server (Mongo حقيقي) + redis-memory-server (Redis حقيقي) + `node dist/main.js` (بناء إنتاجي) — 16 مسارًا ذهبيًا عبر HTTP الحقيقي.
- **النتيجة النهائية: 16/16 PASS** (SEO robots/sitemap · تسجيل/دخول/me · Quote BR-1 للقناتين · دليل الأطباء · بحث الأدوية · مقالات · إشعارات · تأمين · محفظة · مواعيد · حظر أدمن 403 · رفض غير الموثّق 401).
- **3 فجوات حقيقية انكشفت بالاختبار الحي فقط وأُصلحت:**
  1. **DoctorsModule غير مسجل في app.module** — دليل الأطباء العام + الحجز + صندوق المواعيد + الرسائل كله كان كودًا ميتًا غير قابل للوصول (404 حي).
  2. **RolesGuard غير مسجل عالميًا** — كل @Roles(UserRole.ADMIN) في التطبيق كانت بلا أثر؛ 21 كنترولر admin مكشوف لأي مستخدم موثّق. الإصلاح: تسجيل عالمي + **قاعدة مسار نظامية** (أي /admin/* يتطلب admin/super_admin حتى بلا decorator) + توحيد حالة الأحرف بين enumين.
  3. **seo.service.ts فقد تكامل المقالات** (فقدان الماونت المتكرر — ضاعت قبل زيب M6 أصلًا): modelFor بلا case للمقالات ← meta/sitemap لا يرى المقالات. أُعيد: الحقن + الحالة + الاسم بالعناوين + الوصف + JSON-LD Article + sitemap 0.6 — تحقق حي: 3 مقالات مبذورة تظهر في sitemap وmeta(found:true).
