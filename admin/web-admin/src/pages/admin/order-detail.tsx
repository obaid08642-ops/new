...
export default function OrderDetailPage() {
  const [routeParams, setRouteParams] = useState<{ kind?: string; id?: string }>({});
  ...
  useEffect(() => {
    ...
    setRouteParams({ kind: query.get('kind') || undefined, id: query.get('id') || undefined });
  }, []);

  const { kind, id } = routeParams;
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      ...
      <button onClick={goBack} className="text-teal-700 font-bold text-sm">→ عودة لمركز القيادة</button>
