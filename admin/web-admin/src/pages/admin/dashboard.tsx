...
const payloadRows = (value: unknown): JsonRecord[] => {
  const source = Array.isArray(value) ? value : (isRecord(value) && Array.isArray(value.data) ? value.data : []);
    ...
    const fetchData = async () => {
      try {
        ...
        if (heatRes.ok) {
          ...
          const raw = payloadRows(heatJson);
          const maxCount = Math.max(1, ...raw.map((r) => Number(r.count) || 0));
          ...
          const mapped: HeatmapData[] = raw.slice(0, 40).flatMap((r) => {
            ...
            // Do not invent a map position for a telemetry record without verified coordinates.
            if (!valid) return [];
            return [{
              region: String(r.city || r.region || '—'),
              demandType: String(r.category || r.demandType || 'unknown'),
              count: Number(r.count) || 0,
              density: Math.min(100, Math.max(10, Math.round(((Number(r.count) || 0) / maxCount) * 100))),
              coordinates: [5 + ((lng - LNG_MIN) / (LNG_MAX - LNG_MIN)) * 90, 95 - ((lat - LAT_MIN) / (LAT_MAX - LAT_MIN)) * 90],
            }];
          });
        ...
        if (ccRes.ok) {
          ...
          setLiveOrders(payloadRows(cc?.live_bookings) as LiveOrder[]);
        }
      } catch (error) {
          ...
          {liveOrders.length === 0 ? (
            ...
            <table className="w-full text-right text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {liveOrders.slice(0, 50).map((o, i: number) => {
