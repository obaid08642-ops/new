...
- [ ] Phase 8 remediation batch B — realtime room ownership: remove generic arbitrary room subscription; authorize each room by canonical appointment/thread/family membership and active participant status; reject malformed/unowned room requests with no event leak; add focused socket-gateway regression tests before any realtime UX change.
