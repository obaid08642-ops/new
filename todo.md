...
- [ ] التحقق القرائي في Sandbox من قائمة الوصفات الطبية وعقد ملكية التفاصيل قبل إضافة أي عرض أو إرسال للصيدلية؛ أثبت Sandbox قراءة القائمة للمالك و`401` بلا جلسة، لكن يبقى اختبار ملكية التفاصيل مقفلاً لعدم وجود عملية تفاصيل مريض موثقة أو معرّف مساري قابل للاختبار (G-PRESCRIPTION-001).
...
- [ ] التحقق القرائي في Sandbox من قائمة محادثات المريض ومنع كشف معرف محادثة غير مملوكة قبل بناء واجهة الدردشة؛ أثبت Sandbox قراءة القائمة للمالك و`401` بلا جلسة، لكن يبقى اختبار ملكية التفاصيل مقفلاً لعدم وجود عملية تفاصيل/رسائل مريض موثقة أو معرّف مساري قابل للاختبار (G-CHAT-001).
...
- [ ] تحويل تفاصيل الدواء المنشور إلى قراءة عامة SSR ببيانات مقيدة، canonical وhreflang وJSON-LD من دون سعر أو مرفقات أو أي بيانات مريض؛ اكتملت النسخة التحفظية لمسار الكتالوج الحالي بـ`MedicalWebPage` وhreflang للغات الست واختبار منع التسرب، لكنها تبقى `noindex` إلى أن يسلم Backend DTO النشر والتصنيف في G-SEO-002.
...
- [x] تصحيح hreflang للصفحة العامة ليضم العربية والإنجليزية والأردية والهندية والبنغالية والفلبينية بدلاً من لغتين فقط؛ تحقق HTML الفعلي من بدائل اللغات الست و`x-default`.
- [x] إعداد حزمة مراجعة للمبرمج والمراجع تشمل عقود Backend المطلوبة، سجل GitHub والإصلاحات المنفذة، وتدقيق فجوة التصميم الحالي مقابل مرجع تطبيق المريض React Native مع خطة مطابقة مرئية قبل أي ادعاء باكتمال الواجهة.
- [ ] إعادة بناء نظام التصميم وواجهة لوحة المريض للويب من مرجع React Native، بما يشمل الألوان والتخطيط والبطاقات والتفاعل والاستجابة، ثم التحقق المرئي RTL/LTR على الهاتف وسطح المكتب قبل تعميم اللغة البصرية على كل الصفحات؛ اكتمل أساس الألوان وخط Cairo المرجعي ولوحة المريض ببطاقات وأيقونات واستجابة هاتفية، ويستمر تعميم اللغة البصرية على بقية الصفحات.
- [x] إصلاح القص الأفقي الظاهر في شاشة الدخول العربية على عرض 390px بعد إدخال خط ونظام التصميم المرجعي؛ تحقق العرض بصرياً على 390px بالعربية، وبقيت شاشة الإنجليزية سليمة في التحقق السابق.
- [x] إصلاح منع موارد Next التطويرية على نطاق معاينة الويب حتى يعمل تسجيل الدخول بـSandbox والتحقق المرئي من لوحة المريض الخاصة دون تغيير أي سياسة إنتاجية؛ أضيفت أصول المعاينة فقط وأعيد تشغيل الخادم.
- [x] استبدال إجراء «دخول المريض» العام داخل الجلسة النشطة بإجراء حساب/خروج واعٍ بالجلسة، مع الحفاظ على عدم كشف أي هوية أو توكن في الرأس؛ تحقق Sandbox من عرض الإجراءين، ثم أعاد الخروج إلى الصفحة العامة وأخفى الوصول الخاص.
- [x] إعادة تصميم صفحة الطلبات الخاصة ببطاقات وحالات فارغة/خطأ مرجعية متجاوبة، مع الإبقاء على الحقول المسموحة وروابط BFF وملكية المورد بلا أي بيانات بديلة؛ تحقق Sandbox من البطاقات الحية ومن غياب أي توكن في HTML.
- [x] إعادة تصميم قائمة المواعيد الخاصة ببطاقات خدمة وحالة وموعد متجاوبة مستلهمة من مرجع الجوال، مع إبقاء الحالة والتاريخ القادمين من BFF فقط ومنع أي حجز أو إلغاء أو تعديل غير موثق؛ تحقق Sandbox من البطاقات الحية ومن غياب أي بيانات موسعة أو توكن من HTML.
- [x] إعادة تصميم قائمة تذكيرات الدواء للقراءة فقط ببطاقات جدول دوائي مستلهمة من مرجع الجوال، مع إبقاء الجرعة والتوقيت والتكرار القادمة من BFF ومنع تسجيل التناول أو التأجيل أو الجدولة المحلية؛ تحقق Sandbox من الحالة الفارغة ومن غياب أي إجراء تغيير أو بيانات غير مسموح بها.
- [x] إعادة تصميم قائمة الإشعارات للقراءة فقط ببطاقات توضح الأولوية ووقت الإنشاء وحالة غير المقروء، مع منع تعليم القراءة أو تنفيذ روابط حمولة الإشعار حتى تثبت عقودها؛ تحقق Sandbox من القائمة الحية والبطاقات غير التفاعلية.
- [x] منع عرض مفاتيح ترجمة أو أولويات تقنية خام من حمولة الإشعارات في الواجهة، وتوثيق عقد Backend للرسالة المحلية القابلة للعرض من دون اختلاق نص بديل؛ تحقق Sandbox من اختفاء مفاتيح `notif.*` والأولويات الخام.
- [x] اعتماد نظام تصميم Premium أصلي لنبض بلس مستلهم من مبادئ Apple Human Interface Guidelines: تباين هادئ، مسافات دقيقة، طبقات زجاجية مدروسة، حركة قصيرة تحترم تقليل الحركة، وأيقونات Vector موحدة بلا emoji أو أزرار نمطية؛ اكتمل الأساس في الرأس والصفحة العامة والبطاقات والأزرار، مع علامة نبض/حماية مستقلة ومراجع رسمية موثقة.
- [ ] إنشاء مكتبة أصول Vector ورسوم توضيحية أصلية تستخدم فقط في الواجهات العامة والخاصة المناسبة، مع مراعاة الأداء والاتجاهين RTL/LTR وعدم إدخال بيانات أو ادعاءات طبية داخل الرسوم.
- [ ] مراجعة وتعزيز SEO وAEO وGEO للصفحات العامة المؤهلة فقط، بما يشمل بنية المحتوى والبيانات المنظمة والروابط الداخلية والـhreflang وملفات الاكتشاف، مع الإبقاء على الصفحات الخاصة والكتالوج المختلط خارج الفهرسة.
- [x] ترقية لوحة المريض الخاصة إلى طبقات Premium: علامة نبض Vector، بطاقات وصول أكثر دقة، ومواد زجاجية محدودة في التنقل فقط، مع الحفاظ على مسارات الجلسة والبيانات كما هي؛ تحقق Sandbox من المظهر الحي ومن بقاء الروابط وبيانات الجلسة ضمن حدودها.
- [x] ترقية كتالوج الدواء القرائي ببحث وبطاقات Premium وأيقونات Vector، مع الإبقاء على `noindex` ووسم `WebPage` وعدم إضافة سعر أو سلة أو وصفة أو ادعاء دوائي إلى الواجهة أو البيانات المنظمة؛ تحقق العرض الحي من البيانات القادمة من المصدر فقط.
- [x] معالجة مهلة اتصال API العامة للكتالوج كحالة خطأ قرائية صريحة داخل الصفحة بدلاً من استثناء خادمي، مع عدم إعادة المحاولة آلياً أو إنشاء بيانات بديلة؛ أضيف اختبار وحدة لفشل الشبكة.
- [x] استبدال خط Cairo كخط واجهة افتراضي بنظام طباعي حديث أقرب لإحساس Apple، مع fallback عربي ولغات لاتينية/هندية/بنغالية صريح، وإبقاء Cairo خياراً ثانوياً فقط إن لزم للقراءة الموسعة؛ يستخدم النظام خط الجهاز على أجهزة Apple بلا تضمين أو توزيع SF، مع fallback قانوني متعدد النصوص.
- [x] صقل الحركة إلى انتقالات طبقية هادئة ومختصرة للأزرار والبطاقات والتنقل، مع احترام تفضيل تقليل الحركة ومنع أي حركة مشتتة أو معالجة مزيفة لبيانات طبية؛ تحقق العرض بصرياً واجتازت الدفعة فحوص الإنتاج.
- [x] ترقية صفحة الملف الصحي إلى واجهة Premium ببطاقات حالة وأيقونات Vector للحقول المصرح بها فقط، مع الإبقاء على حالات النقص والخطأ والرفض ومنع أي كشف لهوية أو تأمين أو تفاصيل طبية غير معتمدة؛ تحقق Sandbox من الحقول والحالات الحية.
- [x] منع عرض enum غير المحدد للجنس في الملف الصحي بدلاً من عرض `unspecified` الخام، مع إخفاء أي قيمة غير معتمدة بدل تخمين معناها؛ تحقق Sandbox من اختفائها ويُطلب من Backend DTO محلي أو enum موثق قبل إضافة تسمية مترجمة.
