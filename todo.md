...
- [x] الكتالوج العام: أضيفت eligibility/review/provenance fail-closed إلى Medicine/ProviderProfile/Facility/Lab/Radiology/HomeCare؛ قُيدت أسطح public/SEO/discovery والخدمات العامة بـ`public_eligibility:true` و`medical_review_status:'approved'` فوق الحالة التشغيلية. أضيف `CatalogPublicationService` لإسقاط public مدقق، metadata/sitemap/feed/deep-link، invalidation وevent idempotent؛ وmigration dry-run/apply/rollback محمية. اجتازت Backend build و70 suites/401 tests. لا migration أو نشر. الدليل: `NABDAH_CATALOG_GOVERNANCE_PUBLICATION_HARDENING_20260820.md`.
- [x] projection/events: بعد اعتماد medicine/provider أو ApprovalWorkflow، يُحدث `CatalogPublicationService` projection/cache/metadata/sitemap/feed/deep-link متزامناً ويسجل `catalog.publication.projected` بمفتاح idempotency sparse-unique؛ replay لا يعمل fanout ثانياً. اختبارات publication/EventBus مغطاة ضمن 70 suites/401 tests. لا worker خارجي أو نشر background أو إرسال sitemap فعلي.
