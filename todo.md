...
- [ ] Phase 8 remediation batch N — workflow/event integrity: reject unknown lifecycle state mapping, route externally reachable booking/order transitions through canonical workflow guards, block event fanout when durable event persistence fails, and add focused state/event regression tests before provider/patient status UI polish.
