...
- [ ] Phase 8 remediation batch K — payment intent idempotency: require an owned booking and client idempotency key for payment intent creation/retry, prevent parallel active intents at database/query level, enforce gateway-reference uniqueness, expose an existing pending intent safely, and add focused concurrency/duplicate regressions without contacting a live gateway.
