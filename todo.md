...
- [ ] Phase 8 remediation batch E — Patient locale-aware navigation: remove unconditional RTL and Arabic-only bottom-navigation assumptions; derive direction, labels and accessibility metadata from the active supported locale; retain compact/premium navigation state behavior; and add focused static/runtime tests before broader screen restyling.
