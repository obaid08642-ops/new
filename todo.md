...
- [ ] طلب/توثيق إضافة عقد `GET /home-care/bookings/{id}` في OpenAPI والخلفية قبل إنشاء صفحة تفاصيل الرعاية المنزلية.
...
- [ ] طلب/توثيق تعديل Backend لعقد OTP بحيث يعيد تحقق الرمز الناجح جلسة مريض آمنة أو تفويضاً قصير العمر قابلاً لاستبداله بجلسة، قبل إضافة تسجيل الدخول بـOTP إلى الويب.
- [ ] التحقق القرائي في Sandbox من قائمة العائلة وحدود BOLA لسجلات الأعضاء قبل إنشاء أي واجهة عائلة.
...
- [ ] طلب/توثيق DTO عائلة يعيد `display_name` اختياريًا من الخلفية بدلاً من `user_id` وصلاحيات العضو الخام فقط.
