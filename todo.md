...
- [ ] Phase 8 remediation batch F — family ownership contracts: reconcile member identifier usage with canonical backend IDs, remove legacy family chat data paths, require server-backed active membership before chat/permission operations, and add focused tests without reading or changing non-sandbox family data.
