...
- [ ] Phase 8 remediation batch C — payment authorization and webhook integrity: verify payment retry/read ownership before any mutation, require signed trusted gateway webhook configuration, fail closed when signature material is absent/invalid, and add focused regression tests without attempting live payments while Moyasar is owner-deferred.
