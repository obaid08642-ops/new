...
- [ ] Phase 8 remediation batch G — clinical vitals contract: reconcile Patient vital input/output keys with the canonical Backend schema (`glucose`, `heart_rate` and approved units), reject malformed clinical entries instead of silently remapping/drop values, and add focused contract tests before any health-summary UI change.
