...
- [ ] Provider PharmacyChatResponder truthfulness: replace unauthenticated Socket.IO connection, locally fabricated invoice/chat entries, Arabic-only UI and optimistic send with a participant-authenticated, persistent pharmacy-chat contract or a fail-closed unavailable state.
- [ ] Provider shared operations truthfulness: audit `NotificationsCenter` local read-state mutation and `SupportCenter` hard-coded ticket data, then bind each to an evidenced backend contract or remove/contain it without representing demo data as live operations.
