import { Injectable, NotFoundException, BadRequestException, ConflictException, ServiceUnavailableException, Inject, forwardRef, Optional } from '@nestjs/common';
...
export class HealthService {
   ...
   */
  async refillNow(_user: any, _id: string): Promise<never> {
    // Refill previously delegated to legacy OrdersService.create, which can
    // auto-dispatch, persist PII, and create an orphan before canonical gates.
    // No reservation or database mutation is allowed until a canonical refill
    // PharmacyOrder command is implemented.
