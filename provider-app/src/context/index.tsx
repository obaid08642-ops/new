...
export function ThemeProvider({ children }: { children: ReactNode }) {
 const sys = Appearance.getColorScheme() as ThemeMode ?? 'light';
 const [mode, setMode] = useState<ThemeMode>(sys);
  ...
  useEffect(() => {
    ...
    const sub = Appearance.addChangeListener(({ colorScheme }) => {
      ...
      Vault.set('THEME_MODE', colorScheme);
    });
    return () => sub.remove();
  }, []);
  ...
  const toggle = useCallback(() => {
    setMode(m => {
      ...
      Vault.set('THEME_MODE', nm);
      return nm;
    });
  }, []);

 return (
 <ThemeCtx.Provider value={{ mode, theme: getTheme(mode), toggle, set: setMode }}>
 {children}
 </ThemeCtx.Provider>
 );
}
