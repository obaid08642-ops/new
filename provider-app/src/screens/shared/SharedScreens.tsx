...
export function WithdrawalWorkflow({ onBack }: { onBack: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [balRes, bankRes, mineRes] = await Promise.all([
        ...
        client.get('/provider/bank-account').catch(() => ({ data: null })),
        client.get('/provider/payouts/mine').catch(() => ({ data: [] })),
      ]);
      const b = balRes.data || {};
  ...
  const canSubmit = !!balance && !balance.negative && bankFormValid && !!amt && amt >= 100 && amt <= balance.available && !hasPending && !submitting;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await client.post('/provider/payouts/request', { amount: amt, iban: cleanIban });
      setSubmitted(true);
    } catch (err: any) {
