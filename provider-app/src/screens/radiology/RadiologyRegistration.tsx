...
// ─── Types ────────────────────────────────────────────────────────────────────
const CENTER_TYPES = [
  { id: 'lab', color: 'tokens.purple', label_ar: 'معمل تحاليل', label_en: 'Radiology' },
  { id: 'radiology',color: 'tokens.mintDeep', label_ar: 'مركز أشعة', label_en: 'Radiology Center' },
  { id: 'both', color: 'tokens.navy', label_ar: 'معمل تحاليل + أشعة', label_en: 'Radiology + Radiology' },
] as const;

interface RadiologyRegData {
  // Step 1
...
function LStep2({ data, update, onNext, onBack, step, total, bare = false, submitRef }: {
  ...
  const validate = () => {
    ...
    // Custom validation for Radiology/Radiology separation
    const isRadiology = data.centerType === 'lab' || data.centerType === 'both';
    const isRad = data.centerType === 'radiology' || data.centerType === 'both';
    
    if (isRadiology && !(data as any).labCategory?.trim()) {
      e.labCategory = AR ? 'فئة المختبر مطلوبة' : 'Radiology category required';
    }
  ...
  const pickDocument = (field: keyof RadiologyRegData) => {
    Alert.alert(
  ...
  const DocCard = ({ label, field, req }: { label: string; field: keyof RadiologyRegData; req?: boolean }) => {
  ...
  const body = (
    <>
      ...
      <NInput label={AR ? 'رقم ترخيص وزارة الصحة MOH' : 'MOH License Number'}
        ...
        hint={AR ? 'ترخيص المركز/مركز الأشعة من MOH' : 'Radiology/Radiology license from MOH'} />

      {/* Radiology Specific Setup */}
      {(data.centerType === 'lab' || data.centerType === 'both') && (
        <>
          <NInput label={AR ? 'فئة المختبر (MOH Category)' : 'MOH Radiology Category'}
          ...
          <NInput label={AR ? 'الاعتماد الدولي/المحلي (مثل CBAHI, CAP)' : 'Radiology Accreditation (e.g. CBAHI, CAP)'}
...
function LStep4({ data, update, onNext, onBack, step, total, bare = false, submitRef }: {
  ...
  const isRadiology = data.centerType === 'lab' || data.centerType === 'both';
  const isRad = data.centerType === 'radiology' || data.centerType === 'both';
  const [tab, setTab] = useState<'lab' | 'rad'>(isRadiology ? 'lab' : 'rad');
  ...
  const validate = () => {
    if (isRadiology && data.enabledTests.length === 0 && !isRad) {
      Alert.alert(AR ? 'تنبيه' : 'Warning', AR ? 'يجب تفعيل تحليل واحد على الأقل' : 'Enable at least one test');
      return false;
    }
    if (isRad && data.enabledScans.length === 0 && !isRadiology) {
  ...
  const body = (
    <>
      ...
      {/* LAB TESTS */}
      {(tab === 'lab' && isRadiology) && filteredTests.map(test => {
...
function LStep8Signature({ data, update, onDone, onBack, step, total }: {
  ...
  if (submitted) return <RegistrationSuccess onDone={onDone} email={data.managerEmail} providerType="radiology" />;
