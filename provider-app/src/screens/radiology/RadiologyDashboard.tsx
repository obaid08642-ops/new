...
import { ProviderWalletScreen, MedicalJobsScreen } from '../shared/SharedScreens';
        ...
        {({ navigation }) => {
          ...
          return (
            <View style={{ flex: 1 }}>
              ...
              {activeTab === 'settings' && <RadiologySettingsScreen onLogout={onLogout} />}
...
function RadiologyHome({ onNav }: { onNav: (s: string, p?: any) => void }) {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    } catch { setStats({ todayCount: 8, inScanCount: 2, completedCount: 5, revenue: 3200 }); } finally { setLoading(false); }
  }, []);
  useEffect(() => { fetchOrders(); }, [fetchOrders]);
  return (
    <NScroll refreshControl={<RefreshControl refreshing={loading} onRefresh={fetchOrders} tintColor="#009688" />}>
      <NHeader title={AR ? 'لوحة الأشعة ☢️' : 'Radiology Dashboard ☢️'} right={<TouchableOpacity onPress={() => onNav('wallet')} style={{ padding: SP.sm }}><I name="wallet" size={24} color={theme.primary} /></TouchableOpacity>} />
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SP.md, marginBottom: SP.xl }}>
        ...
        <NStatCard icon="✓" label={AR ? 'مكتمل' : 'Completed'} value={String(stats.completedCount)} color="#4CAF50" style={{ width: '47%' }} />
        ...
        return (
          <NCard key={order.id} style={{ marginBottom: SP.md }} accent={meta.color} onPress={() => onNav('order_detail', order)}>
            ...
            <View style={{ flexDirection: 'row', gap: SP.sm, flexWrap: 'wrap' }}>
              {sq.is_pregnant !== undefined && <View style={{ backgroundColor: sq.is_pregnant ? '#F4433620' : '#4CAF5020', borderRadius: R.sm, paddingHorizontal: 6, paddingVertical: 2 }}><Text style={{ fontSize: 9, color: sq.is_pregnant ? '#F44336' : '#4CAF50' }}>{AR ? `حمل: ${sq.is_pregnant ? 'نعم ⚠️' : 'لا ✓'}` : `Pregnant: ${sq.is_pregnant ? 'YES ⚠️' : 'NO ✓'}`}</Text></View>}
              {sq.has_pacemaker !== undefined && <View style={{ backgroundColor: sq.has_pacemaker ? '#F4433620' : '#4CAF5020', borderRadius: R.sm, paddingHorizontal: 6, paddingVertical: 2 }}><Text style={{ fontSize: 9, color: sq.has_pacemaker ? '#F44336' : '#4CAF50' }}>{AR ? `منظم: ${sq.has_pacemaker ? 'نعم ⚠️' : 'لا ✓'}` : `Pacemaker: ${sq.has_pacemaker ? 'YES ⚠️' : 'NO ✓'}`}</Text></View>}
              {sq.has_contrast_allergy !== undefined && <View style={{ backgroundColor: sq.has_contrast_allergy ? '#F4433620' : '#4CAF5020', borderRadius: R.sm, paddingHorizontal: 6, paddingVertical: 2 }}><Text style={{ fontSize: 9, color: sq.has_contrast_allergy ? '#F44336' : '#4CAF50' }}>{AR ? `صبغة: ${sq.has_contrast_allergy ? 'نعم ⚠️' : 'لا ✓'}` : `Contrast: ${sq.has_contrast_allergy ? 'YES ⚠️' : 'NO ✓'}`}</Text></View>}
          ...
          return (
            <NCard key={order.id} style={{ marginBottom: SP.md }} accent={meta.color} onPress={() => onNav('order_detail', order)}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ fontSize: FS.xs, color: theme.textSub }}>{order.scheduled_at ? new Date(order.scheduled_at).toLocaleString('ar-SA') : '—'}</Text>
    ...
    <NScroll>
      ...
      <NCard style={{ marginBottom: SP.lg }}>
        ...
        <View style={{ backgroundColor: theme.surface2, borderRadius: R.md, padding: SP.md }}>
          <Text style={{ fontSize: FS.xs, color: theme.textSub, textAlign: AR ? 'right' : 'left' }}>{AR ? 'الموعد:' : 'Scheduled:'} {currentOrder.scheduled_at ? new Date(currentOrder.scheduled_at).toLocaleString('ar-SA') : '—'}</Text>
      ...
      <NSecHeader title={AR ? '🛡️ استبيان السلامة' : '🛡️ Safety Questionnaire'} />
      <NCard style={{ marginBottom: SP.lg }}>
        {[{ key: 'is_pregnant', ar: 'حمل أو اشتباه حمل', en: 'Pregnant / Suspected' }, { key: 'has_pacemaker', ar: 'منظم ضربات قلب', en: 'Pacemaker' }, { key: 'has_metal_implant', ar: 'دعامات معدنية', en: 'Metal Implants' }, { key: 'has_contrast_allergy', ar: 'حساسية من الصبغة', en: 'Contrast Allergy' }].map(item => {
          ...
          return (<View key={item.key} style={{ flexDirection: AR ? 'row-reverse' : 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: SP.sm, borderBottomWidth: 1, borderColor: theme.border }}><Text style={{ fontSize: FS.sm, color: theme.text }}>{AR ? item.ar : item.en}</Text><Text style={{ fontSize: FS.sm, fontWeight: FW.bold, color: val ? '#F44336' : '#4CAF50' }}>{val ? (AR ? 'نعم ⚠️' : 'YES ⚠️') : (AR ? 'لا ✓' : 'NO ✓')}</Text></View>);
        })}
      ...
      <View style={{ gap: SP.md, marginBottom: SP.xl }}>
        ...
        {currentOrder.state === 'ARRIVED_CHECKIN' && (<>
          <NBtn label={AR ? '☢️ استدعاء لغرفة الأشعة' : '☢️ Call to Scanning Room'} loading={loading} onPress={() => doAction('start-scan')} style={{ backgroundColor: '#FF9800' }} />
        ...
        {currentOrder.state === 'REPORT_DRAFT' && <NBtn label={AR ? '👁️ إرسال للمراجعة الطبية' : '👁️ Submit for Radiologist Review'} loading={loading} onPress={() => doAction('submit-report-for-review')} />}
        ...
        {currentOrder.state === 'SCAN_ABORTED' && (<View style={{ backgroundColor: '#F4433612', borderRadius: R.md, padding: SP.md }}><Text style={{ color: '#F44336', fontWeight: FW.bold, textAlign: 'center', marginBottom: SP.md }}>{AR ? `⚠️ سبب الإلغاء: ${currentOrder.abort_reason}` : `⚠️ Abort Reason: ${currentOrder.abort_reason}`}</Text><NBtn label={AR ? '📅 إعادة الجدولة' : '📅 Reschedule'} loading={loading} onPress={() => doAction('reschedule-scan')} /></View>)}
  ...
  return (
    <NScroll>
      ...
      <NCard style={{ marginBottom: SP.lg }}>
        ...
        <NBtn label={pdfUploaded ? (AR ? '✅ PDF مرفوع — إعادة الرفع' : '✅ PDF Uploaded — Re-upload') : (AR ? '⬆️ رفع تقرير PDF' : '⬆️ Upload PDF Report')} loading={loading} onPress={handleUploadPdf} style={{ backgroundColor: pdfUploaded ? '#4CAF50' : '#009688' }} />
      ...
      <NSecHeader title={AR ? '🖼️ قسم 2: صور DICOM (اختياري)' : '🖼️ Section 2: DICOM Images (Optional)'} />
      <NCard style={{ marginBottom: SP.lg }}>
        ...
        <NBtn label={AR ? '⬆️ رفع صور الأشعة (JPEG/PNG)' : '⬆️ Upload Scan Images (JPEG/PNG)'} variant="outline" loading={loading} onPress={() => show(AR ? 'سيتم دعمه مع تكامل S3' : 'Coming with S3 integration', 'info')} />
      </NCard>

      {reportStatus === 'draft' && pdfUploaded && <NBtn label={AR ? '👁️ إرسال للمراجعة الطبية' : '👁️ Submit for Radiologist Review'} loading={loading} onPress={handleSubmitForReview} style={{ marginBottom: SP.md }} />}
...
function CatalogManagerTab() {
  ...
  useEffect(() => { client.get('/provider/capabilities/radiology-services').then(r => setServices(r.data || [])).catch(() => {}).finally(() => setLoading(false)); }, []);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
            ...
            <View style={{ flexDirection: AR ? 'row-reverse' : 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 4, marginTop: 4, flexWrap: 'wrap' }}>
                  {s.requires_pregnancy_check && <View style={{ backgroundColor: '#FF980020', borderRadius: 4, padding: 2 }}><Text style={{ fontSize: 8, color: '#FF9800' }}>🤰 Pregnancy</Text></View>}
                  {s.requires_metal_implant_check && <View style={{ backgroundColor: '#9C27B020', borderRadius: 4, padding: 2 }}><Text style={{ fontSize: 8, color: '#9C27B0' }}>🔧 Metal</Text></View>}
                  {s.requires_contrast_allergy_check && <View style={{ backgroundColor: '#F4433620', borderRadius: 4, padding: 2 }}><Text style={{ fontSize: 8, color: '#F44336' }}>💉 Contrast</Text></View>}
                </View>
              </View>
      ...
      <NSheet visible={showAdd} onClose={() => setShowAdd(false)} title={AR ? 'إضافة فحص جديد' : 'Add New Scan'}>
        <Text style={{ fontSize: FS.xs, color: '#FF9800', textAlign: AR ? 'right' : 'left', marginBottom: SP.md }}>{AR ? '⚠️ يذهب للمراجعة الإدارية قبل ظهوره للمرضى.' : '⚠️ Goes for admin review before patient visibility.'}</Text>
        ...
        <Text style={{ fontSize: FS.sm, fontWeight: FW.bold, color: theme.text, marginVertical: SP.md }}>{AR ? '🛡️ عوامل الخطر (Risk Flags)' : '🛡️ Risk Flags'}</Text>
...
// ══════ SETTINGS ══════
function RadiologySettingsScreen({ onLogout }: { onLogout: () => void }) {
  const { theme } = useTheme(); const { lang } = useLang();
  const AR = lang === 'ar';
  return (
