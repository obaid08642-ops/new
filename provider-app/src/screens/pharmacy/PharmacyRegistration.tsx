...
import { SP, R, FS, FW, PHARMA_CATS, CITIES, LIMITS, C, INSURANCE } from '../../constants';
...
const INIT: PharmacyRegData = {
  ...
  nameAr:'', nameEn:'', type:'', managerName:'', managerPhone:'',
  ...
  return (
    <NScroll>
      ...
      <TouchableOpacity onPress={() => setShowVacationCal(true)}>
        <NInput label={AR ? 'إجازة مخططة (إيقاف الطلبات مؤقتاً)' : 'Planned Vacation'} placeholder={AR ? 'اختر التاريخ من التقويم...' : 'Choose date...'} value={data.vacationDate} onChange={() => {}} editable={false} icon="📅" />
      </TouchableOpacity>
  ...
  const handleSignature = async (signature: string) => {
    ...
    try {
      ...
      await ProviderApi.step3({
        ...
        const images: string[] = [];
        if (data.logoUri) images.push(await ProviderApi.uploadFile(data.logoUri, 'image/jpeg', 'pharmacy_logo'));
