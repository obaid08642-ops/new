        ...
        <View style={{ backgroundColor: theme.surface2, padding: SP.md, borderRadius: R.md, marginBottom: SP.md }}>
          ...
          {data.offersHome && (
            <View>
              ...
              </View>              <View style={{ marginTop: SP.md }}>
  ...
  return (
    <NScroll>
      ...
      <TouchableOpacity onPress={() => setShowVacationCal(true)}>
        <NInput label={AR ? 'إجازة مخطط لها' : 'Planned Vacation'} placeholder={AR ? 'اضغط لاختيار تاريخ إجازتك...' : 'Choose vacation date...'} value={data.vacationDate} onChange={(val) => { update({ vacationDate: val }); setShowVacationCal(false); }} editable={false} icon="📅" />
      </TouchableOpacity>
...
function Step7Signature({ data, update, onDone, onBack, step, total }: any) {
  ...
  const handleSignature = async (signature: string) => {
    ...
    try {
      ...
      if (data.scheduleType === 'unified') {
        ...
        wh = (data.perService['clinic']?.days || []).map((d: string) => ({
      ...
      if (data.profilePhotoUri && !data.profilePhotoUri.startsWith('http')) {
        images.push(await ProviderApi.uploadFile(data.profilePhotoUri, 'image/jpeg', 'doctor_profile.jpg'));
      } else if (data.profilePhotoUri) {
        images.push(data.profilePhotoUri);
      }
      ...
      await ProviderApi.submit({ signer_name: data.signerName, signer_role: data.signerRole, lat: data.lat, lng: data.lng });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
      ...
      <OtpModal visible={showOtp} onClose={() => setShowOtp(false)} target={data.managerEmail || data.email} onVerify={async (code) => { const ok = await handleVerifyOtp(code); if(ok) { setShowOtp(false); finishSubmit(); return true; } return false; }} />
    </View>
  );
}
