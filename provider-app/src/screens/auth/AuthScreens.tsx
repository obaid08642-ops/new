...
export function SplashScreen({ onDone }: { onDone: () => void }) {
 ...
 <Text style={{ fontSize: 54, color: '#FFF' }}></Text>
 </View>
 </Animated.View>
...
export function WelcomeScreen({
        ...
        try {
          ...
          if (!r.ok) {
            ...
            if (msg.includes('Network Error') || msg.includes('Network request failed')) {
              alert(`Debug Info:\nTried to connect to: ${API_BASE}\nMake sure the backend is running on this IP and port 8002!`);
            }
          }
        } catch (err: any) {
          const msg = err.message || err.error || err;
          show(AR ? `خطأ: ${msg}` : `Error: ${msg}`, 'error');
          // TEMPORARY DEBUGGING ALERT FOR USER:
          if (String(msg).includes('Network') || String(msg).includes('Network request failed') || String(msg).includes('timeout')) {
            alert(`Debug Info:\nTried to connect to: ${API_BASE}\nMake sure the backend is running on this IP and port 8002!`);
          }
        }
      }
 ...
 <ScrollView
 contentContainerStyle={{ padding: SP.xl, paddingTop: SP.sm, paddingBottom: SP.huge }}
...
export function LoginScreen({
 ...
 <ScrollView contentContainerStyle={{ flexGrow: 1, padding: SP.xl, paddingTop: 60 }}
 ...
 <NDivider label={AR ? 'أو' : 'or'} />
        ...
        <View style={{ marginVertical: SP.lg, gap: SP.sm }}>
          <Text style={{ textAlign: 'center', color: theme.textSub, fontSize: FS.sm, marginBottom: SP.sm }}>{AR ? 'حسابات اختبار سريعة' : 'Quick Test Accounts'}</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: SP.sm }}>
            {[
              { id: 'doctor_test@nabd.com', label: AR ? 'طبيب' : 'Doctor' },
              { id: 'facility_test@nabd.com', label: AR ? 'مستشفى' : 'Facility' },
              { id: 'pharmacy_test@nabd.com', label: AR ? 'صيدلية' : 'Pharmacy' },
              { id: 'lab_test@nabd.com', label: AR ? 'مختبر' : 'Lab' },
              { id: 'radiology_test@nabd.com', label: AR ? 'أشعة' : 'Radiology' },
              { id: 'nursing_test@nabd.com', label: AR ? 'تمريض' : 'Nursing' }
            ].map(ta => (
              <TouchableOpacity key={ta.id} onPress={() => { setId(ta.id); setPass('Test1234'); }} style={{ paddingHorizontal: SP.md, paddingVertical: SP.sm, borderRadius: R.md, borderWidth: 1, borderColor: theme.border, backgroundColor: theme.surface2 }}>
                <Text style={{ color: theme.primary, fontSize: FS.xs, fontWeight: FW.bold }}>{ta.label}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
  ...
  <Modal visible={showBioPrompt} transparent animationType="fade">
    <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: SP.xl }}>
      <View style={{ backgroundColor: theme.card, padding: SP.xl, borderRadius: R.xl, width: '100%', alignItems: 'center' }}>
        <Text style={{ fontSize: 50, marginBottom: SP.md }}>👤</Text>
...
export function ForgotPasswordScreen({
 ...
 setLoading(false); animNext(); setStep(3);
 };
 ...
 const data = await res.json();
 if (!res.ok) throw new Error(data.message || (AR ? 'فشل استعادة كلمة المرور' : 'Failed to reset password'));
 
 show(AR ? 'تم تغيير كلمة المرور ' : 'Password changed ', 'success');
 onSuccess();
 } catch (e: any) {
 setErr(e.message || (AR ? 'حدث خطأ ما' : 'Something went wrong'));
 } finally {
...
export function PendingScreen({
 ...
 <View style={{ flex: 1, backgroundColor: theme.bg }}>
 <StatusBar barStyle={theme.statusBar} />
 <ScrollView contentContainerStyle={{ padding: SP.xl, paddingTop: 80, paddingBottom: 48 }}>
 ...
 {[
 { icon: '️', ar: 'البريد الإلكتروني', en: 'Email' },
