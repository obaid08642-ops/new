...
export function NLogo({ size = 56 }: { size?: number }) {
 ...
 <Text style={{ fontSize:size*0.52, color:theme.primary }}></Text>
 </View>
 );
}
...
export function NInput({
  ...
  icon, iconRight, onIconRight, multi, lines=4, maxLen, editable=true,
  ...
  multi?:boolean; lines?:number; maxLen?:number; editable?:boolean;
      ...
      <View style={[{ marginBottom:SP.lg }, style]}>
        <View pointerEvents="box-none">
          ...
          <Animated.View style={[s.inputWrap, {
            ...
            {icon && <Text style={[s.inputIcon, { color:focused?theme.primary:theme.textHint, paddingTop:multi?SP.xs:0 }]}>{icon}</Text>}
            <TextInput
              ref={inputRef}
            ...
            {secure && (
              <TouchableOpacity onPress={() => setShowPass(p=>!p)}>
                <Text style={s.inputIcon}>{showPass ? '👁️' : '👁️‍🗨️'}</Text>
              </TouchableOpacity>
            )}
            {iconRight && !secure && (
...
// ─── Empty State ──────────────────────────────────────────────────────────────
export function NEmpty({ icon='', title, sub, actionLabel, onAction }:
...
export function NSearch({ value, onChange, placeholder, style }:
 ...
 <Text style={{ fontSize:18, color:theme.textHint }}></Text>
 ...
 {value.length>0 && (
 <TouchableOpacity onPress={()=>onChange('')}><Text style={{ fontSize:14, color:theme.textHint }}></Text></TouchableOpacity>
 )}
 </View>
 );
}
...
export function NInteractiveMap({ radius, baseLabel = 'الرياض' }: { radius: number; baseLabel?: string }) {
  ...
  // Calculate a mock circle scale/width based on radius (e.g. 1KM to 50KM)
  const baseSize = 80;
  ...
  return (
    <View style={{
      ...
      {/* Saudi Neighborhood Mock Tags */}
      <View style={{ position: 'absolute', top: 20, left: 30, opacity: 0.5 }}>
        <Text style={{ color: theme.textSub, fontSize: 10 }}>حي الياسمين</Text>
      </View>
      <View style={{ position: 'absolute', bottom: 20, right: 30, opacity: 0.5 }}>
        <Text style={{ color: theme.textSub, fontSize: 10 }}>حي الصحافة</Text>
      </View>
      <View style={{ position: 'absolute', top: 40, right: 50, opacity: 0.5 }}>
        <Text style={{ color: theme.textSub, fontSize: 10 }}>حي النرجس</Text>
      </View>
      <View style={{ position: 'absolute', bottom: 50, left: 45, opacity: 0.5 }}>
        <Text style={{ color: theme.textSub, fontSize: 10 }}>حي الملقا</Text>
      </View>
