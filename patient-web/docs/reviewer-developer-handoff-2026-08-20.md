...
**تحقق الهاتف المتزامن:** فُحصت العربية والإنجليزية على عرض `390px`. بقيت العناوين والنصوص والأزرار قابلة للقراءة، وتحولت بطاقات الثقة إلى امتداد رأسي طبيعي بعد الدعوات من دون قص أفقي. يستمر محدد اللغات في تمرير أفقي مقصود لاستيعاب اللغات الست بدلاً من تقليل حجم النص إلى مستوى غير مقروء.

**تحقق الملف الصحي Premium:** أظهر حساب Sandbox بطاقات الهوية والملخص الطبي والتأمين بالحقول المصرح بها فقط، مع حالات فارغة صريحة للمجالات التي لا ترجع بيانات عرض. حجبت الواجهة قيمة الجنس التقنية `unspecified` بدلاً من عرضها للمريض أو تخمين ترجمتها؛ يلزم Backend بتوثيق enum محلي أو حقل عرض قبل إتاحتها.
...
## 8. ملحق تحديث التصميم والأمان والاكتشاف — 20 أغسطس 2026

> يحل هذا الملحق محل أي انطباع سابق بأن لغة التصميم بقيت عامة أو أن الحركة لم توحد بعد. لا يغير ذلك قرار عدم النشر النهائي قبل إغلاق العقود الخلفية.

### ما تغير بعد اللقطة الأولى

| المجال | التنفيذ والتحقق |
|---|---|
| هوية Premium | علامة نبض/حماية Vector أصلية، نظام طباعي يعتمد خط الجهاز مع بدائل عربية ومتعددة النصوص، مواد زجاجية محدودة، وأيقونات Lucide/Vector بلا emoji أو نسخ لأصول Apple. |
| الحركة | نظام موحد قصير وهادف: ضغط وتحريك انتقالات ضمن `120–220ms` و`transform`/`opacity` فقط؛ يعطل الحركة غير الأساسية عند `prefers-reduced-motion`. |
| الدخول والتنقل | صفحة دخول Premium وصولية، رأس علامة أقوى، ومحدد لغات منظم؛ تحققت العربية والإنجليزية على `390px` و`1280px`. نموذج الدخول لا يثبت أي بيانات Sandbox في HTML. |
| القوائم والتفاصيل | ترقية القوائم الخاصة والملف الصحي والملخص الحيوي وكتالوج المعلومات، ثم تفاصيل الطلب والموعد والدواء والفحص. صفحات التفاصيل تثبت روابط رجوع مقيدة، ولا تضيف عملاً أو حقلاً خارج العقد. |
| حواجز SSR | توسعت اختبارات التفاصيل لتمنع تسرب التوكن، واسم المريض، والعنوان، والسعر، والتقارير والمرفقات، وتتحقق من رابط الرجوع الآمن. |
| Sandbox | نجح تحقق ملكية الطلب والموعد بعد الدفعة. أي تحقق خارجي بطيء شغل بمهلة موسعة ومن دون عملية تعديل على الإنتاج. |
| الاكتشاف العام | الصفحة الرئيسية وحدها تحصل على Open Graph/Twitter. اكتملت بدائل `hreflang` للغات الست و`x-default` للصفحة الرئيسية والكتالوج، بينما يحتفظ الكتالوج المختلط بـ`noindex` ولا يضاف له `Drug` أو عرض/سعر. |
...
|---|---|
| `pnpm check` | ناجح. |
| `pnpm test` | **53 ملفاً ناجحاً، 85 اختباراً ناجحاً؛ 14 ملف Sandbox و23 اختباراً معزولة افتراضياً.** |
| `pnpm build` | ناجح على Next.js 16.3.1. |
| تحقق HTTP | الصفحة العامة تعرض Open Graph/Twitter؛ `/en/login` يعيد `X-Robots-Tag: noindex, nofollow, noarchive`؛ الكتالوج المختلط يبقى `noindex`. |

### مراجع التصميم والاكتشاف

توثق `docs/premium-design-research.md` قرارات الحركة والمواد والوصولية المستلهمة من Apple وMaterial وNHS دون نسخ هوية. ويوثق `docs/seo-public-review-2026-08-20.md` قرار الاكتشاف العام من مراجع Google: المحتوى الأصلي المراجع وبنية HTML الواضحة أهم من أي «وسم خاص بالذكاء الاصطناعي»، ولا يوجد ضمان ترتيب.[1] [2]
