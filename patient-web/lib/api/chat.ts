...
/**
 * Thread-detail variant (participants only — the upstream already enforces
 * membership): adds the message body and own/other alignment to the strict
 * summary. Never use this for list surfaces.
 */
export type ChatThreadMessage = ChatMessageSummary & { body: string; mine: boolean };
export function extractChatThreadMessages(payload: unknown, viewerRole = "patient"): ChatThreadMessage[] {
  return extractChatMessageSummaries(payload).flatMap((summary) => {
    const root = asRecord(payload);
    const list = Array.isArray(root?.messages) ? root.messages : listFrom(payload);
    const match = list.find((value) => asRecord(value)?.id === summary.id);
    const body = match && typeof asRecord(match)?.body === "string" ? String(asRecord(match)?.body).slice(0, 4000) : "";
    return [{ ...summary, body, mine: summary.senderRole === viewerRole }];
  });
}
