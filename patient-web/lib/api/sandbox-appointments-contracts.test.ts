...
function firstAppointmentId(value: unknown): string | undefined {
  const root = value && typeof value === "object" ? value as Record<string, unknown> : null;
  const items = Array.isArray(value) ? value : Array.isArray(root?.data) ? root.data : Array.isArray(root?.appointments) ? root.appointments : [];
  const first = items[0];
...
describeSandbox("Sandbox appointment contracts", () => {
  it("allows a patient to read only their own appointment list and rejects another patient if an item exists", async () => {
    ...
    const listResponse = await fetch(`${baseUrl}/care/appointments`, { headers: { authorization: `Bearer ${ownerToken}` }, signal: AbortSignal.timeout(12_000) });
    expect(listResponse.status).toBe(200);
    const appointmentId = firstAppointmentId(await listResponse.json().catch(() => null));
    if (!appointmentId) return;

    const ownerDetail = await fetch(`${baseUrl}/care/appointments/${appointmentId}`, { headers: { authorization: `Bearer ${ownerToken}` }, signal: AbortSignal.timeout(12_000) });
    expect(ownerDetail.status).toBe(200);
    const otherDetail = await fetch(`${baseUrl}/care/appointments/${appointmentId}`, { headers: { authorization: `Bearer ${otherToken}` }, signal: AbortSignal.timeout(12_000) });
