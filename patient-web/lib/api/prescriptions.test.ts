...
describe("prescription response guards", () => {
  it("keeps state, item count, and creation time only", () => {
    ...
    expect(prescriptions).toEqual([{ id: prescriptionId, state: "CREATED_BY_DOCTOR", itemCount: 1, createdAt: "2026-08-20T10:00:00.000Z", doctorName: undefined, medicationNames: ["private"], items: [{ name: "private", dose: "private", frequencyHours: undefined, durationDays: undefined, instructions: undefined }] }]);
  });
});
