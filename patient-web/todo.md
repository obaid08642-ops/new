...
- [x] OpenAPI: أضيف مولد عقد موحد بـproduction server `https://api.nabd.plus/api/v1` وBearer JWT مسمى `access-token`؛ لا OAuth scopes مصطنعة لأن role/ownership endpoint-level. وثقت `GET /user/insurance` كـdeprecated compatibility (`insurance_policies`)، و`GET/POST /users/me/insurance` كـcanonical (`insurance`) و`GET /insurance/active` كـprojection (`insurance_details`) وcompany catalog، مع 401/403/schemas. سُجل Controller legacy الذي كان غير مسجل في module، واختبار OpenAPI regression (4) وbuild و69 suites/397 tests تمر. Swagger production ظل gated ولم يفعل. الدليل: `NABDAH_OPENAPI_INSURANCE_CONTRACT_HARDENING_20260820.md`.
- [x] الكتالوج العام: أضيفت eligibility/review/provenance fail-closed إلى Medicine/ProviderProfile/Facility/Lab/Radiology/HomeCare؛ قُيدت أسطح public/SEO/discovery والخدمات العامة بـ`public_eligibility:true` و`medical_review_status:'approved'` فوق الحالة التشغيلية. أضيف `CatalogPublicationService` لإسقاط public مدقق، metadata/sitemap/feed/deep-link، invalidation وevent idempotent. استُبدل backfill الشامل بـsmart backfill: الأدوية legacy `verified:true` والمزود النشط ذو ترخيص verified فقط يرثان approval مع provenance؛ غير ذلك pending مخفي. اجتازت Backend build و73 suites/411 tests. لا migration أو نشر. الأدلة: `NABDAH_CATALOG_GOVERNANCE_PUBLICATION_HARDENING_20260820.md` و`NABDAH_SMART_CATALOG_BACKFILL_REVIEWER_RUNBOOK_20260820.md`.
- [x] projection/events: بعد اعتماد medicine/provider أو ApprovalWorkflow، يُحدث `CatalogPublicationService` projection/cache/metadata/sitemap/feed/deep-link متزامناً ويسجل `catalog.publication.projected` بمفتاح idempotency sparse-unique؛ replay لا يعمل fanout ثانياً. اختبارات publication/EventBus وsmart-backfill مغطاة ضمن 73 suites/411 tests. لا worker خارجي أو نشر background أو إرسال sitemap فعلي.
...
## توحيد كتالوج التأمين والشعارات — 2026-08-20

- [x] توحيد جميع مستهلكي شركات وفئات التأمين التشغيلية في Patient وProvider وAdmin على Backend: `GET /insurance/companies` ثم `GET /insurance/companies/:id/networks`، بلا fallback شركات أو فئات محلي.
- [x] توسيع حوكمة Backend غير الحذفّية للشركات والشبكات: تحتفظ الكيانات التاريخية، وتحجب الشبكات `retired` من العرض العام، وتبقى الشركات غير النشطة مرئية للإدارة والتاريخ التعاقدي فقط.
- [x] توحيد controller الثانوي مع `InsuranceService.listCompanies()` وتحديث عقد OpenAPI المرتبط.
- [x] إضافة 17 مرشح شعار WebP صغيراً وشفافاً من مواقع شركات التأمين الرسمية فقط، موثقاً بمصدر رسمي وبصمة SHA-256 لكل أصل؛ لا توجد شعارات مولدة أو من مجمعات طرف ثالث.
- [x] تصحيح `assets/insurance-logos/manifest.json`: العدد المعلن والفعلي **31**، مع 17 `official_source_collected` و14 `uncollected`؛ تحققت جميع المسارات والبصمات محلياً بنجاح.
- [x] ربط إظهار `logo_url` الخادمي المتاح في اختيار التأمين بحجز الاستشارة في Patient وبشاشة الإعداد المشتركة لجميع أنواع Provider، مع fallback واجهة محايد فقط عند غياب شعار موثق.
- [x] تحديث لوحة Admin لإظهار الشعار وحالة الكتالوج وعلاقة الخلف ومنع الحذف النهائي للفئات/السجلات التاريخية.
- [x] تشغيل البوابات بعد التعديلات: Patient typecheck و23/59 Jest، Provider typecheck و1/31 Jest، Backend build و73/411 Jest، وفحص manifest وأداة نشر الأصول بوضع dry-run.
- [ ] استكمال 14 شعاراً معلّقاً من موقع الشركة أو مادة هوية رسمية فقط؛ لا يُستبدل أي منها بصورة من محرك بحث أو موقع طرف ثالث. تشمل المسارات الانتقالية/المحجوبة: Buruj، AXA، Alalamiya، United Cooperative، Gulf Union، Al Ahlia، Wafa، Malath، Allianz SF، Gulf General، Alinma Tokio Marine، Al Etihad، Wataniya، Saudi Enaya.
- [ ] اعتماد ربط الـ17 أصل المرشح بتخزين R2/CDN وكتابة `logo_url` الحي عبر مراجع/DevOps فقط؛ الأداة `publish-insurance-logo-assets.ts` تبقى dry-run افتراضياً ولا تُشغّل بـ`--apply` دون بيانات تخزين معتمدة.
- [ ] اعتماد فئات كل شركة من وثيقة شبكة/منتج رسمية منفصلة؛ لا تُعمم فئات عامة بين الشركات.

آخر تحديث: 2026-08-20 — لم يُنفذ أي نشر للخادم أو رفع إلى CDN في هذه الدفعة.


## دفعة مرشحي الشعارات وحوكمة فئات التأمين — 2026-08-20

- [x] تحقق قراءة فقط من CDN الإنتاج: `bupa` و`tawuniya` و`cigna` أعادت `200 image/webp` من المسار `catalog/insurance-logos/`، وظهر `logo_url` و`logo_sha256` من مسار API العام؛ أبلغ المالك أن الأصول الـ17 السابقة منشورة ومطابقة للبصمات.
- [x] جمع أصل الخليجية العامة من مورد موقع الشركة الرسمي، وتحويله إلى `gulf_general.webp` lossless transparent (56×79) وتوثيق بصمته SHA-256 في manifest.
- [x] جمع أصل عناية السعودية من `saudienaya.com/assets/img/logo.svg`، وتحويله إلى `saudi_enaya.webp` lossless transparent (256×77) وتوثيق بصمته. يبقى السجل `pending_review` وغير نشط ولا يربط أو يعرض للعامة قبل اعتماد علاقة الخلف/الاندماج.
- [x] تحقق manifest: 31 سجلاً، 19 شعاراً موثق المصدر و12 معلّقاً؛ جميع مسارات الأصول وبصمات SHA-256 سليمة. وضعا `publish-insurance-logo-assets.ts` و`reconcile-insurance-catalog.ts` dry-run فقط ولم ينفذا أي رفع أو تغيير بيانات.
- [x] تدقيق فئات API الحي: الفئات `a/b/c1/c2/c3/vip` موجودة في 30/30 شركة، ولذلك سُجلت كإرث غير مسند لا كفئات رسمية معتمدة. وثيقة الحوكمة تحدد backfill توثيقياً بلا أي تغيير لما يراه المريض.
- [x] بوابات Backend بعد الدفعة: build ناجح و73 suites/411 tests ناجحة.
- [ ] يرفع المراجع `gulf_general.webp` فقط إلى CDN بعد مراجعة البصمة ثم يربط `logo_url`؛ لا يرفع أو يربط `saudi_enaya.webp` قبل قرار الكيان الانتقالي.
- [ ] استكمال 12 شعاراً معلقاً من نطاقات الشركات أو مواد هوية رسمية فقط، مع إبقاء بروج/AXA/العالمية/وفا/Allianz SF وغيرها خاضعة لحالة الاندماج أو التقاعد الموثقة.
- [ ] اعتماد كل فئة شركة من صفحة منتج/شبكة رسمية وملء `source_url` و`source_label` و`verified_at` و`catalog_status` و`provenance` لكل `insurance_network`؛ لا تنفذ سياسة إخفاء للفئات الموروثة قبل قرار مالك واختبار مقارنة قبل/بعد.
