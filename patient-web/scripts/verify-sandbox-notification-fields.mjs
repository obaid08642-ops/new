...
if (!baseUrl || !identifier || !password) throw new Error("Notification verification requires configured Sandbox environment variables");
...
function firstRow(payload) {
  ...
  for (const value of [payload.data, payload.items, payload.results, payload.notifications]) if (Array.isArray(value)) return value[0];
  return undefined;
}
...
const response = await fetch(`${baseUrl}/notifications`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });
...
console.log(`notifications: ${keys.length ? `keys=${keys.join(",")}` : "empty result"}`);
