...
if (!baseUrl || !identifier || !password) throw new Error("Chat verification requires configured Sandbox environment variables");
...
function firstRow(payload) {
  ...
  for (const value of [payload.data, payload.items, payload.results, payload.threads]) if (Array.isArray(value)) return value[0];
  return undefined;
}
...
if (!response.ok) throw new Error(`Chat threads returned ${response.status}`);
const row = firstRow(await response.json());
const keys = row && typeof row === "object" && !Array.isArray(row) ? Object.keys(row).sort() : [];
console.log(`chat: ${keys.length ? `thread_keys=${keys.join(",")}` : "empty result"}`);
