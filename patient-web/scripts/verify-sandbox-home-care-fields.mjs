...
if (!baseUrl || !identifier || !password) throw new Error("Home-care verification requires configured Sandbox environment variables");
...
const response = await fetch(`${baseUrl}/home-care/bookings/my`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });
if (!response.ok) throw new Error(`Home-care list returned ${response.status}`);
...
console.log(`home-care: ${keys.length ? `keys=${keys.join(",")}` : "empty result"}; id_format=${id ? (id.length === 36 ? "uuid_length" : `length_${id.length}`) : "none"}`);
