...
if (!baseUrl || !identifier || !password) throw new Error("Reminder verification requires configured Sandbox environment variables");
...
function firstRow(payload) {
  ...
  for (const value of [payload.data, payload.items, payload.results, payload.reminders]) if (Array.isArray(value)) return value[0];
  return undefined;
}
...
const response = await fetch(`${baseUrl}/health/reminders`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });
if (!response.ok) throw new Error(`Medication reminders returned ${response.status}`);
const row = firstRow(await response.json());
const keys = row && typeof row === "object" && !Array.isArray(row) ? Object.keys(row).sort() : [];
console.log(`reminders: ${keys.length ? `keys=${keys.join(",")}` : "empty result"}`);
