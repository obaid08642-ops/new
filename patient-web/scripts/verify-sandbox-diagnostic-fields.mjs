...
if (!baseUrl || !identifier || !password) throw new Error("Diagnostic verification requires configured Sandbox environment variables");
...
function firstRow(payload) {
  ...
  for (const value of [payload.data, payload.items, payload.results]) if (Array.isArray(value)) return value[0];
  return undefined;
}
...
for (const domain of ["labs", "radiology"]) {
  const response = await fetch(`${baseUrl}/${domain}/bookings/mine`, { headers: { authorization: `Bearer ${accessToken}` }, signal: AbortSignal.timeout(12_000) });
  if (!response.ok) throw new Error(`${domain} list returned ${response.status}`);
  const row = firstRow(await response.json());
  const keys = row && typeof row === "object" && !Array.isArray(row) ? Object.keys(row).sort() : [];
  const id = row && typeof row === "object" && typeof row.id === "string" ? row.id : undefined;
  console.log(`${domain}: ${keys.length ? `keys=${keys.join(",")}` : "empty result"}; id_format=${id ? (id.length === 36 ? "uuid_length" : `length_${id.length}`) : "none"}`);
}
