...
const getRequiredConsents = () => ({
  terms: process.env.NABD_TERMS_POLICY_ID || "terms",
  termsVersion: process.env.NABD_TERMS_POLICY_VERSION || "v1",
  privacy: process.env.NABD_PRIVACY_POLICY_ID || "privacy",
  privacyVersion: process.env.NABD_PRIVACY_POLICY_VERSION || "v1",
});
...
export async function POST(request: Request) {
  ...
  const configured = getRequiredConsents();
  if (!configured.terms || !configured.termsVersion || !configured.privacy || !configured.privacyVersion) {
    return NextResponse.json({ message: "registration_policy_configuration_missing" }, { status: 503, headers: { "cache-control": "no-store" } });
  }
