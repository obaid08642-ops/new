...
function jsonRequest(path: string, body: unknown, headers?: HeadersInit) { return new Request(`https://web.test${path}`, { method: "POST", headers: { "origin": "https://web.test", "referer": "https://web.test/", "content-type": "application/json", ...headers }, body: JSON.stringify(body) }); }
...
describe("patient OTP BFF routes", () => {
  ...
  it("sends only the exchange cookie upstream and never returns token-shaped data", async () => {
    ...
    const response = await exchangeSession(new Request("https://web.test/api/auth/session/exchange", { method: "POST", headers: { origin: "https://web.test", referer: "https://web.test/", cookie: "nabd_otp_exchange=secret; unrelated=drop", "x-nabd-device-id": "device-test" } }));
  ...
  it("requires the one-time exchange cookie", async () => {
    expect((await exchangeSession(new Request("https://web.test/api/auth/session/exchange", { method: "POST", headers: { origin: "https://web.test", referer: "https://web.test/" } }))).status).toBe(400);
  });
});
