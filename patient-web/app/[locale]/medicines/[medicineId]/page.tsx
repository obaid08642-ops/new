...
import { MedicineAddToCart } from "@/components-next/medicine-add-to-cart";
...
export default async function MedicineDetailPage({ params }: Props) {
  ...
  return <main className={`main ${styles.page}`}><JsonLd data={schema} />
    ...
    <section className={styles.detail} aria-label={name}><dl className={styles.grid}>{facts.map(([label, value]) => <div className={styles.item} key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><p className={styles.notice}>{t("detailNotice")}</p><MedicineAddToCart medicineId={medicine.id} locale={locale} labels={{ addToCart: cartT("addToCart"), adding: cartT("adding"), checkoutFailed: cartT("checkoutFailed") }} /></section>
  </main>;
}
