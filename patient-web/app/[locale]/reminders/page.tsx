...
import { VectorChronicCare } from "@/components-next/vector-illustrations";
...
export default async function RemindersPage({ params }: Props) {
  ...
  const doseRows = reminders.flatMap((reminder) =>
    (reminder.todayDoses.length ? reminder.todayDoses : reminder.times.map((timeKey) => ({ timeKey, status: "pending" as const }))).map((dose) => ({
      reminder,
      ...dose,
    })),
  );
    ...
    <main className={`main ${styles.page}`}>
      <section className={styles.intro}>
        ...
        <div className={styles.introVector}>
          <VectorChronicCare size={48} aria-hidden="true" />
        </div>
      </section>
      {reminders.length ? (
          ...
          {reminders.map((reminder) => (
            <article className={styles.card} key={reminder.id}>
              ...
              <div className={styles.cardBody}>
                ...
                {reminder.dose ? <span className={styles.dose}>{reminder.dose}</span> : null}
      ...
      <p>
        <Link className={styles.cta} href={`/${locale}/reminders/add`}>
          {locale === "ar" ? "تذكير جديد" : "New reminder"}
        </Link>
      </p>
    </main>
  );
