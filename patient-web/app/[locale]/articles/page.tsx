    ...
    <main className={`main ${styles.page}`}>
      <section className={styles.hero}>
        ...
        <form className={styles.search} method="get">
          ...
          <input name="q" defaultValue={query.q || ""} maxLength={80} placeholder={t("searchPlaceholder")} aria-label={t("searchPlaceholder")} />
      ...
      {articles.length ? (
          ...
          {articles.map((article) => (
            <Link className={styles.card} key={article.slug} href={`/${locale}/articles/${article.slug}`}>
              ...
              <span className={styles.copy}>
                <strong dir="auto">{locale === "ar" ? article.titleAr || article.titleEn : article.titleEn || article.titleAr || t("untitled")}</strong>
                <span dir="auto">{article.category || t("categoryUnavailable")}</span>
              </span>
              <Chevron className={styles.arrow} size={18} aria-hidden="true" />
            </Link>
          ))}
        </section>
      ) : (
        <section className={styles.empty}>
          ...
          <p>{query.q || query.category ? t("noResults") : t("empty")}</p>
        </section>
      )}
    </main>
  );
