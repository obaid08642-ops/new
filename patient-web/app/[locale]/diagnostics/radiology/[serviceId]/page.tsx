...
export default async function RadiologyServiceDetailPage({ params }: Props) {
  ...
  return <main className={`main ${styles.page}`}><JsonLd data={[serviceLd({ name: name ?? t("title"), path: `/diagnostics/radiology/${serviceId}`, locale, description: description ?? null }), breadcrumbList([{ name: t("title"), locale, path: "/diagnostics/radiology" }, { name: name ?? t("title"), locale, path: `/diagnostics/radiology/${serviceId}` }])]} /><Link className={styles.back} href={`/${locale}/diagnostics/radiology`}><Arrow size={17} aria-hidden="true" />{t("backToRadiology")}</Link><section className={styles.hero}><div><p className={styles.eyebrow}><ShieldCheck size={15} aria-hidden="true" />{t("eyebrow")}</p><h1>{name}</h1><p className={styles.subtitle}>{description ?? t("detailDescriptionUnavailable")}</p></div><span className={styles.heroIcon}><Image size={28} aria-hidden="true" /></span></section><section className={styles.detailCard} aria-label={t("detailTitle")}><div className={styles.meta}>{service.modality ? <span>{service.modality}</span> : null}{service.bodyPart ? <span>{service.bodyPart}</span> : null}{service.price !== undefined ? <span>{t("price", { value: service.price })}</span> : null}{service.durationMinutes !== undefined ? <span>{t("duration", { value: service.durationMinutes })}</span> : null}{service.turnaroundHours !== undefined ? <span>{t("turnaround", { value: service.turnaroundHours })}</span> : null}</div><div className={styles.badges}>{service.homeVisitSupported ? <span>{t("homeVisit")}</span> : null}{service.facilityVisitSupported ? <span>{t("facilityVisit")}</span> : null}{service.contrastRequired ? <span>{t("contrast")}</span> : null}{service.fastingRequired ? <span>{t("fasting")}</span> : null}</div>{preparation?.length ? <div className={styles.section}><h2>{t("preparationTitle")}</h2><ul>{preparation.map((item) => <li key={item}>{item}</li>)}</ul></div> : null}<p className={styles.notice}>{t("bookingUnavailableNotice")}</p></section></main>;
}
