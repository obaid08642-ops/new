...
export default async function DiagnosticDetailPage({ params }: Props) {
  ...
  if (!response.ok) return <main className={`main ${styles.page}`}><section className={styles.state} role="alert"><VectorLabs size={48} aria-hidden="true" /><h1 style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const, overflow: "hidden", overflowWrap: "anywhere" }}>{t("unavailableTitle")}</h1><p style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const, overflow: "hidden", overflowWrap: "anywhere" }}>{t("unavailable")}</p><RetryButton /></section></main>;
  const booking = extractDiagnosticBooking(await response.json().catch(() => null));
  if (!booking) return <main className={`main ${styles.page}`}><section className={styles.state} role="alert"><VectorLabs size={48} aria-hidden="true" /><h1 style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const, overflow: "hidden", overflowWrap: "anywhere" }}>{t("unavailableTitle")}</h1><p style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const, overflow: "hidden", overflowWrap: "anywhere" }}>{t("unavailable")}</p><RetryButton /></section></main>;
  ...
  return <main className={`main ${styles.page}`}>
    ...
    <section className={styles.hero}><div className={styles.heroText}><p className={styles.eyebrow}><ShieldCheck size={15} aria-hidden="true" />{t("eyebrow")}</p><h1 style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const, overflow: "hidden", overflowWrap: "anywhere" }}>{label}</h1><span className={styles.status}>{status}</span></div><span className={styles.heroIcon}>{domain === "labs" ? <VectorLabs size={48} aria-hidden="true" /> : <VectorRadiology size={48} aria-hidden="true" />}</span></section>
