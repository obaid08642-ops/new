...
export default async function MentalHealthPage({ params }: Props) {
  ...
  const cards = [
    [t("moodEntries"), data.mood.totalEntries],
    [t("avgMood"), data.mood.avgMood ?? t("notAvailable")],
    [t("avgEnergy"), data.mood.avgEnergy ?? t("notAvailable")],
    [t("avgStress"), data.mood.avgStress ?? t("notAvailable")],
    [t("meditationSessions"), data.meditation.totalSessions],
    [t("meditationMinutes"), data.meditation.totalMinutes],
  ];
    ...
    <main className={`main ${styles.page}`}>
      ...
      <section className={styles.grid} aria-label={t("title")}>
        {cards.map(([label, value]) => (
          <div className={styles.card} key={String(label)}>
            <span>{label}</span>
            <strong>{value}</strong>
          </div>
        ))}
