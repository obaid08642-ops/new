...
import { ArrowLeft, ArrowRight, Clock3, HousePlus, ShieldCheck } from "lucide-react";
import { extractHomeCareService } from "@/lib/api/home-care-services";
import { getPublicHomeCareService } from "@/lib/api/home-care-services-server";
import { isLocale } from "@/lib/i18n";
import { HomeCareBookingForm } from "@/components-next/home-care-booking-form";
...
export default async function HomeCareServicePage({ params }: Props) {
  ...
  const t = await getTranslations("HomeCareServices"); const response = await getPublicHomeCareService(serviceId);
  if (!response || response.status === 404) notFound();
  if (!response.ok) return <main className={`main ${styles.page}`}><section className={styles.state} role="alert"><HousePlus size={28} aria-hidden="true" /><h1>{t("unavailableTitle")}</h1><p>{t("unavailableBody")}</p><Link href={`/${locale}/home-care/services`} className={styles.action}>{t("back")}</Link></section></main>;
