...
export default async function HealthTimelinePage({ params }: Props) {
  ...
  return <main className="main" style={{ padding: "24px 16px", maxWidth: 720, margin: "0 auto", background: "#F5F5F7", minHeight: "60vh" }}>
    <Link href={`/${locale}/health`} style={{ display: "inline-flex", alignItems: "center", gap: 4, color: "#16213A", textDecoration: "none", marginBottom: 12 }}><ChevronLeft size={17} />{t("back")}</Link>
    <h1 style={{ display: "flex", alignItems: "center", gap: 8, color: "#16213A" }}><FileClock size={22} />{t("title")}</h1>
      ...
      <ul style={{ listStyle: "none", padding: 0, display: "grid", gap: 10 }}>
        {list.map((item: any, i: number) => (
          <li key={String(item?.id ?? i)} style={{ background: "#fff", borderRadius: 18, padding: "16px 18px", boxShadow: "0 4px 14px rgba(22,33,58,.06)", color: "#16213A" }}>
            <strong>{String(item?.title ?? item?.type ?? item?.kind ?? "")}</strong>
            <span style={{ display: "block", fontSize: 13, opacity: .6, marginTop: 4 }}>{String(item?.created_at ?? item?.date ?? "").slice(0, 16)}</span>
          </li>
        ))}
      </ul>
    )}
