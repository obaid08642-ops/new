    ...
    <main className={`main ${styles.page}`}>
      ...
      {meds.length ? (
          ...
          {meds.map((med) => (
            <article className={styles.card} key={med.id}>
              <div className={styles.cardTop}>
                <span>{med.active ? t("active") : t("inactive")}</span>
                <span className={styles.glyph}>
                  <Pill size={18} aria-hidden="true" />
                </span>
              </div>
              <p className={styles.value}>{med.name || t("unnamed")}</p>
              {med.dose ? <p className={styles.date}>{med.dose}</p> : null}
              {med.frequency ? (
        ...
        <section className={styles.state}>
          <VectorPharmacy size={42} aria-hidden="true" />
          <p>{t("empty")}</p>
        </section>
      )}
