...
import { PharmacyChatClient } from "@/components-next/pharmacy-chat-client";
...
/** Parity with app pharmacist-chat: negotiation threads, messages, substitute decisions. */
    ...
    <main className="main" style={{ display: "grid", gap: 12, padding: "24px 0 64px" }}>
      <section style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: 24, border: "1px solid #E8EDEE", borderRadius: 20, background: "linear-gradient(135deg, #FDFDFC 0%, #F0FDF9 60%, #E7FFF6 100%)", boxShadow: "0 12px 32px rgba(30,51,46,.07)" }}>
        <div style={{ display: "grid", gap: 6 }}>
          ...
          <h1 style={{ margin: 0, color: "#1E332E", fontSize: 20, fontWeight: 900, lineHeight: 1.3, overflowWrap: "anywhere", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{ar ? "التفاوض مع الصيدلية" : "Pharmacy negotiation"}</h1>
        </div>
      ...
      <PharmacyChatClient orderId={orderId} locale={locale} />
          </section>
    </main>
  );
}
