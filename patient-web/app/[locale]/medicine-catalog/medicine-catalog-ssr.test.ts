...
describe("public medicine catalogue SSR boundary", () => {
  ...
  // I-wave1: the catalogue index page is public/indexable; detail pages stay noindex until DTO.
  it("marks the catalogue index as indexable with canonical + language alternates", async () => {
    const metadata = await generateMetadata({ params: Promise.resolve({ locale: "en" }), searchParams: Promise.resolve({ q: "query", page: "1" }) });

    expect(metadata.robots).toMatchObject({ index: true, follow: false });
