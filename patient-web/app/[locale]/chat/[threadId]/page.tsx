...
export default async function ChatThreadPage({ params }: Props) {
  ...
  return (
    <main className={`main ${styles.page}`}>
      <Link className={styles.activity} href={`/${locale}/chat`}>
      ...
      <section className={styles.intro}>
        <div className={styles.introText}>
          ...
          <h1>{thread ? t(`types.${thread.type}`) : t("thread")}</h1>
        </div>
          ...
          messages.map((message) => (
            <article className={styles.card} key={message.id}>
              <span className={styles.cardIcon}>
                {message.hasAttachment ? <FileText size={19} aria-hidden="true" /> : <MessageCircle size={19} aria-hidden="true" />}
              </span>
              <div className={styles.cardBody}>
                <strong className={styles.type}>{message.deleted ? t("deleted") : t(`messageTypes.${message.type}`)}</strong>
      ...
      <p className={styles.notice}>{t("bodyHidden")}</p>
    </main>
  );
}
