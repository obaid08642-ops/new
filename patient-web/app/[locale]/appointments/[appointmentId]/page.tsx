...
export default async function AppointmentDetailPage({ params }: Props) {
  ...
  if (!response.ok) return <main className={`main ${styles.page}`}><section className={styles.state} role="alert"><VectorDoctor size={48} aria-hidden="true" /><h1>{t("unavailableTitle")}</h1><p>{t("unavailableBody")}</p><RetryButton /></section></main>;
  const appointment = extractAppointmentDetail(await response.json().catch(() => null));
  if (!appointment) return <main className={`main ${styles.page}`}><section className={styles.state} role="alert"><VectorDoctor size={48} aria-hidden="true" /><h1>{t("unavailableTitle")}</h1><p>{t("unavailableBody")}</p><RetryButton /></section></main>;
  ...
  return <main className={`main ${styles.page}`}>
    ...
    <section className={styles.hero}>
      ...
      <span className={styles.heroIcon}><VectorDoctor size={48} aria-hidden="true" /></span>
    </section>
